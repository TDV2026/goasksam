...
9. Powerseller lead is GATED, never default. Leading with a partner requires ALL of: estimated value from actual comps above POWERSELLER_MIN_VALUE_USD (75000, env-overridable), car segment matching the partner's specialties, seller region within partner coverage, and an active matching partner in the partners table. Any condition fails: platform recommendation is primary and the partner never leads. Amendment (July 2026, dual-option): when the lead gate fails but the context is $50k+ (met-comps estimate or asking price) and a region-covered active partner exists, the partner ALWAYS renders as a modest secondary "also worth considering" card, no segment match required (the make-specific why-line falls back to his attributed specialty note). A stated DIY preference suppresses it per rule 10.
...
15. Recommendations are final. No hedging, no escape hatches, no "feel free to revisit." If the user questions, explain the tradeoff. Never offer alternatives unless asked. Recommendation closes are declarative statements ending in a period, never questions.
...
21. Refinement chip copy reruns the evidence, never promises a different answer (Sep 2026, copy template; the surface expression of rule 18). Any refinement affordance (mileage, transmission, config marker, observable fact) must frame the action as changing WHICH REAL SALES are used, not as producing a new/better number or range. Template: "Still around 66,700 miles? If not, update it and I'll rerun the market around the current mileage." Allowed verbs: rerun / recompute / pull the sales that match / narrow to. BANNED: any phrasing that claims or implies a different range, a higher/lower number, a more accurate value, or "tighten the read" (valuation language). A restated fact already shown elsewhere (e.g. the exact-car hero's "Last sold $X ... N mi") is never repeated in refinement or contradiction copy just to have something there; if a line adds no new information, omit it.
...
Decision coherence (locked): evidence-only consignment houses (RM Sotheby's, Gooding, ACC) are routable:false, can never be the pick or a comparison card, and a stronger non-routable source is always explained with its concrete reason. Result chips, bullets and headlines derive from one routeFacts object with consistency rules. All source slugs map to display names (platformDisplayName). Result framing: comparison-led or every-sale-closed-here, never small-sample fractions as headlines; weekday timing insights sit in the headline bullets with grounding. PowerSeller preference (updated, overrides the prior "involvement question does not exist" rule): the wizard asks a final PowerSeller-preference question (step 8) AFTER the summary is confirmed and before results compile ("How would you like to handle the sale?", chips Yes-PowerSeller / No-handle-myself / Not sure, dash-free subtext). It stores sellState.sellerPreference ("powerseller" | "diy" | "unsure"). Step 8 is the SINGLE ask: there is NO post-result "Want it handled, or run it yourself?" re-ask (the old awaitingPathChoice chips are removed). The preference alone drives the result layout: "powerseller" renders PowerSeller-forward (the howS block leads under its own TRACK-RECORD plate, the platform cards sit below); "diy" renders platform-first (the platform holds the pick plate) with the PowerSeller block quietly below; "unsure" renders platform-first with the PowerSeller block prominent below, both doors visible and the click is the choice. The pick badge follows the DATA (the evidence platform) in every case except "powerseller", where the howS-forward composition leads; the old "IF SELLING YOURSELF" demotion of the platform card is gone. The PowerSeller dossier's plate is track-record chrome ("{Name}'s track record"), visually distinct from the market Data plates and never borrowing "Sam's pick"/"Data: 180 days". The "squarely in his lane" claim requires a genuine specialty match with the car (era/generation proximity to the partner's stated specialties, e.g. air-cooled/vintage); make-level-only history weakens to the data-true form ("Porsches are core to his tracked record"). No own-voice fee-value claims ("the fee earns its keep", "my personal preference") render. A stated DIY preference still permanently suppresses re-pitching. Chat grounding: the chat layer never contradicts the engine's recommendation (decision facts are injected into post-result context), never states platform mechanics or fees as fact, never invents market commentary.
...
- Partner premium tiles move on their own (documented so nobody rediscovers the mechanism): the +N% "Track record" tile is the data_verified premium PRE-computed offline by task=premium&persist=1 (api/usageDashboard.js), NOT a request-time value (unlike the tracked-sales count/top-makes, which are request-time). It is recomputed NIGHTLY as a third job in .github/workflows/nightly.yml, AFTER ingest + warm (so the archive is freshest), over ALL FIVE partners. Each partner's figure is written as one atomic PATCH of that partner's specialties JSON (whole object replaced at once), so a seller never sees a half-updated number; changes propagate within the ~10-min in-memory partners cache. Archive-only, zero OldCarsData spend. A partner dropping below the n>=10 positive-median gate has its tile CLEARED (correct outcome, not a failure). Silent-failure surfacing is three-level: a per-partner log line every night, a RED run (curl -fsS plus a jq guard on wrote==true) that fires GitHub's failure email on any network/HTTP/persist error, and a named culprit line ("NOT WRITTEN: <partner>"). Requires the PROBE_KEY GitHub Actions secret (separate from the Vercel env var of the same name).
...
## Locked layout rules
- One Box hierarchy (locked Sep 2026): EVERY answer state gets ONE dominant visual object at most; all other evidence is compact rows (with a thumbnail, or a clearly marked "no photo" placeholder slot - never a text-only row). General match: the CLOSEST MATCH photo card is dominant (full content width), FEWER MILES + MORE MILES stack as compact rows below it (never a side-by-side column - that squeezed and clipped the lower card). Exact car: the seller's OWN car hero is the only large photo; every market receipt below is compact, never a second large CLOSEST MATCH card competing with the owner's car. House-led: the class/car context is dominant; house transactions stay compact rows and no large "closest" receipt is invented (it is class evidence, not an individual-car comparison). This is consistency of hierarchy, not identical component geometry across states. Verified no clip/overflow at desktop and 390px.
...
- Pre-war Bentley ½-glyph comp-matching gap (Sep 2026, "1928 Bentley 4.5 Litre / Blower" resolver fix). Pre-war Bentleys resolve correctly to their displacement models ("4½ Litre", "4½ Litre Supercharged", "Speed Six", "8 Litre") via curated aliases in lib/vehicleData.js, and the Blower is its own model so it never mixes with an ordinary 4½ Litre (see the material-variant guard, lib/materialVariants.js). BUT the model keyword uses the ½ glyph while OCD titles these cars inconsistently ("4½-Litre", "4 1/2 Litre", "4/8-Litre"), so the exact model-rungs under-match and these ultra-rare, house-sold cars land on a make-context "Bentley sales" read at LOW confidence rather than a variant-precise band. This is ACCEPTED, not a bug: for museum-piece rarities a low-confidence pre-war-Bentley-era read is the honest floor, and the material-variant guard guarantees it never falsely mixes a Blower with a standard tourer. Do NOT re-diagnose if a future OCD title-matching audit surfaces it; revisit only if we decide variant-precise pre-war Bentley reads are worth a MODEL_FAMILY-style multi-form fetch-term set (glyph + "4 1/2" + "4.5").
