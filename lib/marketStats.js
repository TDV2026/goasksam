...
export async function computePlatformBaselines(env) {
  ...
  const rows = await supabaseSelect(env, "vehicle_market_records?select=platform,make,price,auction_status&limit=10000&order=ingested_at.desc");
  if (!rows) return null;
  const platforms = {};
  const hasMixedStatuses = rows.some(row => row.auction_status && !isSold(row.auction_status));
  for (const row of rows) {
    ...
    if (band) {
      ...
      p.byBand[band].prices.push(Number(row.price));
    }
  }
  const finish = bucket => ({
    listings: bucket.listings,
    medianPrice: median(bucket.prices),
    sellThroughPercent: hasMixedStatuses && bucket.listings >= STAT_MINIMUMS.sellThroughSample
