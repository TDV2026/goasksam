...
export async function computePlatformBaselines(env) {
  ...
  for (let page = 0; page < 12; page++) {
    const batch = await supabaseSelect(env, `vehicle_market_records?select=platform,make,price,auction_status,seller_username&limit=1000&offset=${page * 1000}&order=ingested_at.desc`);
    if (!batch) { if (!rows.length) return null; break; }
    rows.push(...batch);
    if (batch.length < 1000) break;
  }
  if (!rows.length) return null;
  // Sell-through can only be computed over a complete-coverage population:
  // search-path rows are fetched sold-only by construction, so blending them
  // in fakes a high rate. Partner listings are backfilled with ALL statuses,
  // making them the one unbiased sample. Medians still use everything sold.
  const partnerRows = await supabaseSelect(env, "partners?active=is.true&select=seller_usernames&limit=50");
  const completeCoverage = new Set((partnerRows || []).flatMap(p => p.seller_usernames || []).map(u => String(u).toLowerCase()));
  const platforms = {};
  for (const row of rows) {
    const key = String(row.platform || "unknown");
    if (!platforms[key]) platforms[key] = { listings: 0, sold: 0, covered: 0, coveredSold: 0, prices: [], byBand: {} };
    const p = platforms[key];
    const covered = completeCoverage.has(String(row.seller_username || "").toLowerCase());
    p.listings++;
    if (isSold(row.auction_status)) p.sold++;
    if (covered) { p.covered++; if (isSold(row.auction_status)) p.coveredSold++; }
    ...
    if (band) {
      if (!p.byBand[band]) p.byBand[band] = { listings: 0, sold: 0, covered: 0, coveredSold: 0, prices: [] };
      const b = p.byBand[band];
      b.listings++;
      if (isSold(row.auction_status)) b.sold++;
      if (covered) { b.covered++; if (isSold(row.auction_status)) b.coveredSold++; }
      if (isSold(row.auction_status)) b.prices.push(Number(row.price));
    }
  }
  const finish = bucket => ({
    listings: bucket.listings,
    medianPrice: median(bucket.prices),
    sellThroughPercent: bucket.covered >= STAT_MINIMUMS.sellThroughSample
      ? Math.round(bucket.coveredSold / bucket.covered * 100)
      : null,
    sellThroughSample: bucket.covered
  });
