...
// Diagnostic ONLY: raw authed GET against ${BEEHIIV_BASE}/{pub}/{path}, returns the
// status + truncated body so the right subscriber-lookup endpoint can be found. Never
// used by the product path. Callers must mask any email in the body before returning it.
export async function beehiivRawGet(path) {
  const key = process.env.BEEHIIV_API_KEY;
  const pub = process.env.BEEHIIV_PUBLICATION_ID;
  if (!key || !pub) return { ok: false, reason: "not_configured" };
  ...
  const timer = setTimeout(() => ctrl.abort(), BEEHIIV_TIMEOUT_MS);
  try {
    const res = await fetch(`${BEEHIIV_BASE}/${encodeURIComponent(pub)}/${safe}`, { headers: { Authorization: `Bearer ${key}`, Accept: "application/json" }, signal: ctrl.signal });
    clearTimeout(timer);
    const t = await res.text().catch(() => "");
    return { ok: res.ok, httpStatus: res.status, body: String(t).slice(0, 800) };
