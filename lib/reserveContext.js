...
export function computeReserveCells(rows, dataMonth, windowLabel) {
  const cells = new Map();
  for (const r of rows || []) {
  ...
  for (const c of cells.values()) {
    ...
    out.push({
      ...
      window: windowLabel || "three months",
    });
  }
  return out;
}
...
// 6-month fallback table (cascading windows, Aug 2026): the same cells computed
// over the last 6 complete months. A (platform, make, band) that cannot clear
// 10/10 at 3 months may clear at 6 months; the card then states "over the past
// six months". Regenerated alongside RESERVE_CONTEXT by refreshReserveContext.js.
export const RESERVE_CONTEXT_6MO = [];

// Request-time lookup, CASCADING to the tightest window that clears: try the
// 3-month table first (freshest), fall back to the 6-month table. The matched
// cell carries its own `window` label so the card states the true window. No cell
// in either -> null (no make-level or platform-level approximation).
export function findReserveContext(platform, make, priceUsd, table = RESERVE_CONTEXT, fallback = RESERVE_CONTEXT_6MO) {
  const band = reserveBand(priceUsd);
  if (!band) return null;
  const p = norm(platform), m = norm(make);
  const match = t => (t || []).find(c => norm(c.platform) === p && norm(c.make) === m && c.band_key === band.key) || null;
  return match(table) || match(fallback) || null;
}
