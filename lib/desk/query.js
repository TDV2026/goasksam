...
export const MEASURES = new Set([
  ...
  "record",              // the single highest sale + top five (halos/race/restomods set aside)
...
export const FILTER_KEYS = new Set(Object.keys(DIMENSIONS).concat([
  ...
  "price_min", "price_max",   // numeric USD price cap on the sale (hammer basis, the compute basis)
  "mileage_max", "mileage_min",  // explicit odometer bounds (miles); mileage_band ("low"/"high") is the era-threshold form
  "transmission"         // "manual" | "automatic" (already a DIMENSION; a filter after scope)
]));
const NUMERIC_FILTERS = new Set(["price_min", "price_max", "mileage_max", "mileage_min"]);
...
export function validateDsl(raw) {
  ...
  for (const [k, v] of Object.entries(rawFilters)) {
    ...
    if (NUMERIC_FILTERS.has(k)) {
      ...
      else ignored.push({ path: `filters.${k}`, value: v, reason: "not a positive number" });
      continue;
    }
    if (k === "mileage_band") {
      const b = String(v).toLowerCase().trim();
      if (b === "low" || b === "high") filters.mileage_band = b;
      else ignored.push({ path: "filters.mileage_band", value: v, reason: "mileage band is low or high" });
      continue;
    }
    filters[k] = v;
  }
...
export function echoChips(dsl, resolvedLabel) {
  ...
  if (f.transmission) chips.push([].concat(f.transmission).join("/"));
  if (f.price_min != null || f.price_max != null) chips.push((f.price_min != null ? "over $" + Number(f.price_min).toLocaleString("en-US") : "") + (f.price_min != null && f.price_max != null ? " " : "") + (f.price_max != null ? "under $" + Number(f.price_max).toLocaleString("en-US") : ""));
  if (f.mileage_band) chips.push(f.mileage_band === "low" ? "low mileage" : "high mileage");
  else if (f.mileage_max != null) chips.push("under " + Number(f.mileage_max).toLocaleString("en-US") + " mi");
