...
export const WINDOWS = new Set(["7d", "30d", "90d", "6mo", "12mo", "24mo", "36mo", "qtd", "ytd", "this_quarter", "last_quarter", "this_year", "last_year"]);
...
export function validateDsl(raw) {
  ...
  // A query must reference at least one car (a make/model/descriptor) OR be a
  // pure cross-market question (venue/time only). Require a car anchor for now.
  const hasCar = filters.make || filters.model || filters.descriptor || filters.generation || filters.trim;
  if (!hasCar) errors.push("no car referenced — name a make, model, or a descriptor like 'air-cooled 911'");
...
export function windowLabel(w) {
  const map = {
    ...
    "6mo": "last 6 months", "12mo": "last 12 months", "24mo": "last 24 months", "36mo": "last 36 months",
