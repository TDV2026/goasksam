...
export async function handleDeskRequest(body, ctx) {
  ...
  if (action === "interpret") {
    ...
    // Render the existing single-car answer under the card when the reading is one runnable scope OR a
    // same-model grouping (air-cooled 911s -> one union read, spec Q3), and no clarify/refuse/comparison
    // /multi-model-grouping (those are Stage C/D).
    const sameModelGrouping = reading.grouping && reading.grouping.sameModel;
    const runnable = (reading.scopes.length === 1 || sameModelGrouping) && !reading.clarify && !reading.unsupported && !reading.meta && !reading.cannotApply && !(reading.grouping && !reading.grouping.sameModel) && !(reading.structural || []).some(s => s.kind === "comparison");
