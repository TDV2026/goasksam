...
function readingToDsl(reading) {
  let s = (reading.scopes || [])[0];
  // A same-model grouping (air-cooled 911s) reads as ONE union scope over its remaining members
  // (spec Q3 single read); dropping a member narrows the union.
  if (!s && reading.grouping && reading.grouping.sameModel && reading.grouping.members && reading.grouping.members.length) {
    const m = reading.grouping.members;
    s = { make: m[0].make, model: m[0].model, yearStart: Math.min.apply(null, m.map(x => x.yearStart).filter(Boolean)), yearEnd: Math.max.apply(null, m.map(x => x.yearEnd).filter(Boolean)) };
  }
  if (!s || !s.make || !s.model) return null;
  ...
  // Numeric + band filters (previously dropped): a stated price cap, mileage band and
  // era all apply AFTER the scope. era [start,end] scopes model years unless the scope
  // already pinned a tighter trim-year range.
  if (f.price && (f.price.min || f.price.max)) { if (f.price.min) filters.price_min = f.price.min; if (f.price.max) filters.price_max = f.price.max; }
  if (f.mileage && f.mileage.band) filters.mileage_band = f.mileage.band;
  if (f.mileage && f.mileage.max) filters.mileage_max = f.mileage.max;
  if (Array.isArray(f.era) && f.era.length === 2) { if (filters.year_min == null) filters.year_min = f.era[0]; if (filters.year_max == null) filters.year_max = f.era[1]; }
...
export async function handleDeskRequest(body, ctx) {
  ...
  if (action === "interpret") {
    ...
    const resp = {
      ...
      cannot_apply: reading.cannotApply || null,
    ...
    // NO SILENT SUBSTITUTION: when a reading needs a question TYPE that is not built yet (trend,
    // side-by-side comparison, ranking across MULTIPLE models), say so in ONE line and show nothing
    // else - never fall through and render a different (count/median) answer. Built types: single read,
    // record, velocity/repeat, venue/count group-bys, same-model grouping (one union read).
    const isTrend = reading.metric && reading.metric.measure === "trend";
    const isComparison = (reading.structural || []).some(s => s.kind === "comparison");
    const isMultiGrouping = reading.grouping && !reading.grouping.sameModel;
    const isRanking = reading.scopes.length >= 2 && !isComparison;   // several cars ranked (F-body)
    if (!reading.clarify && !reading.unsupported && !reading.meta && !reading.cannotApply && (isTrend || isComparison || isMultiGrouping || isRanking)) {
      resp.not_built = isTrend
        ? "The trend view (change between two windows) isn't built yet, so I'm not showing a stand-in answer."
        : isComparison
          ? "The side-by-side comparison view isn't built yet, so I'm not showing a stand-in answer."
          : "The ranking view isn't built yet, so I'm not showing a stand-in answer.";
      return resp;
    }
    // Render the single-car answer when the reading is one runnable scope OR a same-model grouping
    // (air-cooled 911s -> one union read, spec Q3).
    const sameModelGrouping = reading.grouping && reading.grouping.sameModel;
    const runnable = (reading.scopes.length === 1 || sameModelGrouping) && !reading.clarify && !reading.unsupported && !reading.meta && !reading.cannotApply;
    if (body.run !== false && runnable) {
      const dsl = readingToDsl(reading); const v = dsl ? validateDsl(dsl) : { dsl: null };
      if (v.dsl) { try { const result = await executeDsl(v.dsl, ctx.env, { vehicle: reading.scopes[0] || (dsl && dsl.filters) }); resp.answer = shapeRun(question, v, result); } catch (e) { resp.answer_error = String(e.message || e); } }
    }
    return resp;
  }
