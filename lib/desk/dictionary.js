...
const NICKNAMES = [
  ...
  nick("Duesenberg", ["duesy", "a duesenberg", "model j", "duesenberg model j"], [m("Duesenberg", "Model J", 1928, 1937, null)], "Duesenberg Model J family (test-set Q29); the iconic Duesenberg", "approved", "'a Duesenberg' = the Model J family, the collector Duesenberg")
];
...
export const GROUPINGS = [
  {
    ...
    category: "grouping", type: "grouping", status: "approved",
  ...
  },
  {
    ...
    // chipLabel is the APPROVED, authoritative reading-card label; the interpreter's own labels are
    // overridden to these (the model card must never invent them). "930" alone means the Turbo to any
    // Porsche person, so 1974 to 1989 is "G-body" and it INCLUDES the 930 Turbo. 1989 is deliberately
    // in BOTH G-body and 964 (the ambiguous handover year, per Sam).
    members: [
      m("Porsche", "911", 1964, 1973, "901", { chipLabel: "901 (1964 to 1973)" }),
      m("Porsche", "911", 1974, 1989, null, { label: "G-body", chipLabel: "G-body (1974 to 1989, incl. 930 Turbo)" }),
      m("Porsche", "911", 1989, 1994, "964", { chipLabel: "964 (1989 to 1994)" }),
      m("Porsche", "911", 1995, 1998, "993", { chipLabel: "993 (1995 to 1998)" })
    ],
...
// Compact reference block for the interpreter prompt: the APPROVED phrases the model may use, grouped
// by category, with what each expands to. Groupings list their members so the model returns the full
// list. Kept terse to hold the reading step under ~2s (spec s3). The model is told it may name ONLY
// makes/models/generations/groupings that appear here (or resolve through the resolver, checked by the
// validator); anything else goes to "unresolved".
export function dictionaryPromptBlock() {
  const lines = [];
  const scopeStr = e => (e.models || []).filter(m => !m.makeOnly).map(m => `${m.make} ${m.model}${m.generation ? "(" + m.generation + ")" : ""} ${m.yearStart}-${m.yearEnd}`).join("; ")
    || (e.models || []).map(m => `${m.make} (make only)`).join("; ");
  const byCat = {};
  for (const e of ENTRIES) { if (e.status !== "approved") continue; (byCat[e.category] = byCat[e.category] || []).push(e); }
  const dump = (label, cat, fmt) => { lines.push(label); for (const e of (byCat[cat] || [])) lines.push("  " + e.phrase + " -> " + fmt(e)); };
  dump("PLATFORM/CHASSIS CODES:", "platform_chassis", scopeStr);
  dump("NICKNAMES:", "nickname", scopeStr);
  dump("ERAS (model-year spans):", "era", e => `${e.yearRange[0]}-${e.yearRange[1]}`);
  dump("WINDOWS (sale-date):", "window", e => e.window);
  dump("CHANNELS:", "channel", e => e.channel);
  dump("VENUES:", "venue", e => e.venue === "__event__" ? "event " + (e.event && e.event.label) : e.venue);
  dump("PRICE PHRASES:", "price", e => JSON.stringify(e.price));
  dump("MILEAGE PHRASES:", "mileage", e => e.mileage.band + "-mile (era threshold)");
  dump("METRICS:", "metric", e => e.metric.measure || "ASK(" + (e.metric.ask || []).join("|") + ")");
  dump("BODY:", "body", e => e.body);
  dump("TRANSMISSION:", "transmission", e => e.transmission);
  dump("STRUCTURAL:", "structural", e => JSON.stringify(e.structural));
  lines.push("GROUPINGS (return the full member list):");
  for (const g of GROUPINGS) { if (g.status !== "approved") continue; lines.push("  " + g.name + " [" + g.members.map(m => `${m.make} ${m.model} ${m.yearStart}-${m.yearEnd}`).join(", ") + "]"); }
  return lines.join("\n");
}
