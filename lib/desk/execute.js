...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  // Data-error floor: a handful of rows carry real car titles but junk prices (an RM
  // "1985 911 Carrera Targa" recorded at $20). No collector car sells at a major venue
  // under this floor, so these are unambiguously bad data, never a real cheap sale. This
  // is pool hygiene (not a displayed statistic); the count removed is surfaced in coverage.
  const DESK_PRICE_FLOOR = 2500;
  const beforeFloor = pool.length;
  pool = pool.filter(r => Number(r.value) >= DESK_PRICE_FLOOR);
  const droppedLowPrice = beforeFloor - pool.length;
  ...
  const coverage = {
    ...
    dropped_low_price: droppedLowPrice,
    window: win.label,
