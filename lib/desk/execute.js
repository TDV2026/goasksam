...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  try {
    ...
    // RECORD FALLBACK: an all-time record scan can still exceed the Supabase statement timeout on a
    // high-volume model (M3) even with the sale_price index, because make/model ILIKE prevent a
    // clean index scan. Retry ONCE over a bounded 6-year window (the date btree bounds the set) so
    // the record ALWAYS returns; the definition then states the window actually scanned.
    if (isRecord && sinceIso <= "2001-01-01") {
      sinceIso = isoDay(Date.now() - 2192 * DAY); win.fromIso = sinceIso; win.label = "the last 6 years (an all-time scan timed out)";
