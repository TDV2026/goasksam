...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  if (isRecord) {
    ...
    record = {
      ...
      // highest SINCE coverage began. Earlier sales (e.g. a 1988 M3 that sold on BaT in 2020, before
      // our BaT history) are not in our data. State the earliest coverage date and list per-source.
      definition: `Record means the single highest standard sale in our data since ${covShort || "our coverage began"}, not all-time: sales before our coverage started are not included. Halos, race cars, restomods and period tuner cars are set aside unless the question asks for them.`,
