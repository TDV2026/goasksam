...
import { sourceCoverage, coverageForSources } from "./coverage.js";
...
const DOW_ORDER = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];   // table order matches the chart
function sortRowsByDim(rows, primary) {
  if (primary === "day_of_week") { rows.sort((a, b) => DOW_ORDER.indexOf(a.group) - DOW_ORDER.indexOf(b.group)); return; }
  if (["month", "year", "quarter", "model_year", "price_band"].includes(primary)) { rows.sort((a, b) => String(a.group).localeCompare(String(b.group))); return; }
...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  // (e) A RECORD question wants the single highest sale in ALL of our data. The price-sorted scan
  // (spec.recordSort) is index-backed now (docs/supabase-sale-price-index.sql applied Sep 2026), so
  // an all-time scan of even a high-volume model (M3) completes under the statement timeout. Scan
  // all-time when no explicit window/date was requested.
  ...
  if (isRecord && !filters.sale_from && !filters.sale_to && (!filters.window || filters.window === "36mo")) {
    win.fromIso = "2000-01-01"; win.label = "our full data (since coverage began)";
  }
  ...
  // the code's generation window; the trim (Turbo) then scopes the title correctly. GUARD: only remap
  // an ACTUAL 911 generation code (930/964/993/996/997/991/992/901) - a 3-digit REAL model like 944,
  // 924, 928, 356 must NOT be remapped to 911 (that scoped a "944 Turbo" record onto 911 Turbos).
  let forcedGens = null;
  if (/porsche/i.test(vehicle.make) && /^\d{3}$/.test(String(vehicle.model || ""))) {
    ...
    if (g) { vehicle = { ...vehicle, model: "911" }; forcedGens = [g]; }
  }
  ...
  const fetchQ = async (spec, maxAttempts = 3) => {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      diag.queryError = false;
      const t0 = Date.now();
      const rows = (await fetchQualifying(spec, sinceIso, env, diag)) || [];
      if (!diag.queryError) return rows;
      // Smart retry: a FAST failure (~8s) is a transient hiccup - retry it cheaply. A SLOW failure
      // (>15s) is a real statement timeout - do NOT burn more ~28s attempts (that blows the 60s
      // function limit); fail now so the record's bounded-window fallback can run instead.
      if (Date.now() - t0 > 15000 || attempt >= maxAttempts - 1) break;
      await new Promise(r => setTimeout(r, 400 * (attempt + 1)));
    }
    const e = new Error("archive query failed after retries"); e.deskQueryError = true; throw e;
  };
  // With make=eq the index-served record query is fast (~8-16s) and its transient failures return
  // FAST (~8s), so normal retries absorb them cheaply; a bounded-window fallback still catches the
  // rare slow all-time timeout. 3 attempts + fallback fits well under the 60s function limit.
  const attemptsFor = 3;
  let recordFallbackMode = false;   // set true by the catch to run the bounded (date-filtered) fallback
  const fillPool = async () => {
    for (const gen of genList) {
      ...
      // high-volume model whose record predates the recent 1000. house-tier too: the record is about
      // the top SALE (a named house receipt is valid WITHOUT a photo, RULE 1) - otherwise the photo
      // gate drops the top standard house sale (the $270k RM 1997 M3 Evolution) and the record is wrong.
      if (isRecord) { spec.recordSort = true; spec.houseTier = true; if (recordFallbackMode) spec.recordFallback = true; }
      let rows = await fetchQ(spec, attemptsFor);
      ...
      if (rows.length < 8 && codeTok && codeTok.toLowerCase() !== String(spec.model || "").toLowerCase()) {
        const more = await fetchQ({ ...spec, model: codeTok }, attemptsFor);
  ...
  try {
    ...
    if (!(e && e.deskQueryError)) throw e;
    // RECORD FALLBACK: if the all-time index scan still times out under DB load, retry ONCE over a
    // bounded 24-month window WITH the date filter (recordFallbackMode) - the date btree genuinely
    // shrinks the set, a cheap query - so the record ALWAYS returns; the definition states the window
    // actually scanned.
    if (isRecord && sinceIso <= "2001-01-01") {
      recordFallbackMode = true;
      sinceIso = isoDay(Date.now() - 730 * DAY); win.fromIso = sinceIso; win.label = "the last 24 months (an all-time scan timed out)";
      pool = []; seen.clear(); genLabels.length = 0; diag.excluded = [];
      try { await fillPool(); }
      catch (e2) { if (e2 && e2.deskQueryError) return { ok: false, reason: "query_error", window: win, coverage: null }; throw e2; }
    } else {
      return { ok: false, reason: "query_error", window: win, coverage: null };
    }
  }
  ...
  // Numeric + observable filters, applied AFTER scope (spec s5). Every applied filter is
  // surfaced as a chip; a filter the archive cannot support is flagged not-applied.
  // Price cap on the compute basis (hammer USD, r.value), so it matches the medians shown.
  if (filters.price_min != null) pool = pool.filter(r => Number(r.value) >= Number(filters.price_min));
  if (filters.price_max != null) pool = pool.filter(r => Number(r.value) <= Number(filters.price_max));
  // Mileage: an explicit odometer cap, or an era-threshold band (spec s6): under 10k for
  // 2010+, under 30k for 1990-2009, under 50k before 1990 (and the inverse for "high").
  // A row with no recorded mileage cannot satisfy a mileage filter, so it is excluded from
  // the filtered pool (the coverage line notes the mileage-unknown count).
  const milesOf = (r) => { const mi = r.mi || mileageInfo(r); const v = mi && (mi.structured != null ? mi.structured : mi.stated); return Number.isFinite(v) ? v : null; };
  const eraMileThreshold = (year) => (year >= 2010 ? 10000 : year >= 1990 ? 30000 : 50000);
  let mileageUnknown = 0;
  if (filters.mileage_band === "low" || filters.mileage_band === "high" || filters.mileage_max != null || filters.mileage_min != null) {
    pool = pool.filter(r => {
      const m = milesOf(r); if (m == null) { mileageUnknown++; return false; }
      if (filters.mileage_max != null && m > Number(filters.mileage_max)) return false;
      if (filters.mileage_min != null && m < Number(filters.mileage_min)) return false;
      if (filters.mileage_band) { const th = eraMileThreshold(Number(r.year) || 0); return filters.mileage_band === "low" ? m < th : m >= th; }
      return true;
    });
  }
  // Transmission: classify each row (parity with groupKey) and keep the wanted class.
  if (filters.transmission) {
    const want = [].concat(filters.transmission).map(s => String(s).toLowerCase());
    const txOf = (r) => { const t = String(r.transmission || "").toLowerCase(); return /man|stick|\bmt\b/.test(t) ? "manual" : /auto|pdk|dct|tiptronic|\bat\b/.test(t) ? "automatic" : "other"; };
    pool = pool.filter(r => want.includes(txOf(r)));
  }
  // (#2) SET-ASIDE applies to EVERY answer, not only record questions: halos, race cars, restomods
  // and period tuners are removed from the STATS pool (median, quartiles, min, max, drill-downs and
  // receipts all read the same eligible pool) and listed separately with a reason. Skipped when the
  // query NAMES the halo/generation/trim (wantHalo) - then the special IS the subject.
  // wantHalo (keep specials in) is true ONLY when the query's TRIM names a special itself (CSL,
  // Sport Evolution, GT3, ...) - then the special IS the subject. A GENERATION scope (e36) or a
  // non-special trim is NOT a halo subject, so set-aside still applies there (#2: medians, quartiles,
  // min, max, drill-downs and receipts all read the eligible pool within a generation too).
  const wantHalo = !!(trimTok && recordExcludeReason(trimTok, vehicle.make, { wantHalo: false }));
  const setAsideRows = [];
  {
    // ALWAYS split. When the query names a halo trim (wantHalo), that halo stays IN, but race cars,
    // restomods and period tuners are STILL set aside (a "911 GT3" query keeps street GT3s but sets
    // aside GT3 Cup race cars). When no halo is named, halos are set aside too.
    const eligiblePool = [];
    for (const r of pool) {
      const why = recordExcludeReason(r, vehicle.make, { wantHalo });
      if (why) { setAsideRows.push({ row: r, reason: why }); excluded.push({ row: r, reason: "set aside: " + why }); }
      else eligiblePool.push(r);
    }
    pool = eligiblePool;
  }
  ...
  // Distribution strip gets its OWN, larger sample (up to 500 priced sales) so the chart shows more
  // than the 300-row table receipts. Evenly strided across the price-bearing pool (not date-recent)
  // so the spread is representative; stripSampled states "N of M" whenever it is a subset.
  const STRIP_CAP = 500;
  const pricedPool = pool.filter(r => Number.isFinite(Number(r.value)) && Number(r.value) > 0);
  const stripRows = pricedPool.length <= STRIP_CAP ? pricedPool
    : Array.from({ length: STRIP_CAP }, (_, i) => pricedPool[Math.floor(i * pricedPool.length / STRIP_CAP)]);
  const stripReceipts = stripRows.map(r => toReceipt(r, null));
  const stripSampled = pricedPool.length > stripReceipts.length ? { shown: stripReceipts.length, of: pricedPool.length } : null;
  ...
  // Filter honesty (spec s5): note the numeric/observable filters in force, and how many
  // rows a mileage filter had to drop for want of a recorded odometer (coverage honesty).
  const appliedFilters = {};
  if (filters.price_min != null || filters.price_max != null) appliedFilters.price = { min: filters.price_min || null, max: filters.price_max || null };
  if (filters.mileage_band || filters.mileage_max != null || filters.mileage_min != null) { appliedFilters.mileage = { band: filters.mileage_band || null, max: filters.mileage_max || null, min: filters.mileage_min || null }; if (mileageUnknown) appliedFilters.mileage.excluded_no_odometer = mileageUnknown; }
  if (filters.transmission) appliedFilters.transmission = [].concat(filters.transmission).join("/");
  if (Object.keys(appliedFilters).length) coverage.filters = appliedFilters;
  if (outcomeIsNonSold || rateMeasures.length) {
  ...
  let record = null;
  if (isRecord) {
    // pool is ALREADY the eligible pool (the #2 set-aside ran above); setAsideRows holds the specials.
    const eligible = pool.filter(r => Number.isFinite(Number(r.value)) && Number(r.value) > 0)
      .slice().sort((a, b) => Number(b.value) - Number(a.value));
    const setAside = setAsideRows.filter(x => Number.isFinite(Number(x.row.value)) && Number(x.row.value) > 0)
      .slice().sort((a, b) => Number(b.row.value) - Number(a.row.value));
    // Coverage dates come from the FIXED per-source table (true earliest sale_date per source in the
    // whole archive), NEVER from this query's pool. A pool-derived date is fake (a 24-month fallback
    // window's earliest sale is not "coverage"). The all-time path states "since <true earliest>" and
    // lists each contributing source's true start; the FALLBACK path says "last 24 months" with NO
    // coverage-start wording and NO per-source dates, so it never overclaims.
    const isFallback = win.fromIso > "2001-01-01";
    let covStart = null, covSources = [];
    if (!isFallback) {
      // Only the sources PRESENT in this record's pool (~5-8), not the full 22-source table, so a cold
      // instance pays ~1s not ~3s (keeps the record under the 3s bar). Seeds the shared cache.
      const present = [...new Set((coverage.sources || []).map(s => s.source))];
      const table = await coverageForSources(env, present).catch(() => ({}));
      covSources = present.map(src => ({ source: src, since: table[src] || null })).filter(x => x.since).sort((a, b) => a.since.localeCompare(b.since));
      covStart = covSources.length ? covSources[0].since : null;
    }
    record = {
      definition: isFallback
        ? `Record here is the highest standard sale over the last 24 months: the all-time scan timed out, so this is a recent-window record, not our full coverage. Halos, race cars, restomods and period tuner cars are set aside unless the question asks for them.`
        : `Record means the single highest standard sale in our data since ${covStart || "our earliest coverage"}, not all-time: sales before our coverage began are not included. Halos, race cars, restomods and period tuner cars are set aside unless the question asks for them.`,
      window_kind: isFallback ? "fallback_24mo" : "all_time",
      coverage_start: covStart,
      coverage_sources: covSources,
      top: eligible.slice(0, 5).map(r => toReceipt(r, null)),
      set_aside: setAside.slice(0, 8).map(x => ({ ...toReceipt(x.row, null), record_excluded_as: x.reason })),
...
async function executeMarket(dsl, env, opts, win, filters) {
  ...
  // Larger (up to 500) strided sample for the distribution strip.
  const STRIP_CAP = 500;
  const pricedPool = pool.filter(r => Number.isFinite(Number(r.value)) && Number(r.value) > 0);
  const stripRows = pricedPool.length <= STRIP_CAP ? pricedPool
    : Array.from({ length: STRIP_CAP }, (_, i) => pricedPool[Math.floor(i * pricedPool.length / STRIP_CAP)]);
  ...
  return {
    ...
    velocity: null, answer: rows, receipts, stripReceipts, stripSampled, coverage, thin: total < THIN_MIN
  };
}
