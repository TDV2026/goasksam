...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  // (e) A RECORD question means "the highest in our data since coverage began", so it scans the
  // FULL archive, not the default 36-month window. Only when no explicit window/date was requested.
  ...
  if (isRecord && !filters.sale_from && !filters.sale_to && (!filters.window || filters.window === "36mo")) {
    win.fromIso = "2000-01-01"; win.label = "our full data (record, since coverage began)";
  }
  ...
  if (isRecord) {
    ...
    record = {
      definition: `Record means the single highest sale in our data since coverage began${covStart ? ` (${covStart})` : ""}. Halos, race cars and restomods are set aside unless the question asks for them.`,
