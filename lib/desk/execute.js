...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  // 1. resolve the car
  const text = filters.descriptor || [filters.make, filters.model, filters.trim].filter(Boolean).join(" ");
  let vehicle = opts.vehicle ? sanitizeResolvedVehicle(opts.vehicle) : null;
  let resolution = null;
  if (!vehicle) {
    resolution = await resolveVehicle(text, { env });
    if (resolution.status !== "valid" || !resolution.vehicle || !resolution.vehicle.make) {
