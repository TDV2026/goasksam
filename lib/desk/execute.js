...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  // A RECORD does its own bounded-window fallback on failure, so its fetches must fail FAST (1 try),
  // not burn 3x ~28s retries on a doomed all-time scan and blow the 60s function limit before the
  // fallback runs.
  const attemptsFor = isRecord ? 1 : 3;
