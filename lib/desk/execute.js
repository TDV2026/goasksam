...
function expandGenerations(vehicle, filters) {
  ...
  // No year and no descriptor: if the nameplate HAS curated generations, union ALL of them (each
  // year-bounded). This covers the whole model history AND avoids a badged family (M3, M4) whose
...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  for (const gen of genList) {
    ...
    let rows = (await fetchQualifying(spec, sinceIso, env, diag)) || [];
    ...
    if (rows.length < 8 && codeTok && codeTok.toLowerCase() !== String(spec.model || "").toLowerCase()) {
      const more = (await fetchQualifying({ ...spec, model: codeTok }, sinceIso, env, diag)) || [];
...
async function executeMarket(dsl, env, opts, win, filters) {
  ...
  for (const key of allKeys) {
    ...
    const row = {
      ...
      median: measures.includes("median") ? median(vals) : undefined,
      p25: measures.includes("p25") ? percentile(vals, 0.25) : undefined,
      p75: measures.includes("p75") ? percentile(vals, 0.75) : undefined,
      min: measures.includes("min") ? (vals[0] ?? null) : undefined,
      max: measures.includes("max") ? (vals[vals.length - 1] ?? null) : undefined,
  ...
  const bySource = new Map();
  for (const r of pool) { const s = r.source || sourceSlugOf(r); const d = r.auction_end_date || r.date; if (!bySource.has(s) || (d && d < bySource.get(s))) bySource.set(s, d); }
  const receipts = pool.map(r => toReceipt(r, null)).sort((a, b) => (b.date || "").localeCompare(a.date || ""));
