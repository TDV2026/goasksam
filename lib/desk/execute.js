...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  if (isRecord) {
    ...
    const covShort = covStart ? String(covStart).slice(0, 10) : null;
    // Never "all-time": our data starts at each source's coverage date, so a record is only the
    // highest SINCE our coverage. Earlier sales (e.g. a 1988 M3 that sold on BaT in 2020, before our
    // BaT history) are not in our data. When the all-time scan timed out and we fell back to a bounded
    // window, say THAT (it is a recent-window record, not our full coverage), so we never overclaim.
    const isFallback = win.fromIso > "2001-01-01";
    record = {
      definition: isFallback
        ? `Record here is the highest standard sale over the last 24 months (since ${covShort || "recently"}): the all-time scan timed out, so this is a recent-window record, not our full coverage. Halos, race cars, restomods and period tuner cars are set aside unless the question asks for them.`
        : `Record means the single highest standard sale in our data since ${covShort || "our coverage began"}, not all-time: sales before our coverage started are not included. Halos, race cars, restomods and period tuner cars are set aside unless the question asks for them.`,
      coverage_start: covShort,
      coverage_sources: (coverage.sources || []).map(s => ({ source: s.source, since: s.earliest ? String(s.earliest).slice(0, 10) : null })),
