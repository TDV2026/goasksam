...
export async function executeDsl(dsl, env, opts = {}) {
  ...
  const exReceipts = excluded.map(e => toReceipt({ ...e.row, _saleType: classifySaleType(e.row) }, e.reason));
  ...
  const exReasons = {};
  for (const e of excluded) { const k = /data-error floor/.test(e.reason) ? "house data-error floor" : e.reason; exReasons[k] = (exReasons[k] || 0) + 1; }
  const coverage = {
    ...
    floored_house_rows: flooredCount,
    window: win.label,
