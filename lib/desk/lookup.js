...
// Find a normalized phrase as a whole-token run in text; return {start,end} char offsets or null.
function findPhrase(text, phrase, consumed) {
  if (!phrase) return null;
  const re = new RegExp("(^|[^a-z0-9])" + phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/\s+/g, "\\s+") + "([^a-z0-9]|$)", "g");
  let m;
...
export async function lookupQuestion(question, opts = {}) {
  ...
  for (const s of scopeEntries) {
    ...
    for (const mm of models) reading.scopes.push({ ...mm, source: s.phrase });
  }
  ...
  const isUnsupported = /(worth|will .* be worth|next year|should i (sell|buy)|is it a good (buy|time))/.test(text);
  ...
  // ---- 9) leftover words as fates (no silent drops) ----
  // Any still-unconsumed meaningful word not captured by residual (offline) is flagged.
