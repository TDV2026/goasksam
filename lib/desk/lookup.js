...
export async function lookupQuestion(question, opts = {}) {
  ...
  if (groupingEntries.length) {
    ...
    // Same-model grouping (air-cooled 911s): keep the GENERATIONS as removable members and mark
    // sameModel so it reads as a SINGLE read (not a ranking, spec Q3) but a follow-up can drop one.
    reading.grouping = { name: g.name, members, definedBy: g.definedBy, status: g.status, sameModel: distinct.size === 1 };
  }
