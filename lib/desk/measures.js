...
export async function fetchBroadSales(win, env, filters = {}) {
  const cols = "id,price:sale_price,auction_end_date:sale_date,source:platform,raw_title:listing_title,year,vin_norm," +
    "mileage:raw_record->>mileage,transmission:raw_record->>transmission,currency:raw_record->>currency," +
    "srcurl:raw_record->>url,srcurl2:raw_record->>source_url,city:raw_record->>city,precision:raw_record->>auction_end_precision,image:raw_record->>featured_image_url";
  ...
  if (filters.venue) { const v = String([].concat(filters.venue)[0]).toLowerCase(); const label = SLUG_LABEL[v] || v; base += `&or=(source_slug.eq.${encodeURIComponent(v)},platform.eq.${encodeURIComponent(label)})`; }
