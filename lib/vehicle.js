...
async function loadTaxonomy(options = {}) {
  ...
  if (!makes) {
    // Union both free (unmetered) make sources rather than falling back from
    // one to the other: OldCarsData omits newer marques (Genesis is vPIC-only)
    // while vPIC omits some the archive has, and a make the seller types must
    // resolve regardless of which source lists it. OldCarsData is concatenated
    // first so its display casing wins on the overlap.
    let ocdMakes = [];
    let vpicMakes = [];
    try {
      const json = await fetchJson(`${OLDCARSDATA_BASE}/makes`);
      ocdMakes = json.data || [];
    } catch { /* fall through to vPIC / curated */ }
    try {
      vpicMakes = await getVpicMakes();
    } catch { /* fall through to curated */ }
    makes = uniqueValues([...ocdMakes, ...vpicMakes]);
    if (!makes.length) {
...
async function getModelsForMake(taxonomy, make) {
  ...
  if (!models) {
    try {
      ...
      models = dropNonModels((json.data || []).map(cleanModelName)).map(name => ({ name, yearRanges: null }));
    } catch {
      models = [];
    }
  }
...
async function getVpicModelsForMake(make) {
  ...
  try {
    ...
    const json = await fetchJson(url);
    const results = Array.isArray(json.Results) ? json.Results : [];
    models = dropNonModels([...new Set(results.map(item => item.Model_Name).filter(Boolean))]);
  } catch {
    models = [];
  }
...
export async function resolveVehicle(rawInput, options = {}) {
  ...
  if (!model) {
    ...
    const catalogExact = matchExactModel(remainder, catalogModels);
    if (catalogExact) {
      model = catalogExact;
  ...
  if (!model) {
    ...
    if (designation) {
      ...
      modelSource = "seller_designation";
    }
  }
