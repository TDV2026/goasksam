...
export async function resolveVehicle(rawInput, options = {}) {
  ...
  const partialVehicle = extra => ({
    raw: text, year, make: null, model: null, trim: null, color, mileage, bodyStyle, confidence: "low", ...extra
  });
  ...
  if (model) {
    ...
    // Year coverage from our own records: an out-of-range year gets a
    // challenge, never silent acceptance. Unknown coverage accepts as before.
    if (!rule?.ranges && taxonomy.env) {
