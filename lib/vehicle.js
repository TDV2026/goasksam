...
const NOISE_WORDS = /\b(i have|i've got|my car is|to sell|sell|selling|have a|with|miles|mile|mi|km)\b/gi;
...
function removeKnownNoise(raw, make, year, aliasTexts = []) {
  ...
  for (const aliasText of aliasTexts) {
    value = value.replace(new RegExp(`(^|[^a-z0-9])${escapeRegExp(aliasText)}([^a-z0-9]|$)`, "ig"), "$1$2");
  }
  return value
...
function fuzzyModelCandidate(text, modelNames) {
  const tokens = normalize(text).split(/\s+/).filter(token => token.length >= 4 && !/^\d+$/.test(token));
  let best = null;
...
function extractTrim(remainder, matchedModel, aliasTrim) {
  ...
  return formatTrim(leftover);
}

function modelSuggestionChips(make, year, yearModels, archiveModels) {
  ...
  return (archiveModels || []).slice(0, 5).concat("Not sure");
}
...
export async function resolveVehicle(rawInput, options = {}) {
  ...
  const { year, token: yearToken } = extractYearInfo(text);
  ...
  const partialVehicle = extra => ({
    raw: text, year, make: null, model: null, trim: null, color, mileage, confidence: "low", ...extra
  });
  ...
  if (!make) {
    ...
    if (makeAlias && makeAlias.kind !== "misspelling") {
      ...
      const remainder = removeKnownNoise(text, null, year, [makeAlias.alias]);
      const suggestion = [year, makeAlias.make, remainder].filter(Boolean).join(" ").replace(/\s+/g, " ").trim();
  ...
  if (!make) {
    ...
    if (fuzzy && year) {
      const remainder = removeKnownNoise(text, null, year, [fuzzy.token]);
      const suggestion = [year, fuzzy.make, remainder].filter(Boolean).join(" ").replace(/\s+/g, " ").trim();
      return confirmationResult(
        partialVehicle({}),
  ...
  if (!year || !make) {
    ...
    const understoodModel = aliasModel || inferredModelHint || null;
    ...
    return baseResult("needs_clarification", partialVehicle({ make, model: understoodModel }), {
      question,
      missing: [!year ? "year" : null, !make ? "make" : null, !understoodModel ? "model" : null].filter(Boolean),
    ...
    if (yearModels.length && !matchExactModel(model, yearModels) && !rule?.ranges) {
      ...
      if (knownToVpic && modelSource !== "alias") {
        return baseResult("invalid_vehicle", partialVehicle({ make, model }), {
          ...
          chips: modelSuggestionChips(make, year, yearModels, archiveModelNames),
          suggestion: null,
          baseVehicle: [year, make].join(" ")
        }, corrections);
      }
  ...
  if (!model) {
    return baseResult("needs_clarification", partialVehicle({ make, confidence: makeConfidence }), {
      question: `${make} made a lot of different cars in ${year}. Which model are we talking about? Pick one below, or type the exact model if it is not shown.`,
      chips: modelSuggestionChips(make, year, yearModels, archiveModelNames),
      baseVehicle: [year, make].join(" ")
    }, corrections);
  }
  ...
  const trim = extractTrim(remainder, modelSource === "alias" ? modelAlias?.alias : model, aliasTrim);
  ...
  const vehicle = {
    ...
    canonicalLabel: [year, make, model, trim].filter(Boolean).join(" ")
  };
