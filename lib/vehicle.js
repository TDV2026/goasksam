...
import { MARKET_SPEC_WORDS, marketSpecEntryFor, detectSearchSpec, classifyRecordSpec } from "./marketSpec.js";
...
// vPIC/OCD sometimes file a body-style ENUMERATION as a model name (Crosley's only
// "model" is literally "Sedan, Convertible, & Wagon"). These are not real nameplates: a
// single-model make with only this resolves to a garbage label. Drop a name whose tokens
// are ALL body-style words when there are 2+ of them (so a real single-word nameplate like
// "Spider" or "Van" is never dropped). Aug 2026 (Crosley/TR7 mangling fix).
const BODY_STYLE_ENUM_WORDS = new Set(["sedan", "saloon", "convertible", "cabriolet", "cabrio", "coupe", "coup", "wagon", "estate", "hatchback", "liftback", "fastback", "hardtop", "softtop", "and", "or"]);
function isBodyStyleEnumModel(name) {
  const toks = String(name || "").toLowerCase().replace(/[^a-z]+/g, " ").split(/\s+/).filter(Boolean);
  return toks.length >= 2 && toks.every(t => BODY_STYLE_ENUM_WORDS.has(t));
}
// OCD/vPIC sometimes file SEVERAL real nameplates under one comma/"and"-joined model
// string ("DB2, DB2/4, and DB Mark III" for Aston Martin; "GT, GTB, and GTC" etc.).
// As a single catalog entry it means a real model the seller typed (DB2) never
// exact-matches, so the resolver falls to the fuzzy/designation matcher and "corrects"
// it to a sibling (DB2 -> DB9). Split these into their component models so each real
// nameplate is recognized on its own. Guarded: only splits on a genuine comma/&/" and "
// enumeration yielding 2+ parts, so a normal single name is never shredded.
function splitEnumModelName(name) {
  const raw = String(name || "").trim();
  if (!/,|&|\band\b/i.test(raw)) return [raw];
  const parts = raw.split(/\s*,\s*|\s*&\s*|\s+and\s+/i)
    .map(s => s.trim())
    .filter(part => part && !/^(and|&|the)$/i.test(part));
  return parts.length >= 2 ? parts : [raw];
}
const dropNonModels = names => (names || []).filter(name => !NON_MODEL_TOKENS.has(normalize(name)) && !isBodyStyleEnumModel(name));
// A single body-style word ("convertible", "coupe", "wagon") is never a make/model
// signal on its own. Excluded from make-inference candidate tokens so a body word
// can never independently drive a make guess (the "1988 325i Convertible" -> Crosley
// garble: "convertible" uniquely term-matched Crosley's junk enum model). Uses the
// same enum-word vocabulary; the "and"/"or" members are harmless here (also stopwords).
const isBodyStyleWord = token => BODY_STYLE_ENUM_WORDS.has(normalize(token));
...
// "sportbrake" (Jaguar's estate wagon) is a real body variant a buyer names; without it the
// resolver drops the word and an "XF Sportbrake" reads as a plain "XF". Kept verbatim for the label.
const BODY_STYLE_WORDS = ["vert", "convertible", "cabriolet", "cabrio", "coupe", "hardtop", "softtop", "soft top", "t top", "targa top", "sportbrake"];
// Abbreviated/informal body-style words normalize to their canonical style, so the
// stored bodyStyle field AND the label read cleanly: "1990 Ford Mustang Vert" would
// otherwise print "Vert" verbatim. The comp-fetch keyword also stabilizes on the
// canonical term. Anything not listed keeps its own word.
const BODY_STYLE_CANONICAL = { vert: "convertible", cabrio: "cabriolet", softtop: "convertible", "soft top": "convertible" };
function extractBodyStyle(text) {
  const lower = normalize(text);
  const word = BODY_STYLE_WORDS.find(w => new RegExp(`(^|\\s)${w.replace(/\s/g, "\\s")}(\\s|$)`).test(lower)) || null;
  return word ? (BODY_STYLE_CANONICAL[word] || word) : null;
}
  ...
  if (taxonomy.fromDb) {
    ...
    if (rows?.length) {
      ...
      models = rows
        ...
        .flatMap(row => splitEnumModelName(row.name).map(name => ({ name, yearRanges: row.year_ranges || null })));
    }
  }
  if (!models) {
    try {
      const json = await fetchJson(`${OLDCARSDATA_BASE}/models?make=${encodeURIComponent(make)}`);
      models = dropNonModels((json.data || []).map(cleanModelName).map(n => stripMakePrefix(n, make)))
        .flatMap(name => splitEnumModelName(name))
        .map(name => ({ name, yearRanges: null }));
    } catch {
      models = [];
    }
  }
  // Dedupe by normalized name (splitting can surface a component already listed).
  { const seen = new Set(); models = models.filter(m => { const k = normalize(m.name); if (!m.name || seen.has(k)) return false; seen.add(k); return true; }); }
...
async function inferMakeFromModel(taxonomy, text) {
  ...
  const tokens = normalize(text).split(/\s+/)
    .filter(token => token.length >= 3 && !/^\d{1,2}$/.test(token) && !TRIM_STOPWORDS.has(token) && !AMBIGUOUS_ENGLISH_MODELS.has(token) && !MARKET_SPEC_WORDS.has(token) && !isBodyStyleWord(token))
  ...
  for (const token of tokens.slice(0, 4)) {
    ...
    // A junk body-style-enumeration model name ("Sedan, Convertible, & Wagon") must never
    // drive a make inference, even if a body word slipped a token match. Filter them out
    // BEFORE counting owners, so a make whose only match is such an entry can't be inferred.
    const makeSlugs = new Set(rows.filter(row => textHasTerm(row.name, token) && !isBodyStyleEnumModel(row.name)).map(row => row.make_slug));
...
// Common English words that appear in off-script sentences ("after i give you this
// what WILL happen") and must never fuzzy-match a make. "will" is a legit 4-char prefix
// of "Willys", so the prefix-truncation boost below would otherwise surface it. These
// words are never makes; skipping them stops a stray sentence word from becoming a
// "Did you mean the Willys?" misfire.
const FUZZY_MAKE_STOPWORDS = new Set(["will", "what", "this", "that", "with", "your", "from", "have", "give", "gives", "take", "want", "need", "tell", "know", "when", "where", "does", "done", "than", "then", "after", "before", "would", "could", "should", "thing", "happen", "going", "been", "were", "they", "them", "here", "there", "like", "just", "some", "into", "over", "back", "down", "more", "most", "very", "much", "also", "only", "even", "well", "good", "help", "sell", "sold", "buy", "worth", "price", "value", "best", "which", "thanks", "thank", "hello", "please", "sorry", "about", "answer", "question", "happens", "next"]);
function fuzzyMakeCandidate(text, makes, consumedTokens) {
  const tokens = normalize(text).split(/\s+/).filter(token => token.length >= 4 && !consumedTokens.has(token) && !/^\d+$/.test(token) && !FUZZY_MAKE_STOPWORDS.has(token));
  let best = null;
...
// VIN model-year, decoded from the VIN itself with the position-7 cycle disambiguation.
// The position-10 year code repeats every 30 years (L = 1990 OR 2020); position 7 breaks the
// tie: a DIGIT means the 1980-2009 cycle, a LETTER means 2010+. vPIC's DecodeVinValues
// sometimes returns the WRONG cycle (e.g. WBSAK0301LAE33492, a 1990 E30 M3, decodes to
// "2020"), so we compute the year ourselves and prefer it over vPIC's ModelYear whenever the
// VIN is a decodable 17-char. Falls back to vPIC only when we can't compute it.
const VIN_YEAR_CODE = { A: 1980, B: 1981, C: 1982, D: 1983, E: 1984, F: 1985, G: 1986, H: 1987, J: 1988, K: 1989, L: 1990, M: 1991, N: 1992, P: 1993, R: 1994, S: 1995, T: 1996, V: 1997, W: 1998, X: 1999, Y: 2000, "1": 2001, "2": 2002, "3": 2003, "4": 2004, "5": 2005, "6": 2006, "7": 2007, "8": 2008, "9": 2009 };
function vinModelYear(vin) {
  const v = String(vin == null ? "" : vin).toUpperCase().replace(/[^A-Z0-9]/g, "");
  if (v.length !== 17) return null;
  const base = VIN_YEAR_CODE[v[9]];          // position 10 = year code
  if (base == null) return null;
  const newerCycle = /[A-Z]/.test(v[6]);     // position 7: letter => 2010+ cycle, digit => 1980-2009
  let year = newerCycle ? base + 30 : base;
  const maxYear = new Date().getFullYear() + 2;  // guard: never return an impossible future year
  if (year > maxYear) year -= 30;
  return year;
}
function resolveVinYear(vin, vpicModelYear) {
  const vpicYear = vpicModelYear && /^\d{4}$/.test(String(vpicModelYear)) ? Number(vpicModelYear) : null;
  const computed = vinModelYear(vin);
  if (computed != null && vpicYear != null && computed !== vpicYear) {
    console.warn(`VIN year: position-7 decode says ${computed}, vPIC says ${vpicYear}; trusting ${computed} (position-7 cycle rule).`);
  }
  return computed != null ? computed : vpicYear;
}
async function decodeVin(vin) {
  try {
    ...
    const year = resolveVinYear(vin, row.ModelYear);
    return make ? { year, make, model } : null;
  } catch { return null; }
}

// Separator-tolerant VIN detection (VIN feature, flag-gated). VINs pasted from
// documents carry spaces/dashes ("1FA-6P8CF5 J5123456"); strip those separators,
// then match a 17-char run of the VIN alphabet (A-Z0-9 minus I/O/Q). A VIN always
// mixes letters AND digits, so require both, which keeps a 17-letter word or a run
// of digits from ever matching. A pre-1981 chassis number ("CSX2300") is far short
// of 17 chars and never triggers. Never used on the flag-off path (which keeps the
// strict word-boundary detectVin above), so ordinary input is byte-identical.
function detectVinLoose(text) {
  const up = String(text || "").toUpperCase();
  // 1) A clean 17-char VIN on a word boundary (covers a VIN inside a sentence).
  const strict = up.match(/\b[A-HJ-NPR-Z0-9]{17}\b/);
  if (strict && /[A-Z]/.test(strict[0]) && /[0-9]/.test(strict[0])) return strict[0];
  // 2) Separator-broken VIN: a CONTIGUOUS run of VIN-chars + spaces/dashes/dots that
  //    compacts to EXACTLY 17 VIN-chars. "Exactly 17" (not "contains 17"), plus the
  //    VIN alphabet excluding I/O/Q, stops an ordinary phrase - whose compacted run is
  //    a different length and usually carries an O/I - from ever matching. Must mix a
  //    letter and a digit, which every real VIN does.
  const runs = up.match(/[A-HJ-NPR-Z0-9][A-HJ-NPR-Z0-9\s.\-]*[A-HJ-NPR-Z0-9]/g) || [];
  for (const run of runs) {
    // A VIN pasted with separators has at most a space or two; a multi-WORD phrase
    // ("997 Carrera 4S coupe") that happens to compact to 17 chars is not a VIN.
    if ((run.match(/\s/g) || []).length > 2) continue;
    const compact = run.replace(/[\s.\-]+/g, "");
    if (compact.length === 17 && /[A-Z]/.test(compact) && /[0-9]/.test(compact)) return compact;
  }
  return null;
}

// VIN-SHAPED but INVALID: input that is clearly a VIN attempt (a contiguous run that
// compacts to EXACTLY 17 alphanumerics, mixing letters and digits) yet cannot be a real
// VIN because it contains a banned letter - I, O or Q are excluded from every VIN
// worldwide precisely because they read as 1, 0 and 0. This is what detectVinLoose
// rejects (its alphabet omits I/O/Q), so those strings would otherwise fall through to
// the generic resolver and land at the chat layer as a capability denial. Returns the
// compacted string so the caller can offer a corrective, data-true message. Deliberately
// does NOT flag checksum failures: the North American check digit does not apply to many
// valid foreign VINs (e.g. UK/EU), so a checksum test would wrongly reject real VINs.
// Genuine non-VIN text never has this exact shape (17 contiguous alphanumerics with both
// a letter and a digit), so this can never leak onto ordinary input.
function vinShapedButInvalid(text) {
  const up = String(text || "").toUpperCase();
  const runs = up.match(/[A-Z0-9][A-Z0-9\s.\-]*[A-Z0-9]/g) || [];
  for (const run of runs) {
    // Multi-word phrases ("997 Carrera 4S coupe" -> the O in "coupe") must never read as an
    // invalid VIN: a separator-broken VIN carries at most a space or two, not three words.
    if ((run.match(/\s/g) || []).length > 2) continue;
    const compact = run.replace(/[\s.\-]+/g, "");
    if (compact.length === 17 && /[A-Z]/.test(compact) && /[0-9]/.test(compact) && /[IOQ]/.test(compact)) return compact;
  }
  return null;
}

// CHASSIS-NUMBER-SHAPED input: 1 to 3 short tokens that concatenate to an older-car chassis
// number - alphanumeric (chassis separators / . - allowed), 5 to 14 chars after stripping
// separators, mixing at least one letter and one digit, and NOT a 17-char VIN. Requiring a
// mixed letter+digit form keeps bare numbers (years, prices, ZIPs) and pure words from
// firing; the year-token guard keeps ordinary car queries out. This runs only in the failed-
// resolution path (no make/model was found), so a recognized model/chassis-code alias (E46,
// 911, 240Z) never reaches it, and the short-token floor keeps badge codes (RX-2, 240Z) out.
// Feeds an exact archive match (separator-tolerant) plus a gentle "tell me the car" line.
function chassisShapedInput(text) {
  const t = String(text || "").trim();
  if (!t) return false;
  // 1 to 3 short tokens. BaT (and other sources) display older-car chassis WITH a space
  // ("1E 31588", "DB6 MK2 2478"), so a reader types it that way. Allow that narrow multi-
  // token shape, but keep the guards tight: each token must be a short alphanumeric fragment
  // (chassis separators allowed), and NO token may be a 4-digit year (a year signals an
  // ordinary car query like "2016 Cayman GT4", not a chassis).
  const tokens = t.split(/\s+/);
  if (tokens.length > 3) return false;
  // A multi-token input whose FIRST token is a bare 2-4 digit number is a model-number car
  // query ("458 Speciale", "991 GT3 RS", "550 Spyder", "300 SL"), never a chassis - real
  // multi-token chassis lead with a mixed fragment ("1E 31588", "DB6 MK2 2478"). Without this
  // the space-stripped concatenation ("458Speciale") looked chassis-shaped and dead-ended.
  if (tokens.length > 1 && /^\d{2,4}$/.test(tokens[0])) return false;
  for (const tok of tokens) {
    if (!/^[A-Za-z0-9][A-Za-z0-9/.\-]*[A-Za-z0-9]?$/.test(tok)) return false;
    // Per-token short-fragment cap applies ONLY to multi-token input (the "1E 31588" /
    // "DB6 MK2 2478" widening). A SINGLE token relies on the compact 5-14 range below, so a
    // 12-14 char one-piece chassis (pre-1972 GM 13-char VINs like 194371S119431) routes in.
    if (tokens.length > 1 && tok.length > 11) return false;
    if (/^(?:19|20)\d{2}$/.test(tok)) return false;        // a year -> ordinary query, not chassis
  }
  const compact = t.replace(/[\s/.\-]+/g, "");
  if (compact.length < 5 || compact.length > 14) return false;
  if (compact.length === 17) return false;                 // that is a VIN, handled above
  // Mixed letter+digit is the classic chassis shape. ALSO accept a SINGLE all-digit 10-13 token as
  // a pre-1981 chassis CANDIDATE: many pre-1981 cars (Porsche 911/912/930 "9xxxxxxxxx", and other
  // makes filed all-digit) carry an all-numeric chassis, and such an input is a chassis number
  // before it is a phone number (the caller routes it to the archive chassis match, then asks
  // plainly if unmatched - never the phone-number joke). Shorter all-digit runs (years, prices,
  // ZIPs, 9-digit) stay out, so ordinary numeric input is unaffected.
  if (tokens.length === 1 && /^[0-9]{10,13}$/.test(compact)) return true;
  return /[A-Za-z]/.test(compact) && /[0-9]/.test(compact);
}
// A pre-1981 chassis number a phone number could be confused with. Known-prefix all-digit chassis
// (Porsche 911/912/914/930 lead with 9) are confidently chassis; other all-digit 10-13 tokens are
// candidates we ASK about rather than assume. Used only to tune the honest ask, never to decode.
function looksLikePorscheChassis(text) {
  const c = String(text || "").replace(/[\s/.\-]+/g, "");
  return /^9(11|12|14|30)\d{7,10}$/.test(c);
}

// vPIC BodyClass -> our conservative bodyStyle vocabulary (extractBodyStyle's
// groups). Only the unambiguous ones map; anything else stays null so the wizard
// still asks. Prevents the VIN from silently mis-scoping the comp fetch.
function bodyStyleFromVpic(bodyClass) {
  const b = String(bodyClass || "").toLowerCase();
  if (/convertible|cabriolet|roadster|spyder|spider/.test(b)) return "convertible";
  if (/\bcoupe\b|coupé|fastback/.test(b)) return "coupe";
  if (/sedan|saloon/.test(b)) return "sedan";
  if (/wagon|estate/.test(b)) return "wagon";
  return null;
}

// Richer decode for the VIN feature: year/make/model plus body style and a trim/
// series signal, so the wizard can skip questions the VIN already answers. Same
// free vPIC endpoint; returns null on any failure (caller renders honest fallback).
// vPIC often returns an AMBIGUOUS multi-value trim for a VIN it cannot pin to one
// variant: "Turbo (4WD) / GT2 (2WD)", "GLE350 4MATIC/GLE350 4-M Turbo". Feeding that
// string on for re-resolution produced self-contradictory labels ("911 GT2 2WD Turbo
// 4WD", "GLE350 4MATIC GLE350 M") that reached partner lead emails. A slash means "could
// be either", which is not a usable single trim, so we DROP it (the wizard asks the trim,
// or the read stays model-level). Parenthetical drive-type notes are stripped. Never emit
// a garbled merge; a clean model with no trim beats a contradictory trim.
function cleanVinTrim(raw) {
  let t = String(raw == null ? "" : raw).trim();
  if (!t) return null;
  if (t.includes("/")) return null;                 // ambiguous multi-option: not usable
  t = t.replace(/\([^)]*\)/g, " ").replace(/\s+/g, " ").trim();  // strip "(4WD)" etc.
  return t || null;
}
async function decodeVinRich(vin) {
  try {
    ...
    const model = row.Model ? String(row.Model).trim() : null;
    const year = resolveVinYear(vin, row.ModelYear);
    const trim = cleanVinTrim(row.Trim) || cleanVinTrim(row.Series) || null;
    return { year, make, model, trim, bodyStyle: bodyStyleFromVpic(row.BodyClass) };
  } catch { return null; }
}
...
function resolveMakeTypo(text, makes, consumedTokens = new Set()) {
  const tokens = normalize(text).split(/\s+/)
    .filter(token => token.length >= 4 && !/^\d+$/.test(token) && !consumedTokens.has(token) && !FUZZY_MAKE_STOPWORDS.has(token));
...
function matchExactModel(text, modelNames) {
  return modelNames
    // A bare generic designator ("Model"/"Type"/"Tipo"/"Series", an OCD data quirk for some
    // pre-war makes) is never a real nameplate: it must not exact-match the "model" token in
    // "Duesenberg Model J" and hijack the two-token designator model. Longer names that merely
    // START with a designator ("Model J", "Type 57") are unaffected (the regex is anchored).
    .filter(model => textHasTerm(text, model) && !/^(model|type|tipo|series|serie)$/i.test(String(model).trim()))
    .sort((a, b) => String(b).length - String(a).length)[0] || null;
}

// Generic-designator models (Sep 2026, pre-war fix). A model text that LEADS with a generic
// designator word (Model, Type, Tipo, Series, Serie) names its model as that word PLUS the
// following designation token: "Model J", "Model T", "Type 57", "Tipo 61", "Series 62". Without
// this the bare designator hijacked resolution, resolving to an uppercased "MODEL"/"TIPO" with the
// meaningful token dropped, and the whole read then ran on a garbage nameplate. Only fires when the
// designator is the FIRST token (so "3 Series" / "5 Series", where a number leads, are untouched).
// A bare designator with nothing after it is NOT a model (returns {bare:true}); the caller leaves
// the model unresolved and the model clarification asks once, honestly.
const DESIGNATOR_WORD_RE = /^(model|type|tipo|series|serie)$/i;
function designatorModel(modelText) {
  const toks = String(modelText || "").trim().split(/\s+/).filter(Boolean);
  if (!toks.length || !DESIGNATOR_WORD_RE.test(toks[0])) return null;
  const desig = toks[0].charAt(0).toUpperCase() + toks[0].slice(1).toLowerCase();
  const core = toks[1];
  if (!core) return { bare: true };
  // Uppercase a designation code (a pure-alpha "J"/"SJ" or an alphanumeric "57SC"/"35B"); keep a
  // pure number as typed ("57", "61", "41").
  const coreFmt = /^[a-z]+$/i.test(core) || (/\d/.test(core) && /[a-z]/i.test(core)) ? core.toUpperCase() : core;
  return { model: `${desig} ${coreFmt}` };
}
...
function numericTypoCandidates(text, modelNames) {
  ...
  for (const token of tokens) {
    for (const model of numericModels) {
      ...
      // A SAME-LENGTH single-digit swap between two model numbers is a distinct model, not a
      // typo (Sep 2026): 275 vs 575, 330 vs 360, 991 vs 992 are different cars - "correcting"
      // 275 GTB to 575 GTB invents a model that does not exist and grafts the real trim onto it.
      // Only accept an insertion/deletion typo (length differs, e.g. 9111->911, 91->911).
      if (levenshtein(token, head) === 1 && token.length !== head.length) candidates.set(model, token);
    }
  }
...
function fuzzyModelCandidate(text, modelNames) {
  ...
  for (const model of modelNames) {
    ...
    for (const token of tokens) {
      ...
      // Leading-letter designation guard (Sep 2026): a token that is a letter PREFIX + digits
      // ("P400", "P400S") is a spec designation, not a typo. Dropping the "P" to reach a bare
      // NUMBER buried in a multi-token enum model ("400" inside "350 GT / 400 GT") is a false
      // match (Lamborghini Miura P400 mis-suggested the 350/400 GT at 0.75). Mirror of the
      // trailing-letter numStem guard. Narrow: only bare-numeric SUB-tokens of a COMPOUND
      // model name are excluded; a standalone numeric model (a real "911") is untouched.
      const isLetterNum = /^[a-z]\d{2,4}[a-z]*$/i.test(token);
      const parts = (isLetterNum && modelTokens.length > 1)
        ? modelTokens.filter(part => !/^\d{2,4}$/.test(part))
        : modelTokens;
      const score = Math.max(similarity(token, modelNorm), ...parts.map(part => similarity(token, part)));
...
function formatTrim(value) {
  ...
  const words = cleaned.split(/\s+/).filter(word => !TRIM_STOPWORDS.has(word))
    // De-duplicate repeated tokens (a make-alias + owner-model overlap produced
    // "Shelby GT500 Shelby"); keep first occurrence, preserving order.
    .filter(word => { if (seenTok.has(word)) return false; seenTok.add(word); return true; })
    .slice(0, 5).map(word =>
...
// Body style is captured in its own field (lowercase, for the fetch keyword) but should
// read back in the label the way a trim does: "2008 Chrysler 300 Convertible". Title-cased
// for display only; the bodyStyle FIELD stays lowercase so the comp-fetch keyword is stable.
function formatBodyStyle(value) {
  const v = asText(value).toLowerCase().trim();
  // A body style is one style, never a list. Reject list-like values (commas, slashes,
  // ampersands, "and"/"or") so a junk enumeration can never land in the canonical label.
  // Aug 2026: yesterday's bodyStyle-in-label change would otherwise print a value like
  // "Sedan, Convertible, & Wagon" verbatim if a bad source ever populated bodyStyle.
  if (!v || /[,/&]|\b(?:and|or)\b/.test(v)) return "";
...
// Porsche 911-family refinement (Sep 2026). When the make is Porsche and the text names the
// 911 or one of its chassis-code generations, return the code the text names (so the comp pool
// scopes to that generation) and the FULL trim from the curated grammar, matched longest-first
// so multi-word Porsche trims ("Carrera S", "GT3 RS", "Turbo S") survive the generic trim
// whitelist that otherwise reduced them to "S"/"RS". No-op for any non-911 Porsche or non-Porsche.
function is911FamilyModel(model) {
  const n = normalize(model);
  return n === "911" || PORSCHE_911_CODES.includes(n);
}
function refine911(text, make, model) {
  if (!make || !/porsche/i.test(make)) return null;
  const codeInText = PORSCHE_911_CODES.find(c => textHasTerm(text, c));
  if (!(is911FamilyModel(model) || codeInText || textHasTerm(text, "911"))) return null;
  const outModel = codeInText || (is911FamilyModel(model) ? model : "911");
  let outTrim = null;
  for (const t of PORSCHE_911_TRIMS) {
    const re = new RegExp(`(^|[^a-z0-9])${normalize(t).replace(/\s+/g, "\\s+")}([^a-z0-9]|$)`, "i");
    if (re.test(normalize(text))) { outTrim = t; break; }
  }
  return { model: outModel, trim: outTrim };
}
...
async function rankModelsByRecordVolume(taxonomy, make, candidates) {
  ...
  // No signal (empty result or a timed-out ranking query on the large archive): preserve the
  // INPUT order rather than sorting alphabetically. Alphabetical surfaced junk (1-Series,
  // 2002, 228i) and buried curated enthusiast models; input order keeps a sensible default.
  if (!withCount.some(c => c.n > 0)) return [...list];
  return withCount.sort((a, b) => b.n - a.n || a.name.localeCompare(b.name)).map(c => c.name);
}

// Shared "Which <make> model?" chip builder, so the resolver clarification path AND the One
// Box model_choice path produce the SAME chips (the divergence let One Box drop the M3 that
// the resolver surfaces). Assembles the same inputs modelSuggestionChips uses.
export async function modelChipsForMakeYear(env, make, year) {
  if (!make) return [];
  const taxonomy = await loadTaxonomy(env || {});
  const archiveModels = (await getModelsForMake(taxonomy, make)).map(m => m.name);
  const yearModels = await getVpicModelsForMakeYear(make, year || null);
  return modelSuggestionChips(taxonomy, make, year || null, yearModels, archiveModels);
}
// Curated candidate pools for the high-volume enthusiast makes whose partial decodes hit
// this flow. vPIC's per-year model list is both polluted (BMW Motorrad motorcycles) and
// INCOMPLETE (it omits the M3 for 2020), and the live sales-ranking query can time out on
// the large archive - so a curated pool GUARANTEES the enthusiast models (M3 first for BMW)
// are candidates regardless of query health. Ranked by real sales when available; the pool
// order (M-cars first) is the reliable fallback. Free text still covers anything not shown.
const CURATED_MODEL_CHIPS = {
  bmw: ["M3", "3-Series", "M4", "5-Series", "M5", "X5", "4-Series", "2-Series", "Z4", "M2", "X3", "1-Series", "6-Series", "7-Series", "8-Series", "M8", "X1", "X3 M", "X5 M", "i3", "i8"],
  mercedesbenz: ["C-Class", "E-Class", "AMG GT", "SL", "G-Class", "S-Class", "C63 AMG", "E63 AMG", "SLK", "CLK", "CLS", "SLS AMG"],
  audi: ["A4", "S4", "RS4", "A6", "RS6", "R8", "TT", "Q5", "S5", "RS3", "A5", "RS5", "A7", "S6"]
};
async function modelSuggestionChips(taxonomy, make, year, yearModels, archiveModels) {
  ...
  // Curated make: rank the curated pool by real sales (falls back to the curated order, which
  // leads with the enthusiast models, when the ranking query has no signal). M3 for BMW is
  // therefore always a candidate.
  const curated = CURATED_MODEL_CHIPS[normalize(make)];
  if (curated) {
    // Year-scope the curated pool with the SAME production-rule guard as the union path below,
    // so a curated make never offers a model that did not exist in the decoded year (an X5 or
    // Z4 on a 1990 BMW). Models with no production rule pass through; never strip to empty.
    let pool = curated;
    if (year) { const scoped = curated.filter(name => { const rule = productionRuleFor(make, name, null); return !rule?.ranges || yearInRanges(year, rule.ranges); }); if (scoped.length) pool = scoped; }
    const ranked = await rankModelsByRecordVolume(taxonomy, make, pool);
    return (ranked.length ? ranked : pool).slice(0, 6).concat("Not sure");
  }
  // Candidate pool = vPIC's year-models UNION our OWN archive models for this make. vPIC's
  // per-year list is both polluted (it returns BMW Motorrad motorcycles) and INCOMPLETE - it
  // omits the M3 entirely for 2020 even though we trade them constantly - so a model we
  // demonstrably sell must never be excluded just because vPIC left it off its list. Rank
  // the union by our real recent sales volume so motorcycles/zeros sink and the cars people
  // actually sell (M3 included) rise into the top 5.
  let pool = uniqueValues([...(yearModels || []), ...(archiveModels || [])]);
  // Structural guard (shared intent with the frontend filterChipsByEra): drop any candidate
  // whose curated production range excludes the decoded year, so the sales-ranked pool can
  // never surface a model that did not exist yet/anymore. Models with no production rule pass
  // through (no data -> keep), so this only ever removes a known anachronism.
  if (year) pool = pool.filter(name => { const rule = productionRuleFor(make, name, null); return !rule?.ranges || yearInRanges(year, rule.ranges); });
  if (pool.length) {
    const ranked = await rankModelsByRecordVolume(taxonomy, make, pool);
    if (ranked.length) return ranked.slice(0, 5).concat("Not sure");
  }
...
export function sanitizeResolvedVehicle(input) {
  ...
  return {
    ...
    // Body style MUST survive the round-trip: the backend reads car.vehicle through here,
    // and rungFetchParams uses vehicle.bodyStyle to keep the comp fetch scoped ("911
    // Cabriolet" comps not mixed with coupes). Dropping it here silently disabled that
    // scoping on the normal validated path (Aug 2026 fix).
    bodyStyle: asText(input.bodyStyle).toLowerCase() || null,
    // Market-spec + wheelbase MUST survive the round-trip (same reason bodyStyle
    // does): the backend reads car.vehicle through here and classifyRecord uses
    // them to keep NAS Defenders out of a grey-market comp pool. Dropping them
    // here would silently re-blend the pool on the normal validated path.
    marketSpec: asText(input.marketSpec).toLowerCase() || null,
    marketSpecLabel: asText(input.marketSpecLabel) || null,
    marketSpecShort: asText(input.marketSpecShort) || null,
    wheelbase: asText(input.wheelbase) || null,
    // VIN feature: carry the confirmed VIN so sellerDecision can match the exact car
    // in the archive. Travels in the request body only (never a query string/log).
    vin: /^[A-HJ-NPR-Z0-9]{17}$/i.test(asText(input.vin)) ? asText(input.vin).toUpperCase() : null,
    // Chassis feature: an older-car chassis number that exact-matched an archive record
    // (pre-1981 cars have no 17-char VIN). Carried so sellerDecision can attach the same
    // prior-sale enrichment a VIN gets. Request body only, same discipline as vin.
    chassis: asText(input.chassis) || null,
    confidence: ["high", "medium", "low"].includes(input.confidence) ? input.confidence : "medium",
    unverified: input.unverified ? true : undefined,
    canonicalLabel: asText(input.canonicalLabel) || [input.year, make, model, asText(input.wheelbase), asText(input.trim), formatBodyStyle(input.bodyStyle), asText(input.marketSpec) ? asText(input.marketSpec).toUpperCase() : ""].filter(Boolean).join(" ")
  };
}
...
function extractFieldHints(text) {
  ...
  // Engine displacement ("5.0L", "5.0 L", "6.2L", "2.0T", "5.0-liter", "3.4") is
  // stripped as noise BEFORE normalize() can split the decimal point into two junk
  // tokens - "5" (dropped, digit-only) and "0L" (survives, masquerading as a trim,
  // or as the MODEL on a nameplate not in the taxonomy). Not captured for comps
  // (minimal-safe): displacement never scopes the pool, so removing it cannot
  // narrow results, only prevents corruption. Unit-bearing forms first (longest
  // unit alternative first so "5.0liter" is not clipped to "5.0l"); N+.N+ so
  // multi-digit displacements ("6.75L") are whole; then a bare decimal, guarded
  // against N.N-NN valve-count trims ("2.3-16", "2.5-16"). Mileage ("5.0k") and
  // price survive: no L/T unit and no word boundary before a trailing "k".
  const beforeDisp = working;
  working = working
    // Abbreviated displacement units ("5.0L", "5.0 L", "2.0T", "5.0-l") and the US "liter"
    // ("5.0 liter") are engine noise. The British "litre" spelled out is DELIBERATELY NOT stripped
    // here: pre-war Bentleys are named by displacement ("4½ Litre", "8 Litre") and the curated
    // model alias claims it (the "5.0L -> 0L" corruption stays fixed because 5.0L is still caught).
    .replace(/\b\d+\.\d+\s*-?\s*(?:liters?|l|t)\b/gi, " ")
    // Bare decimal ("3.4") noise, but NOT a decimal that is a displacement-model name ("4.5 litre")
    // and NOT a Porsche RS displacement that names the model ("Carrera RS 2.7", "911 RS 3.0"): the
    // lookbehind keeps a decimal that immediately follows "rs" so the RS displacement survives.
    .replace(/(?<!\brs\s)\b\d+\.\d+\b(?!\s*-\s*\d)(?!\s*litres?\b)/gi, " ")
    .replace(/\s+/g, " ").trim();
  if (working !== beforeDisp) hints.displacementStripped = true;
...
let LAST_RESOLVE_DEBUG = null;
export function lastResolveDebug() { return LAST_RESOLVE_DEBUG; }
export async function resolveVehicle(rawInput, options = {}) {
  const DBG = options.debug ? {} : null; if (DBG) LAST_RESOLVE_DEBUG = DBG;
  ...
  if (!options._vinResolved) {
    if (options.vinConfirm) {
      // VIN INPUT FEATURE (flag-gated via options.vinConfirm, set only for crew/test
      // sessions when app_config vin_input_enabled is on). Separator-tolerant detect,
      // rich decode, then PROPOSE (confirm before trusting) rather than silently
      // accepting the decode. Enrich the decoded vehicle (body style, trim, WMI market
      // spec) so the wizard skips questions the VIN answers, and carry the VIN so the
      // archive can be matched exactly after confirmation. This branch NEVER runs on
      // the flag-off path (the else below is today's behavior, verbatim).
      const vin = detectVinLoose(text);
      if (vin) {
        const decoded = await decodeVinRich(vin);
        if (decoded && decoded.make) {
          const canonicalText = [decoded.year, decoded.make, decoded.model, decoded.trim].filter(Boolean).join(" ");
          const resolved = await resolveVehicle(canonicalText, { ...options, _vinResolved: true, vinConfirm: false });
          const v = resolved.vehicle;
          if (v) {
            v.vin = vin;
            if (decoded.bodyStyle && !v.bodyStyle) v.bodyStyle = decoded.bodyStyle;
            // WMI market spec (Defender SALD -> NAS / SALL -> grey), via lib/marketSpec.js.
            const entry = marketSpecEntryFor(v.make, v.model);
            if (entry) {
              const rs = classifyRecordSpec(entry, { vin, title: "" });
              if (rs.specKey && !v.marketSpec) {
                const spec = entry.specs.find(s => s.key === rs.specKey);
                if (spec) { v.marketSpec = spec.key; v.marketSpecLabel = spec.label; v.marketSpecShort = spec.short; }
              }
              if (rs.variantKey && !v.wheelbase) v.wheelbase = rs.variantKey;
            }
            v.canonicalLabel = [v.year, v.make, v.model, v.wheelbase, v.trim, formatBodyStyle(v.bodyStyle), v.marketSpecShort].filter(Boolean).join(" ");
            // Question reads the plain car once, then the FULL market-spec phrase, so
            // it never doubles up ("Defender NAS, North American Spec").
            const questionLabel = [v.year, v.make, v.model, v.wheelbase, v.trim, formatBodyStyle(v.bodyStyle)].filter(Boolean).join(" ");
            const specPhrase = v.marketSpecLabel ? `, ${v.marketSpecLabel}` : "";
            // COLLAPSE (no-match UX): when the VIN decodes to make+year but NO model, we would
            // otherwise confirm ("...a 1990 BMW. Right?") and THEN ask the model on a second
            // screen. Do it in ONE: attach the year-scoped model chips and reword to "Which
            // model?". kind STAYS vin_confirmation so the match-first paths (/sell Option B, One
            // Box vinResolve) still intercept an exact match before this renders - naming the
            // model IS the confirmation. A full decode (model present) keeps the plain Yes/No.
            let vinQuestion = `That VIN decodes to a ${questionLabel}${specPhrase}. Right?`;
            let vinChips = ["Yes, that's it", "No, let me type it"];
            let vinModelOptions = null;
            if (!v.model && v.make) {
              try { vinModelOptions = ((await modelChipsForMakeYear(options, v.make, v.year)) || []).filter(c => !/^not sure$/i.test(c)); } catch (e) { vinModelOptions = null; }
              if (vinModelOptions && vinModelOptions.length) {
                vinQuestion = `That VIN decodes to a ${[v.year, v.make].filter(Boolean).join(" ")}. Which model?`;
                vinChips = vinModelOptions.concat("Not sure");
              }
            }
            // Corrections carry the decoded LABEL only, never the raw VIN (privacy).
            return baseResult("needs_confirmation", v, {
              kind: "vin_confirmation",
              question: vinQuestion,
              chips: vinChips,
              modelOptions: (vinModelOptions && vinModelOptions.length) ? vinModelOptions : null,
              suggestion: v.canonicalLabel,
              baseVehicle: [v.year, v.make].filter(Boolean).join(" ")
            }, [{ type: "vin_decode", to: v.canonicalLabel }]);
          }
        }
        // Decode failed (grey-market cars often will not decode): honest, not an error.
        return baseResult("needs_clarification", {
          raw: text, year: null, yearRange: null, make: null, model: null, trim: null,
          color: null, mileage: null, bodyStyle: null, confidence: "low"
        }, {
          kind: "vin_decode_failed",
          question: "That VIN doesn't decode, which usually means a car that wasn't sold new in the US. Tell me the car instead, year, make and model.",
          chips: ["Not sure"]
        }, []);
      }
      // VIN-shaped but invalid (banned letter like a capital O for a zero): a data
      // observation about THIS string, never a claim the feature can't decode VINs.
      // Scoped to VIN-shaped input only, so ordinary non-VIN text is untouched.
      const invalidVin = vinShapedButInvalid(text);
      if (invalidVin) {
        return baseResult("needs_clarification", {
          raw: text, year: null, yearRange: null, make: null, model: null, trim: null,
          color: null, mileage: null, bodyStyle: null, confidence: "low"
        }, {
          kind: "vin_invalid_shape",
          question: "That doesn't look like a valid VIN. Real VINs never use the letter O, only the number 0. Want to double check it, or just tell me the year, make and model instead?",
          chips: ["Not sure"]
        }, []);
      }
      // Chassis-number-shaped input (pre-1981 cars have no 17-char VIN): a short single
      // alphanumeric token mixing letters and digits, not a car we resolved. Honest data
      // observation + the real next step. Copy only, no decoder (no marque mapping) - the
      // reader's E-Type chassis "1E14107" used to fall to the chat layer's generic
      // "code of some kind" filler. Deferred: the actual pre-1981 chassis decode/match.
      if (chassisShapedInput(text)) {
        // Copy is confidence-scaled. A CLASSIC chassis shape (mixed letter+digit, or a known
        // pre-1981 all-digit prefix like Porsche 911/912/930) is confidently a chassis. A bare
        // unrecognized all-digit token (which a mistyped phone number also is) gets the SOFTER,
        // honest ASK instead of asserting it is a chassis - never the phone-number joke either way.
        const compact = String(text).replace(/[\s/.\-]+/g, "");
        const confidentChassis = /[A-Za-z]/.test(compact) || looksLikePorscheChassis(text);
        const question = confidentChassis
          ? "That looks like a chassis number from an older car. I keep thousands of these from past sales, but this one isn't in the records I've tracked. Tell me the year, make and model and I'll take it from there."
          : "Is that a chassis number? If so, tell me the make and year and I'll try to match it. If it's something else, just give me the year, make and model.";
        return baseResult("needs_clarification", {
          raw: text, year: null, yearRange: null, make: null, model: null, trim: null,
          color: null, mileage: null, bodyStyle: null, confidence: "low"
        }, { kind: "chassis_hint", question, chips: ["Not sure"] }, []);
      }
      // No VIN in the input: fall through to the normal resolver below (unchanged).
    } else {
      // FLAG OFF: today's silent VIN decode, unchanged (byte-identical).
  ...
  if ((!year && !yearRange) || !make) {
    ...
    // Porsche 911 family: carry the chassis-code model + FULL trim on the year-missing partial
    // too, so One Box (which proceeds on make+model without a year) scopes to "997 Carrera S",
    // not the whole 997 pool. Mirrors the valid-path refinement below.
    let understoodTrim = null;
    const p911partial = refine911(text, make, understoodModel);
    if (p911partial) { understoodModel = p911partial.model || understoodModel; understoodTrim = p911partial.trim || null; }
    // Generic trim on the year-missing partial (Sep 2026, defect 1-strict): non-911 models must
    // carry their trim too, or "928 s4" drops S4 and Sam asks the year as if it were a bare 928.
    // Mirrors the valid path's extractTrim; null when there is no trim, so bare models are untouched.
    if (!understoodTrim && understoodModel) {
      try {
        let t = extractTrim(removeKnownNoise(text, make, year, consumedAliasTexts), understoodModel, null);
        // Never let a bare chassis/generation code (718, 997, 981) survive as a trim ("718 Cayman S"
        // is trim "S", generation 718 - not trim "718 S"). Alphanumeric trims (S4, GT) are kept.
        if (t) t = t.split(/\s+/).filter(tok => !/^\d{2,4}$/.test(tok)).join(" ").trim();
        understoodTrim = t || null;
      } catch { /* keep null */ }
    }
    let question = "What year, make and model are we talking about?";
    if (make && !year) {
      question = `Got it, a ${make}${understoodModel ? ` ${understoodModel}${understoodTrim ? ` ${understoodTrim}` : ""}` : ""}. What year is it?`;
    } else if (year && !make) {
      question = `${year}, noted. What make and model is it?`;
    }
    return baseResult("needs_clarification", partialVehicle({ make, model: understoodModel, trim: understoodTrim }), {
      question,
  ...
  // A bare generic designator ("Model"/"Type"/"Tipo"/"Series") is an OCD data quirk for some
  // pre-war makes (Duesenberg's sole filed model is literally "Model"), never a real nameplate.
  // Strip it from EVERY model candidate list at the source so no matcher - exact, fuzzy or typo -
  // can resolve the "model" token in "Duesenberg Model" to the garbage word; the two-token
  // designator model ("Model J") is formed by the designator rule and is unaffected.
  const notBareDesignator = name => !/^(model|type|tipo|series|serie)$/i.test(String(name || "").trim());
  const archiveModelNames = archiveModels.map(model => model.name).filter(notBareDesignator);
  const yearModels = (await getVpicModelsForMakeYear(make, year)).filter(notBareDesignator);
  ...
  if (DBG) { DBG.modelText = modelText; DBG.remainder = remainder; DBG.aliasModel = aliasModel;
    DBG.yearExact = yearExact; DBG.archiveExact = archiveExact;
    DBG.archiveModelSample = archiveModelNames.filter(m => /miura|400|350/i.test(m)).slice(0, 12);
    DBG.fuzzyAll = fuzzyModelCandidate(remainder, uniqueValues([...yearModels, ...archiveModelNames])); }
  if (yearExact) {
    model = yearExact;
  ...
  // Sub-brand nameplate under a known make ("Land Rover Velar"): when the catalog
  // could not resolve the model but a curated ownership alias in the text maps to a
  // model of THIS make, adopt it. Runs AFTER every catalog exact-match so the fuller
  // catalog name still wins for the prefixed form ("Range Rover Velar"), and only
  // when the make is already the alias's owner (never infers or changes the make).
  if (!model) {
    const owned = MODEL_OWNERSHIP.find(item =>
      item.makes.some(m => normalize(m) === normalize(make)) &&
      item.aliases.some(alias => textHasTerm(text, alias)));
    if (owned) { model = owned.model; modelSource = "ownership"; }
  }

  // Generic-designator model (Model J, Type 57, Tipo 61, Series 62). Runs AFTER the exact/alias/
  // catalog matchers so a fuller curated or taxonomy name still wins, but BEFORE the fuzzy/
  // designation fallback that would otherwise uppercase a bare designator and drop the token.
  // Overrides a null match or a bare-designator match ("Model"/"Tipo"/"Type"); a bare designator
  // with nothing after it clears the model so the clarification asks once.
  {
    const desig = designatorModel(modelText);
    const bareDesig = model && DESIGNATOR_WORD_RE.test(String(model).trim());
    if (desig && desig.model && (!model || bareDesig)) { model = desig.model; modelSource = "designator"; }
    else if (desig && desig.bare && bareDesig) { model = null; modelSource = null; }
  }
  ...
  if (!model) {
    ...
    // Never auto-fill the model with a junk body-style-enumeration name (a make whose
    // only known "model" is "Sedan, Convertible, & Wagon"). Drop it so the flow falls
    // through to the model-clarification ask instead of fabricating a garbage label.
    if (single && isBodyStyleEnumModel(single)) single = null;
    // A bare generic designator ("MODEL"/"TYPE"/"TIPO") is not a real nameplate: some pre-war
    // makes (Duesenberg) are filed in OCD with their sole model literally as "Model", which would
    // otherwise auto-resolve every bare "Duesenberg" / "Duesenberg Model" to the garbage word.
    // Drop it so the flow asks which model, exactly as an unknown model does.
    if (single && DESIGNATOR_WORD_RE.test(String(single).trim())) single = null;
    if (single) {
      model = single;
  ...
  if (!model) {
    // Join a letter-hyphen-digit sequence the space-normalize would otherwise split, so
    // a hyphenated designation is detected the same as its unhyphenated form ("RX-2" is
    // the same designation as "RX2"). This is reached only as a last resort - every
    // catalog/alias/fuzzy match (including hyphenated catalog models like RX-7) was tried
    // above - so it never disturbs a verified nameplate, and the space-normalize elsewhere
    // (fuzzy/catalog) is untouched, so a clean "RX2" still never near-misses "RX-7"/"RX-8".
    const designationText = normalize(String(remainder).replace(/([a-z])-(\d)/gi, "$1$2"));
    const designation = designationText.split(/\s+/).find(token =>
    ...
    if (designation) {
      ...
      for (const m of catalog) {
        ...
        // Leading-letter guard (Aug 2026, same idea as the badge leading-digit guard):
        // a letter-prefixed designation's first letter is its model-series identity
        // (GM C/K/R/V/U series), not a typo. Never "correct" C10 onto a different
        // series like U100 - when both start with a letter and those letters differ,
        // it is a different model line, so skip it.
        const dLead = /^[a-z]/.test(designation) ? designation[0] : null;
        const hLead = /^[a-z]/.test(head) ? head[0] : null;
        if (dLead && hLead && dLead !== hLead) continue;
        // Numbered-series guard (Sep 2026): when the designation and a catalog head
        // share the SAME alpha prefix and differ only in the trailing NUMBER, that
        // number is the model identity within a series (Aston DB2/DB3/DB4/.../DB11,
        // Jaguar XJ6/XJ8), not a typo. "Correcting" DB3 to DB9 is always wrong - every
        // sibling sits at edit distance 1 - so a real but uncataloged series member
        // gets mangled into whichever sibling we happen to carry. Skip: let it fall
        // through and be accepted as an unverified real designation instead.
        const dParts = designation.match(/^([a-z]+)(\d+)$/i);
        const hParts = head.match(/^([a-z]+)(\d+)$/i);
        if (dParts && hParts && dParts[1] === hParts[1] && dParts[2] !== hParts[2]) continue;
  ...
  if (!model) {
    // A typed model we could not match is NOT the same as no model given (Sep 2026, reader
    // "CLK DTM"). If the seller put something in the model position (a substantive, non-noise
    // leftover), never silently drop it and re-ask "which model?" for what they already typed:
    // accept it as an UNVERIFIED designation and run the broader make-level read, which the
    // wizard surfaces as "I don't recognise the {make} {model} as a model I track" (the 450S
    // pattern). Only a genuinely EMPTY model position (bare year + make) still asks the model.
    // Coachwork / spec descriptors are NOT a model - never grab them as the designation (a
    // "1928 Bentley 4.5 Supercharged, Le Mans bodied" must not become model "SUPERCHARGED").
    const COACHWORK_WORDS = new Set(["supercharged", "blower", "tourer", "saloon", "drophead", "bodied",
      "body", "coachwork", "coachbuilt", "coachbuild", "replica", "recreation", "tribute", "special",
      "edition", "style", "styled", "type", "lemans", "sports", "sport", "twoseater", "fourseater"]);
    // Conversational preamble + document words are never a model: "think its a 1973 porsche vin"
    // used to grab "THINK" as the model. These are hedges, fillers and identifier words a seller
    // wraps around the real car, not a nameplate. (The make/typo matcher got the same guard.)
    const CONVERSATIONAL_WORDS = new Set(["think", "thinks", "thinking", "maybe", "probably", "possibly",
      "pretty", "sure", "guess", "guessing", "believe", "reckon", "its", "it's", "thats", "that's",
      "vin", "chassis", "serial", "number", "num", "plate", "reg", "registration", "the", "this",
      "that", "some", "kind", "sort", "about", "around", "roughly", "ish", "maybe's"]);
    const leftoverTokens = normalize(remainder).split(/\s+/).filter(token =>
      token.length >= 2 && !/^\d+$/.test(token) && !TRIM_STOPWORDS.has(token) &&
      !AMBIGUOUS_ENGLISH_MODELS.has(token) && !MARKET_SPEC_WORDS.has(token) && !isBodyStyleWord(token) &&
      !COACHWORK_WORDS.has(token) && !CONVERSATIONAL_WORDS.has(token) &&
      // A bare generic designator ("model"/"tipo"/"series"; "type" is already coachwork) is not a
      // model on its own - accepting it here is what produced the garbage unverified "MODEL"/"TIPO".
      // Drop it so the empty-model branch asks which model, exactly as item 1 requires.
      !DESIGNATOR_WORD_RE.test(token));
    if (leftoverTokens.length) {
      model = leftoverTokens[0].toUpperCase();
      modelSource = "seller_designation_unverified";
    } else {
  ...
  let dirtyTokens = trimValidation.dropped;
  if (!trim && year) {
  ...
  // Porsche 911-family repair (Sep 2026): the generic trim whitelist reduces multi-word 911
  // trims to a fragment when the make was inferred from a bare code ("997 Carrera S" -> "S",
  // "991 GT3 RS" -> "RS"). Re-derive the full trim from the curated 911 grammar and normalize
  // the model to the chassis code the text names, so the comp pool scopes to the right car.
  const p911 = refine911(text, make, model);
  if (p911) { if (p911.model) model = p911.model; if (p911.trim) trim = p911.trim; }
  ...
  // Market-spec + wheelbase capture (Sep 2026): federalized-vs-grey-market and
  // wheelbase are their own axes, not trim. Recognize the designation so it is
  // never dropped as dirty ("NAS", "90") nor mangled into a junk trim ("Spec"),
  // and carry it so the comp pool can separate NAS 90s from grey-market 110s.
  // Data-driven via lib/marketSpec.js; a no-op for every non-mapped nameplate.
  let marketSpec = null, marketSpecLabel = null, marketSpecShort = null, wheelbase = null;
  const specEntry = make && model ? marketSpecEntryFor(make, model) : null;
  if (specEntry) {
    const found = detectSearchSpec(specEntry, text);
    if (found) {
      marketSpec = found.specKey; marketSpecLabel = found.specLabel; marketSpecShort = found.specShort;
      wheelbase = found.variantKey;
      // Scrub the recognized tokens from the trim + dirty list so they render as
      // structured fields, not junk. "North American Spec" leaves no trim behind.
      const specWords = new Set([...MARKET_SPEC_WORDS]);
      if (wheelbase) specWords.add(wheelbase);
      if (trim) {
        const keptTrim = String(trim).split(/\s+/).filter(tok => !specWords.has(normalize(tok)));
        trim = keptTrim.length ? keptTrim.join(" ") : null;
      }
      if (dirtyTokens && dirtyTokens.length) {
        dirtyTokens = dirtyTokens.filter(tok => !specWords.has(normalize(tok)));
      }
    }
  }
  ...
  const vehicle = {
    ...
    canonicalLabel: [year || yearRangeLabel(yearRange), make, model, wheelbase, trim, formatBodyStyle(bodyStyle), marketSpecShort].filter(Boolean).join(" ")
  };
