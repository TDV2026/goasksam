...
// CHASSIS-NUMBER-SHAPED input (copy-only trigger, NOT a decoder): a single token that
// looks like an older-car chassis number - alphanumeric (chassis separators / . - allowed),
// 5 to 14 chars, mixing at least one letter and one digit, and NOT a 17-char VIN. Requiring
// a mixed letter+digit token keeps bare numbers (years, prices, ZIPs) and pure words from
// firing. This runs only in the failed-resolution path (no make/model was found), so a
// recognized model/chassis-code alias (E46, 911, 240Z) never reaches it. The message it
// gates is a gentle "tell me the car", harmless even on the rare false positive. Deferred:
// mapping the chassis to a make/model, or an exact archive match. See memory.
function chassisShapedInput(text) {
  const t = String(text || "").trim();
  if (/\s/.test(t)) return false;                          // single token only
  if (t.length < 5 || t.length > 14) return false;
  if (!/^[A-Za-z0-9][A-Za-z0-9/.\-]*[A-Za-z0-9]$/.test(t)) return false;
