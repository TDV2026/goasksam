...
function extractFieldHints(text) {
  ...
  const regionMatch = working.match(new RegExp("[,.]?\\s*(?:located\\s+)?(?:in\\s+)?(?:the\\s+)?(" + regionAlt + ")\\.?\\s*$", "i"));
...
export function labelIsProvablyCar(label, vehicle) {
  ...
  return String(label).split(/\s+/).filter(Boolean).every(token => {
    const clean = normalize(token);
    return consumed.has(clean) || trimTokenAllowed(token);
  });
}
