...
function extractFieldHints(text) {
  ...
  const hints = {};
  const locationPattern = new RegExp(`[,.]?\s*(?:located\s+)?(?:in\s+)?(?:the\s+)?(${[...Object.keys(REGION_TOKENS), ...US_STATE_NAMES].map(t => t.replace(/\s+/g, "\\s+")).join("|")})\.?\s*$`, "i");
  const locationMatch = working.match(locationPattern);
  if (locationMatch) {
    const token = locationMatch[1].toLowerCase().replace(/\s+/g, " ");
...
export async function resolveVehicle(rawInput, options = {}) {
  ...
  const partialVehicle = extra => ({
    ...
    locationHint: fieldHints.locationHint || null, priceHint: fieldHints.priceHint || null, ...extra
  });
  ...
  if (model) {
    ...
    if (!yearTrimRule && yearModels.length && !matchExactModel(model, yearModels) && !rule?.ranges) {
