...
async function inferMakeFromModel(taxonomy, text) {
  ...
  const tokens = normalize(text).split(/\s+/)
    .filter(token => token.length >= 3 && !/^\d{1,2}$/.test(token) && !TRIM_STOPWORDS.has(token) && !AMBIGUOUS_ENGLISH_MODELS.has(token) && !isBodyStyleWord(token))
...
export function sanitizeResolvedVehicle(input) {
  ...
  return {
    ...
    canonicalLabel: asText(input.canonicalLabel) || [input.year, make, model, asText(input.trim), formatBodyStyle(input.bodyStyle)].filter(Boolean).join(" ")
  };
}
...
export async function resolveVehicle(rawInput, options = {}) {
  ...
  const _dbgRawTrim = trim; // TEMP diagnostic
  ...
  const vehicle = {
    ...
    canonicalLabel: [year || yearRangeLabel(yearRange), make, model, trim, formatBodyStyle(bodyStyle)].filter(Boolean).join(" ")
  };
  if (options.debug) vehicle._dbg = { modelSource, remainder, aliasModel, aliasTrim, aliasTrimForModel, rawTrim: _dbgRawTrim, gluedText, modelText }; // TEMP
