  ...
  if (model) {
    ...
    if (!rule?.ranges) {
      ...
      if (generations.length) {
        ...
        if (year < genMin - 2 || year > genMax + 2) {
          return baseResult("needs_clarification", partialVehicle({ make, model }), {
            question: `A ${year} ${make} ${model} doesn't line up: ${model} production in my data runs ${genMin} to ${genMax}. Double-check the year?`,
            missing: ["year"],
            baseVehicle: [make, model].join(" "),
            chips: ["Change car", "Not sure"]
          }, corrections);
        }
      } else if (taxonomy.env) {
