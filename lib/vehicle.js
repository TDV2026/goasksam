...
export async function resolveVehicle(rawInput, options = {}) {
  ...
  if (!make) {
    ...
    if (makeAlias && makeAlias.kind !== "misspelling") {
      ...
      // Fix 2: a curated single-make misspelling is a confident match, so it
      // resolves SILENTLY rather than asking "Did you mean" (the friendly re-ask
      // is reserved for when nothing clears).
      make = makeAlias.make;
      makeConfidence = "high";
      consumedAliasTexts.push(makeAlias.alias);
  ...
  // Fix 2: systematic edit-distance <=2 typo resolution BEFORE the fuzzy-confirm.
  // A single confident match resolves silently; several equally close makes ask.
  if (!make) {
    const typo = resolveMakeTypo(text, taxonomy.makes, new Set(consumedAliasTexts.map(normalize)));
    if (typo && typo.make) {
      make = typo.make;
      makeConfidence = "high";
      consumedAliasTexts.push(typo.token);
      corrections.push({ type: "typo", from: typo.token, to: typo.make });
    } else if (typo && typo.ambiguous && typo.ambiguous.length > 1) {
  ...
  const modelText = removeKnownNoise(text, make, year, consumedAliasTexts, { gentle: true });
