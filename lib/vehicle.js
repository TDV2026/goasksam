    ...
    if (options.vinConfirm) {
      ...
      if (chassisShapedInput(text)) {
        return baseResult("needs_clarification", {
          ...
          kind: "chassis_hint",
          // No-match copy (locked precision rule): this renders ONLY after the exact-match
          // lookup ran and found nothing (chassis_hint fires only for chassis-shaped input,
          // which always triggers the archive lookup). It claims OUR records only - never
          // that the car did not sell (we may simply lack its identifier, e.g. a pre-2026
          // Cars & Bids sale). Bare numbers / sentences never reach here, so the "I checked"
          // framing never renders when no lookup ran.
          question: "That looks like a chassis number from an older car. I keep thousands of these from past sales, but this one isn't in the records I've tracked. Tell me the year, make and model and I'll take it from there.",
          chips: ["Not sure"]
        }, []);
      }
