...
// LAUNCH FIX (Sep 2026): sales_archive stores the DISPLAY LABEL in its `platform` column
// ("RM Sotheby's"), and One Box aliases source:platform - so the raw value handed to the
// house logic is the label, which never equals the slug ("rmsothebys"), and the premium
// back-out silently never fired. All house math must key on the SLUG. Rows now carry a
// `source_slug` column populated at ingest; where it is present it wins. As a bridge for
// rows not yet backfilled, an EXACT label->slug map (a deterministic bijection, NOT fuzzy
// normalization - "Gooding & Co" -> "gooding") resolves the slug. Already-slug inputs pass
// through unchanged.
const LABEL_TO_SLUG = {
  "bring a trailer": "bringatrailer", "cars & bids": "carsandbids", "hagerty": "hagerty",
  "pcarmarket": "pcarmarket", "all collector cars": "acc", "gooding & co": "gooding",
  "rm sotheby's": "rmsothebys", "hemmings": "hemmings", "sotheby's motorsport": "sothebysmotorsport",
  "mb market": "mbmarket", "autohunter": "autohunter", "barrett-jackson": "barrettjackson",
  "mecum auctions": "mecum", "bonhams": "bonhams", "broad arrow": "broadarrow",
  "car & classic": "carandclassic", "collecting cars": "collectingcars", "the market": "themarket",
  "pistonheads": "pistonheads"
};
// Canonical OCD source slug for a row or a raw source value. Prefers an explicit
// source_slug (ingest-populated / backfilled); else exact label->slug; else a bare
// lowercase (an input that is already a slug resolves to itself).
export function sourceSlugOf(input) {
  if (input && typeof input === "object") {
    const explicit = String(input.source_slug || "").toLowerCase().trim();
    if (explicit) return explicit;
    const label = String(input.source || "").toLowerCase().trim();
    return LABEL_TO_SLUG[label] || label;
  }
  const s = String(input || "").toLowerCase().trim();
  return LABEL_TO_SLUG[s] || s;
}
// Accepts a row OR a raw source/label string; always keys the check on the resolved slug.
export const isHouseSource = input => HOUSE_SOURCES.has(sourceSlugOf(input));

// CONSIGNMENT DOORS (Sep 2026): the six houses ARE routable - each runs a consignment intake page,
// the house equivalent of "Start listing". A DISTINCT verb ("consign", never "list"): consignment
// is an enquiry plus a negotiated agreement and seller's commission, not a self-serve listing. Kept
// here (the house-routing source of truth) rather than ROUTE_POLICIES, because that object's
// membership derives the VOLUME routable flag - a house there would wrongly become a volume listing
// pick. All six verified 200 to the exact consignment page (no homepage redirect), Sep 2026.
export const HOUSE_CONSIGN = {
  rmsothebys: { label: "RM Sotheby's", verb: "consign", url: "https://rmsothebys.com/consign/" },
  broadarrow: { label: "Broad Arrow", verb: "consign", url: "https://www.broadarrowauctions.com/consignment" },
  gooding: { label: "Gooding & Co", verb: "consign", url: "https://www.goodingco.com/consign" },
  barrettjackson: { label: "Barrett-Jackson", verb: "consign", url: "https://www.barrett-jackson.com/consignment-leads" },
  bonhams: { label: "Bonhams", verb: "consign", url: "https://sell.bonhams.com/?category=Motor%20Cars" },
  mecum: { label: "Mecum", verb: "consign", url: "https://www.mecum.com/collector-cars/how-to-sell/" }
};
export const consignUrlFor = input => { const s = sourceSlugOf(input); return HOUSE_CONSIGN[s] ? HOUSE_CONSIGN[s].url : null; };
...
const BON_UK = [[500000, 0.15], [Infinity, 0.12]];   // Bonhams UK motor cars (GBP): 15% to £500k, 12% above (verified published, Sep 2026)
const BON_FR = [[Infinity, 0.15]];                   // Bonhams France & Monaco (EUR): flat 15% (verified published)
const FLAT_10 = [[Infinity, 0.10]];                  // Barrett-Jackson / Mecum: flat 10% (verified empirically, 100/100 & 19/20 round-hammer)
// A schedule is keyed by currency (lowercased); `other` is the fallback for any currency
// not listed, and `usd` is the last resort. Verified rates + citations live in
// docs/source-certification.md. Bonhams is per-region because its GBP/EUR terms differ
// materially from its US terms (empirical USD calibration missed the non-US schedules).
const HOUSE_SCHEDULES = {
  gooding: { usd: NA_250, other: NA_250 },
  bonhams: { usd: NA_250, gbp: BON_UK, eur: BON_FR, other: NA_250 },
...
export function hammerUsd(row) {
  ...
  if (!HOUSE_SOURCES.has(slug)) return toUsd(price, currency);      // online = already hammer
  const sched = HOUSE_SCHEDULES[slug];
  // Currency-specific tier first (Bonhams GBP/EUR), then `other`, then `usd`, then flat.
  const tiers = sched
    ? (sched[currency.toLowerCase()] || (currency === "USD" ? sched.usd : sched.other) || sched.usd)
    : FLAT_10;
...
export function priceDisplay(row) {
  ...
  return {
    ...
    premiumInclusive: isHouseSource(row)
  };
}

// Native-currency figures for a receipt (item 5): the hammer and buyer-paid total in the sale's
// OWN currency, so a EUR/GBP house sale can show the native figure first with the USD beside it.
// Online rows have no premium (hammer === total in native). Returns null when there is no price.
export function nativePrices(row) {
  const price = Number(row.price);
  if (!(price > 0)) return null;
  const currency = String((row.currency || "USD")).toUpperCase();
  const slug = sourceSlugOf(row);
  if (!HOUSE_SOURCES.has(slug)) return { currency, hammer: Math.round(price), total: null };
  const sched = HOUSE_SCHEDULES[slug];
  const tiers = sched ? (sched[currency.toLowerCase()] || (currency === "USD" ? sched.usd : sched.other) || sched.usd) : FLAT_10;
  return { currency, hammer: Math.round(hammerFromTotal(price, tiers)), total: Math.round(price) };
}
...
export function mileageInfo(row) {
  ...
  let stated = null;
  if (structured == null && isHouseSource(row)) {
