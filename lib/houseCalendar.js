...
// DATA-CARRIED event, stated as ONE fact ONLY when the event is genuinely in the record:
//  - RM Sotheby's: `city` is the sale location AND the URL slug encodes the event ("/auctions/mo26/"),
//    which agree, so the event is real data.
//  - Barrett-Jackson: the event is literally in the URL path ("/2026-las-vegas/").
// Everyone else -> null, and the caller uses roomForHouseMonth as an INFERRED, hedged second fact.
// Bonhams is DELIBERATELY NOT here: it carries only a `city` ("Salinas", the county town near the
// Quail/Laguna Seca venue), not an event name - deriving "this is their Monterey-week sale" from that
// city is an inference, so Bonhams is treated with the same two-fact honesty as Gooding/BA/Mecum.
export function eventFromRecord(rec) {
  ...
  if (slug === "rmsothebys" && city) return { room: city, source: "data" };
  if (slug === "barrettjackson") {
    const m = String(rec.url || "").match(/barrett-jackson\.com\/([0-9]{4}-[a-z-]+)\//i);
...
// HOUSE-BY-HOUSE comparison structure (build 3+4). Pure: groups the scoped house receipts by
// house, ranks by the RECORD (share of these sales, then recency, then median hammer), attaches the
// room per receipt (data-carried where present, else inferred + hedged) and the next sale per house.
// ASAP reorders to lead with the soonest next sale instead of the strongest record. Never ranks by
// a promised outcome; the receipts carry the price story.
export function buildHouseComparison(receipts, opts = {}) {
  const todayISO = opts.todayISO || new Date().toISOString().slice(0, 10);
  const houseRecs = (receipts || []).filter(r => r && r.isHouse && Number(r.hammer) > 0 && HOUSE_CALENDAR[String(r.slug || "").toLowerCase()]);
  if (!houseRecs.length) return null;
  const totalHouse = houseRecs.length;
  const groups = {};
  for (const r of houseRecs) {
    const slug = String(r.slug).toLowerCase();
    (groups[slug] || (groups[slug] = { slug, display: houseDisplay(slug) || r.venue, recs: [] })).recs.push(r);
  }
  const houses = Object.values(groups).map(g => {
    const hammers = g.recs.map(r => r.hammer).sort((a, b) => a - b);
    const dates = g.recs.map(r => String(r.date || "").slice(0, 10)).filter(Boolean).sort();
    const receipts = g.recs.slice()
      .sort((a, b) => String(b.date || "").localeCompare(String(a.date || "")) || b.hammer - a.hammer)
      .slice(0, 4).map(r => {
        const dataEv = eventFromRecord(r);
        const month = Number(String(r.date || "").slice(5, 7)) || null;
        const inferred = !dataEv ? roomForHouseMonth(g.slug, month) : null;
        return {
          hammer: r.hammer, allIn: r.allIn || null, date: r.date, year: r.year || null,
          chassis: r.chassis || r.chassisTail || null, image: r.image || null, title: r.title || null, url: r.url || null,
          room: dataEv ? { name: dataEv.room, source: "data" }
              : (inferred ? { name: inferred.room, event: inferred.event, source: "inferred" } : null)
        };
      });
    return {
      slug: g.slug, display: g.display, count: g.recs.length, share: g.recs.length / totalHouse,
      median: hammers[Math.floor((hammers.length - 1) / 2)], lo: hammers[0], hi: hammers[hammers.length - 1],
      mostRecent: dates.length ? dates[dates.length - 1] : null,
      receipts, nextSale: nextSaleForHouse(g.slug, todayISO)
    };
  });
  houses.sort((a, z) => (z.count - a.count) || String(z.mostRecent || "").localeCompare(String(a.mostRecent || "")) || (z.median - a.median));
  let asapLead = null;
  if (opts.asap) {
    const withNext = houses.filter(h => h.nextSale).slice()
      .sort((a, z) => (a.nextSale.year - z.nextSale.year) || (a.nextSale.month - z.nextSale.month));
    asapLead = withNext.length ? withNext[0].slug : null;
  }
  return { houses, totalHouse, asap: !!opts.asap, asapLead };
}
...
export function nextSaleForHouse(slug, todayISO, opts = {}) {
  const cal = HOUSE_CALENDAR[slug]; if (!cal || !cal.events.length) return null;
  const preferDomestic = opts.preferDomestic !== false; // US-only launch: name the next US sale.
  const today = new Date(todayISO || Date.now());
  const y = today.getUTCFullYear(), m = today.getUTCMonth() + 1;
  const scored = cal.events.map(e => {
    // Roll to next year if this event's month is this month or already past.
    const dy = (e.month <= m) ? y + 1 : y;
    return { ...e, year: dy, monthsAway: (dy - y) * 12 + (e.month - m) };
  }).sort((a, b) => a.monthsAway - b.monthsAway);
  const domestic = scored.filter(e => !e.intl);
  const n = (preferDomestic && domestic.length) ? domestic[0] : scored[0];
  return {
