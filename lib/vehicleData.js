...
export const MODEL_ALIASES = [
  ...
  // 300SL is ONE model token, not "300" + trim "SL": the space/hyphen spellings must resolve to the
  // model, or "300 SL" splits into model "300" (all 300-series) + trim "SL" and reads a different,
  // wrong pool than "300SL" (Part 3). The no-space "300SL" already resolves; alias the others.
  { alias: "300 sl", make: "Mercedes-Benz", model: "300SL", kind: "nickname" },
  { alias: "300-sl", make: "Mercedes-Benz", model: "300SL", kind: "nickname" },
  ...
  // Bare Mercedes model families (Sep 2026, reader "CLK DTM" case): OCD/vPIC title these by
  // variant badge (CLK350, SLK55) so the bare family name never matched and the model was
  // silently dropped into a "which model?" re-ask. Map each family head to its "-Class" model so
  // the MODEL_FAMILY defrag (lib/modelFamilies.js) fetches its badges. ML/SLR have no family map,
  // so they resolve to the plain model and fetch by keyword.
  { alias: "clk", make: "Mercedes-Benz", model: "CLK-Class", kind: "nickname" },
  { alias: "cl", make: "Mercedes-Benz", model: "CL-Class", kind: "nickname" },
  { alias: "cls", make: "Mercedes-Benz", model: "CLS-Class", kind: "nickname" },
  { alias: "slk", make: "Mercedes-Benz", model: "SLK-Class", kind: "nickname" },
  { alias: "ml", make: "Mercedes-Benz", model: "M-Class", kind: "nickname" },
  { alias: "slr", make: "Mercedes-Benz", model: "SLR McLaren", kind: "nickname" },
  // SLS AMG and AMG GT were missing from the bare-family set (the CLK fix added CLK/CL/CLS/SLK/ML/
  // SLR only), so the archive scope never recognized them as their own nameplates. "SLS" always
  // means the AMG gullwing (there is no non-AMG SLS); "AMG GT" is the standalone sports car.
  { alias: "sls", make: "Mercedes-Benz", model: "SLS AMG", kind: "nickname" },
  { alias: "sls amg", make: "Mercedes-Benz", model: "SLS AMG", kind: "nickname" },
  { alias: "amg gt", make: "Mercedes-Benz", model: "AMG GT", kind: "nickname" },
  // The CLK DTM AMG is a rare (~100 coupes/80 cabrios) house-sold car titled "CLK DTM AMG", NOT a
  // mainstream CLK. The longest-alias rule makes "clk dtm" win over "clk". It resolves to the FULL
  // nameplate as the MODEL ("CLK DTM"), not CLK + a DTM trim: keeping DTM in the model keeps the
  // comp pool scoped to the DTM (keyword "CLK DTM" = 29 real house-sold records) and stops the
  // evidence ladder from dropping the trim and widening to mainstream CLK350s, which would price a
  // ~$500k car off $15k cars. The rare/house reality then surfaces honestly (thin/house treatment).
  { alias: "clk dtm", make: "Mercedes-Benz", model: "CLK DTM", kind: "nickname" },
  // Pre-war ("W.O." era, ~1921-1931) Bentleys are known by ENGINE DISPLACEMENT, not a model name
  // (3 Litre, 4½ Litre, 6½ Litre, 8 Litre) plus the Speed Six and the supercharged "Blower". None
  // are in vPIC/OCD by a family name, so a typed "4.5 Litre" was dropped to a "which model?" re-ask
  // (or a stray token grabbed as the model). Resolve each to its full nameplate MODEL. The Blower
  // (4½ Litre Supercharged) is a SEPARATE, far pricier model (house-sold, ~$350k-$2M+ in OCD), NOT
  // a trim of the plain 4½ Litre: keeping "Supercharged" in the model stops the ladder from mixing
  // a Blower with an ordinary 4½ Litre tourer (the CLK DTM lesson). The longest-alias rule makes
  // the Supercharged/Blower forms win over the bare displacement.
  ...[
    ["3 litre", "3 Litre"], ["3-litre", "3 Litre"], ["3 liter", "3 Litre"],
    ["4.5 litre", "4½ Litre"], ["4 1/2 litre", "4½ Litre"], ["4½ litre", "4½ Litre"], ["4.5-litre", "4½ Litre"], ["4.5 liter", "4½ Litre"], ["4.5l", "4½ Litre"],
    ["6.5 litre", "6½ Litre"], ["6 1/2 litre", "6½ Litre"], ["6½ litre", "6½ Litre"], ["6.5-litre", "6½ Litre"],
    ["8 litre", "8 Litre"], ["8-litre", "8 Litre"], ["8 liter", "8 Litre"],
    ["speed six", "Speed Six"], ["speed 6", "Speed Six"],
    // Blower = 4½ Litre Supercharged, its own model (must beat the bare "4.5 litre"). "supercharged"
    // alone maps here too: on a pre-war Bentley it is always the Blower (modern Bentleys are turbo),
    // and it catches the "4.5 Supercharged" form after the bare decimal is stripped as displacement.
    ["blower", "4½ Litre Supercharged"], ["supercharged", "4½ Litre Supercharged"],
    ["4.5 litre supercharged", "4½ Litre Supercharged"], ["4 1/2 litre supercharged", "4½ Litre Supercharged"],
    ["4½ litre supercharged", "4½ Litre Supercharged"], ["4.5 supercharged", "4½ Litre Supercharged"],
    ["4.5 litre blower", "4½ Litre Supercharged"], ["4 1/2 litre blower", "4½ Litre Supercharged"]
  ].map(([alias, model]) => ({ alias, make: "Bentley", model, kind: "nickname" })),
  ...
  // GM C/K light-duty trucks (Chevrolet + GMC). The famous squarebody / rounded-line
  // pickups: C10/C20/C30 (2WD), K10/K20/K30 (4WD) for 1960-1987, plus the 1988+ GMT400
  // successors C1500/K1500/C2500/K2500/C3500/K3500. vPIC only lists the commercial
  // C4-C8 and the obscure "U100", not these light-duty codes, so an unseeded "C10" fell
  // to the designation guard and mis-mapped onto U100 (edit distance 2). These ARE model
  // names, not trims. GMC also badged its classic half/three-quarter/one-ton line
  // C15/C25/C35 (and K15/K25/K35), seeded alongside.
  ...["c10", "c20", "c30", "k10", "k20", "k30"].flatMap(code => [
    { alias: code, make: "Chevrolet", model: code.toUpperCase(), kind: "nickname" },
    { alias: code, make: "GMC", model: code.toUpperCase(), kind: "nickname" }
  ]),
  ...["c15", "c25", "c35", "k15", "k25", "k35"].map(code => (
    { alias: code, make: "GMC", model: code.toUpperCase(), kind: "nickname" }
  )),
  ...["c1500", "k1500", "c2500", "k2500", "c3500", "k3500"].flatMap(code => [
    { alias: code, make: "Chevrolet", model: code.toUpperCase(), kind: "nickname" },
    { alias: code, make: "GMC", model: code.toUpperCase(), kind: "nickname" }
  ])
];
...
// Porsche 911 family (Sep 2026). The chassis-code generations OCD files as their own model
// ("996","997","991","992","993","964","930") plus the nameplate "911"/"912". A named 911
// trim must be RETAINED and scope the pool (rule 3.8), but the generic trim whitelist mangled
// multi-word Porsche trims when the make was inferred from a bare code ("997 Carrera S" -> "S",
// "991 GT3 RS" -> "RS", "997 Turbo" -> make dropped). PORSCHE_911_TRIMS is matched longest-first
// against the raw text so the full trim survives regardless of the resolution path.
export const PORSCHE_911_CODES = ["996", "997", "991", "992", "993", "964", "930", "912"];
export const PORSCHE_911_TRIMS = [
  // Air-cooled RS models are named by displacement + RS ("Carrera RS 2.7", "Carrera RS 3.0"):
  // listed FIRST (longest-first) so refine911 keeps the FULL "Carrera RS 2.7", never reducing it
  // to "Carrera" (which then priced a $600k-$1M RS off ordinary Carreras). The decimal survives
  // the displacement stripper via its RS lookbehind guard.
  "Carrera RS 2.7", "Carrera RS 3.0", "Carrera RS 3.8", "Carrera RSR", "Carrera RS",
  "Carrera 4 GTS", "Carrera 4S", "Carrera 4", "Carrera GTS", "Carrera S", "Carrera T", "Carrera",
  "Targa 4 GTS", "Targa 4S", "Targa 4", "Targa", "Turbo S", "Turbo",
  "GT3 RS", "GT3", "GT2 RS", "GT2", "GT4 RS", "GT4", "Speedster", "Sport Classic", "GTS", "T", "S"
];
...
export const MODEL_OWNERSHIP = [
  ...
  { model: "LC500", makes: ["Lexus"], aliases: ["lc500", "lc 500"] },
  ...
  // 911 chassis-code generations: bare-code make inference (so "997 Turbo" resolves Porsche,
  // not just "997 Carrera" via the "carrera" alias). Model normalizes to the code for pool scope.
  { model: "996", makes: ["Porsche"], aliases: ["996"] },
  { model: "997", makes: ["Porsche"], aliases: ["997"] },
  { model: "991", makes: ["Porsche"], aliases: ["991"] },
  { model: "992", makes: ["Porsche"], aliases: ["992"] },
  { model: "993", makes: ["Porsche"], aliases: ["993"] },
  { model: "964", makes: ["Porsche"], aliases: ["964"] },
  { model: "930", makes: ["Porsche"], aliases: ["930"] },
  ...
  // Classic Ferrari displacement nameplates (Sep 2026): the archive files these with the
  // sub-model in the model field ("275 GTB"), so a bare "275" was not a known standalone model
  // and fell to the numeric-typo matcher (275 -> "575 GTB", a car that does not exist). Seeding
  // the bare numbers makes them resolve exactly with the sub-model kept as the trim (275 + "GTB",
  // 250 + "GTO", 365 + "GTB/4"), which also feeds Part 2 sub-model retention and matches the
  // NO_FAMILY_WIDEN fence keys. Not 400/412 (one continuous 2+2 model, not a blend nameplate).
  { model: "250", makes: ["Ferrari"], aliases: ["250"] },
  { model: "275", makes: ["Ferrari"], aliases: ["275"] },
  { model: "330", makes: ["Ferrari"], aliases: ["330"] },
  { model: "365", makes: ["Ferrari"], aliases: ["365"] },
  { model: "512", makes: ["Ferrari"], aliases: ["512"] },
  ...
  // Miura (Sep 2026): not in the year-scoped archive taxonomy, so a bare "Miura" fell to
  // the unverified-designation fallback (uppercase "MIURA") and "Miura P400 S" was worse:
  // the "P400" spec token fuzzy-matched the "400" sub-token of the enum model
  // "350 GT / 400 GT" at 0.75 and mis-suggested that car. Curated ownership resolves Miura
  // straight to Miura (the P400 / P400 S / P400 SV spec ladder rides as the trim), before
  // any fuzzy path. "sv"/"jota"/"roadster" are Miura trims, not separate nameplates.
  { model: "Miura", makes: ["Lamborghini"], aliases: ["miura"] },
  { model: "Testarossa", makes: ["Ferrari"], aliases: ["testarossa"] },
  { model: "Defender", makes: ["Land Rover"], aliases: ["defender"] },
  // Land Rover / Range Rover family. The sub-brand naming ("Range Rover Velar")
  // structurally confused the resolver the same way Mercedes designations did:
  // bare "Velar" was unrecognized and "Range Rover Velar" collapsed into the
  // full-size "Range Rover". Each sub-model now owns its nameplate, and the
  // SPECIFIC entries are ordered BEFORE the generic "Range Rover" so
  // inferMakeFromModel (first-match wins for the model name) and textHasTerm
  // (substring match) resolve "range rover velar" to Velar, not the full-size.
  // "Range Rover" contains "Rover" (a defunct marque in the taxonomy) and was
  // misparsing to Rover Mini; the family owns it under Land Rover throughout.
  { model: "Range Rover Velar", makes: ["Land Rover"], aliases: ["velar", "range rover velar", "land rover velar"] },
  { model: "Range Rover Evoque", makes: ["Land Rover"], aliases: ["evoque", "range rover evoque", "land rover evoque"] },
  { model: "Range Rover Sport", makes: ["Land Rover"], aliases: ["range rover sport"] },
  { model: "Discovery Sport", makes: ["Land Rover"], aliases: ["discovery sport"] },
  { model: "Discovery", makes: ["Land Rover"], aliases: ["discovery", "land rover discovery", "disco", "lr3", "lr4"] },
  { model: "Range Rover", makes: ["Land Rover"], aliases: ["range rover", "rangerover"] },
  { model: "Skyline", makes: ["Nissan"], aliases: ["skyline"] },
  { model: "Carrera", makes: ["Porsche"], aliases: ["carrera"] },
  // Triumph TR-series and iconic Triumphs. "TR7" alone must never infer a wrong
  // make (a subscriber saw "1980 TR7" hijacked to Crosley on the live DB path);
  // curated ownership makes the make deterministic and DB-independent. TR-series
  // codes are unambiguously Triumph; GT6/Spitfire/Stag are its named nameplates.
  { model: "TR2", makes: ["Triumph"], aliases: ["tr2"] },
  { model: "TR3", makes: ["Triumph"], aliases: ["tr3", "tr3a", "tr3b"] },
  { model: "TR4", makes: ["Triumph"], aliases: ["tr4", "tr4a"] },
  { model: "TR5", makes: ["Triumph"], aliases: ["tr5"] },
  { model: "TR6", makes: ["Triumph"], aliases: ["tr6"] },
  { model: "TR7", makes: ["Triumph"], aliases: ["tr7"] },
  { model: "TR8", makes: ["Triumph"], aliases: ["tr8"] },
  { model: "TR250", makes: ["Triumph"], aliases: ["tr250"] },
  { model: "GT6", makes: ["Triumph"], aliases: ["gt6"] },
  { model: "Spitfire", makes: ["Triumph"], aliases: ["spitfire"] },
  { model: "Stag", makes: ["Triumph"], aliases: ["triumph stag"] }
];
...
export const PRODUCTION_RULES = [
  ...
  // BMW model lines, so the year filter can drop anachronisms from the curated chip pool (an
  // X5/Z4 must never be offered for a 1990 BMW). Core sedans/coupes span their real production;
  // the SUV/roadster/M-derivative lines carry their launch year.
  { make: "BMW", model: "3-Series", aliases: ["3 series", "3-series", "3series"], ranges: [[1975, 2026]] },
  { make: "BMW", model: "5-Series", aliases: ["5 series", "5-series", "5series"], ranges: [[1972, 2026]] },
  { make: "BMW", model: "6-Series", aliases: ["6 series", "6-series", "6series"], ranges: [[1976, 1989], [2004, 2026]] },
  { make: "BMW", model: "7-Series", aliases: ["7 series", "7-series", "7series"], ranges: [[1977, 2026]] },
  { make: "BMW", model: "8-Series", aliases: ["8 series", "8-series", "8series"], ranges: [[1989, 1999], [2018, 2026]] },
  { make: "BMW", model: "2-Series", aliases: ["2 series", "2-series", "2series"], ranges: [[2014, 2026]] },
  { make: "BMW", model: "4-Series", aliases: ["4 series", "4-series", "4series"], ranges: [[2013, 2026]] },
  { make: "BMW", model: "1-Series", aliases: ["1 series", "1-series", "1series"], ranges: [[2004, 2026]] },
  { make: "BMW", model: "M2", aliases: ["m2"], ranges: [[2015, 2026]] },
  { make: "BMW", model: "M4", aliases: ["m4"], ranges: [[2014, 2026]] },
  { make: "BMW", model: "M8", aliases: ["m8"], ranges: [[2019, 2026]] },
  { make: "BMW", model: "X1", aliases: ["x1"], ranges: [[2009, 2026]] },
  { make: "BMW", model: "X3", aliases: ["x3"], ranges: [[2003, 2026]] },
  { make: "BMW", model: "X3 M", aliases: ["x3 m", "x3m"], ranges: [[2019, 2026]] },
  { make: "BMW", model: "X5", aliases: ["x5"], ranges: [[1999, 2026]] },
  { make: "BMW", model: "X5 M", aliases: ["x5 m", "x5m"], ranges: [[2009, 2026]] },
  { make: "BMW", model: "X6", aliases: ["x6"], ranges: [[2008, 2026]] },
  { make: "BMW", model: "Z3", aliases: ["z3"], ranges: [[1995, 2002]] },
  { make: "BMW", model: "Z4", aliases: ["z4"], ranges: [[2002, 2026]] },
  { make: "BMW", model: "Z8", aliases: ["z8"], ranges: [[1999, 2003]] },
  { make: "BMW", model: "i3", aliases: ["i3"], ranges: [[2013, 2022]] },
  { make: "BMW", model: "i8", aliases: ["i8"], ranges: [[2014, 2020]] },
  ...
  // GM C/K light-duty trucks: production-year validity for the seeded models above.
  // C10/C20/C30 + K10/K20/K30 (and GMC C15/C25/C35) span 1960-1987; the GMT400
  // successors C1500/K1500/C2500/K2500/C3500/K3500 span 1988-2002.
  ...["C10", "C20", "C30", "K10", "K20", "K30"].flatMap(code => [
    { make: "Chevrolet", model: code, aliases: [code.toLowerCase()], ranges: [[1960, 1987]] },
    { make: "GMC", model: code, aliases: [code.toLowerCase()], ranges: [[1960, 1987]] }
  ]),
  ...["C15", "C25", "C35", "K15", "K25", "K35"].map(code => (
    { make: "GMC", model: code, aliases: [code.toLowerCase()], ranges: [[1967, 1987]] }
  )),
  ...["C1500", "K1500", "C2500", "K2500", "C3500", "K3500"].flatMap(code => [
    { make: "Chevrolet", model: code, aliases: [code.toLowerCase()], ranges: [[1988, 2002]] },
    { make: "GMC", model: code, aliases: [code.toLowerCase()], ranges: [[1988, 2002]] }
  ])
];
...
export const TRIM_VOCABULARY = new Set([
  ...
  // "series" is a model-FAMILY word (3 Series, 5 Series, M Series), never a standalone
  // trim. Whitelisting it let a family fragment leak into the trim ("325i" resolving with
  // a leftover "3 Series" rendered trim "Series"); it is intentionally NOT a trim token.
  "amg","black","edition","anniversary","limited","premium","plus","performance","executive","designo","dtm",
  ...
  // Real factory special-series names that are pure WORDS (no digit), so the alphanumeric fallback
  // in trimTokenAllowed can't save them - they must be enumerated or the whitelist drops them into
  // dirtyTokens (a year-present "2014 Ferrari 458 Speciale" lost "Speciale" and read as a bare 458).
  "speciale","stradale","competizione","monza","aperta","corsa","italia","lusso","superamerica","superfast",
  "california","portofino","roma","purosangue","daytonasp3",
