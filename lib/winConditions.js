...
export function findWinCondition(vehicle) {
  ...
  // Match the full model or its family (first token). vehicle.model is already
  // trim-separated by the resolver ("Camaro SS" -> model "Camaro"), so this is
  // mainly defensive; the family is taken from the RAW model's first word since
  // `model` here is already normalized (no separators to split on).
  const family = norm(String(vehicle.model || "").split(/\s+/)[0]);
  return WIN_CONDITIONS
    .filter(w => norm(w.make) === make && (norm(w.model) === model || norm(w.model) === family))
    .filter(w => {
