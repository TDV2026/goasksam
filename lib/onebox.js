...
async function assessClassEra(spec, env, diag) {
  ...
  return { isClass: true, era: era.label, make: spec.make, model: spec.model || null, receipts, totalN: pool.length, medianHammer: htMedOf(hs), lowHammer: Math.min(...hs), highHammer: Math.max(...hs) };
}
