...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  if (recVia === "title") {
    ...
    const rqBase = `sales_archive?select=${cols}&make=ilike.${encodeURIComponent("*" + spec.make + "*")}&model=ilike.${encodeURIComponent("*" + flexPattern(recModelToken) + "*")}&sale_price=not.is.null${trimQ}`;
    const all = [];
    ...
    if (rawRows.length < 5) {
      for (let off = 0; off < 2000; off += 1000) {
        const pr = await supabaseSelect(env, `sales_archive?select=${cols}&make=ilike.${encodeURIComponent("*" + spec.make + "*")}&listing_title=ilike.${encodeURIComponent("*" + flexPattern(recModelToken) + "*")}&sale_price=not.is.null${trimQ}&order=id.asc&limit=1000&offset=${off}`) || [];
...
const RACE_TITLE_RE = /\bdtm\b|\bfia\b|\bimsa\b|\bnascar\b|\bgt3\s?cup\b|\bgt4\s?clubsport\b|clubsport\b|\bcup\s?car\b|\b(turbo|carrera)\s?cup\b|group\s?[abc45]\b|\brace\s?car\b|\bracing\b|works\s?rally|rally\s?car|\bgrp\.?\s?[abc45]\b|competition\s?(saloon|coupe|car)/i;
