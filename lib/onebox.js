...
async function fetchQualifying(spec, sinceIso, env, diag) {
  // Read the COMPREHENSIVE sales_archive (nightly ingest across every platform), NOT the
  // search-populated vehicle_market_records: vmr only holds cars someone recently searched or
  // the warm touched, so a less-searched but heavily-traded car (E30 M3: 4 in vmr vs 52 in the
  // archive) collapsed to a one-sale pool. sales_archive is sold-only, so no auction_status
  // filter. Columns are aliased to the vmr names the downstream code already uses (price,
  // auction_end_date, source, raw_title), so nothing below changes.
  ...
  let q = `sales_archive?select=${cols}` +
    ...
    `&order=sale_price.desc&limit=1000`;
