...
function shapeCards(rows) {
  return rows.map(r => ({
    ...
    title: titleOf(r).replace(/[–—]/g, "-").replace(/\s+/g, " ").trim(), image: r.image || null, hollow: !!r._hollow
  }));
}
// The round-3 result model, run on a chosen ladder pool. Returns a "result" or a refusal.
function buildResult(chosen, base, iso, spec) {
  const rows = [...chosen.rows].filter(r => Number.isFinite(r._usd) && r._usd > 0)
    .sort((a, b) => String(b.auction_end_date || "").localeCompare(String(a.auction_end_date || "")));
  const N = rows.length, windowLabel = chosen.windowLabel;
  const b = { ...base, count: N, windowLabel };
  const THIN_FLOOR = 4;
  if (chosen.step === "thin" || N < THIN_FLOOR) {
    return { ...b, tier: "refusal", ladderStep: "thin", refusal: { kind: "thin", model: spec.subject, n: N, variants: [], yearSpanPhrase: null }, cards: shapeCards(rows) };
  }
  const p75 = percentile(rows.map(r => r._usd), 0.75), p25 = percentile(rows.map(r => r._usd), 0.25);
  for (const r of rows) r._hollow = r._usd > p75 * 1.5 || r._usd < p25 * 0.4;
  const solid = rows.filter(r => !r._hollow), hollow = rows.filter(r => r._hollow);
  const su = solid.map(r => r._usd);
  const ratio = solid.length >= 6 ? percentile(su, 0.9) / Math.max(1, percentile(su, 0.1)) : (su.length ? Math.max(...su) / Math.max(1, Math.min(...su)) : 1);
  const yrs = solid.map(r => Number(r.year)).filter(Boolean), yspan = yrs.length ? Math.max(...yrs) - Math.min(...yrs) : 0;
  const multiMarket = yspan >= 15 && variantTokens(solid).length >= 2 && ratio > 2.5;
  if (solid.length < THIN_FLOOR || ratio > 6 || multiMarket) {
    const ay = rows.map(r => Number(r.year)).filter(Boolean), ys = ay.length ? Math.max(...ay) - Math.min(...ay) : 0;
    const yearSpanPhrase = ys >= 20 ? "more than twenty years" : ys >= 10 ? "more than a decade" : ys > 0 ? `${ys} years` : null;
    return { ...b, tier: "refusal", ladderStep: "varied", refusal: { kind: "varied", model: spec.subject, n: N, variants: variantTokens(rows).slice(0, 6), yearSpanPhrase }, cards: shapeCards(rows) };
  }
  const span = [r500(Math.min(...su)), r500(Math.max(...su))];
  const cluster = solid.length >= 8 ? [r500(percentile(su, 0.25)), r500(percentile(su, 0.75))] : null;
  let recency = null;
  if (N >= 12) {
    const winIso = iso(365).slice(0, 10);
    const in12 = rows.filter(r => String(r.auction_end_date || "").slice(0, 10) >= winIso);
    if (in12.length >= 3) { const rs = solid.slice(0, Math.min(5, in12.length)).map(r => r._usd); if (rs.length >= 2) recency = { n12: in12.length, total: N, k: rs.length, lo: Math.min(...rs), hi: Math.max(...rs) }; }
  }
  const platforms = foldPlatforms(solid), named = platforms.filter(p => p[0] !== "others");
  ...
  return {
    ...
    haloAside: (chosen.aside && chosen.aside.length) ? { count: chosen.aside.length, label: spec.trim || "the halo variant" } : null,
...
export async function runOneBox(vehicle, generation, searchText, env) {
  ...
  // Fetch the whole FAMILY (no trim scoping): partition locally so one fetch serves every step.
  // Nulling the perf gates makes fetchQualifying return make+model, stock/body/parts filtered,
  // halos INCLUDED (the direction rule sets them aside here, never at fetch time).
  ...
  const fam24 = priceUp(online(await fetchQualifying(famSpec, iso(730), env, diag)));
  const platformsCount = new Set(fam24.map(r => platformName(r.source)).filter(Boolean)).size;
  const base = { resolvedCar, resolvedSpec: spec.resolvedSpec, platformsCount, dedup: diag, vehicle: { make: spec.make, model: spec.model, trim: spec.trim, year: spec.year } };
  const part = rows => ({
    trim: trimRe ? rows.filter(r => trimRe.test(titleOf(r))) : [],
    halo: haloRe ? rows.filter(r => haloRe.test(titleOf(r))) : [],
    exHalo: haloRe ? rows.filter(r => !haloRe.test(titleOf(r))) : rows
  });

  const EXACT_MIN = 5;
  const p24 = part(fam24);
  let chosen;
  if (trimName) {
    if (presentable(p24.trim, EXACT_MIN)) {
      chosen = { rows: p24.trim, step: "exact_trim", windowLabel: "past 2 years", widening: null };
    } else {
      const fam5 = priceUp(online(await fetchQualifying(famSpec, iso(1825), env, diag)));
      const p5 = part(fam5);
      if (presentable(p5.trim, EXACT_MIN)) chosen = { rows: p5.trim, step: "exact_trim_wide", windowLabel: "past 5 years", widening: `Only a few ${trimName} sold in the last two years, so this reads the past five.` };
      else if (presentable(p24.exHalo, 4)) chosen = { rows: p24.exHalo, step: "family", windowLabel: "past 2 years", aside: trimIsHalo ? p24.halo : null, widening: familyWideningSentence(trimName, spec.model, trimIsHalo) };
      else if (presentable(p5.exHalo, 4)) chosen = { rows: p5.exHalo, step: "family_wide", windowLabel: "past 5 years", aside: trimIsHalo ? p5.halo : null, widening: familyWideningSentence(trimName, spec.model, trimIsHalo) };
      else chosen = { rows: (p5.trim.length ? p5.trim : (p5.exHalo.length ? p5.exHalo : fam5)), step: "thin", windowLabel: "past 5 years", widening: null };
    }
  } else {
    // Base / unspecified: family with the halo ALWAYS set aside; never widen up into it.
    chosen = { rows: p24.exHalo, step: "base", windowLabel: "past 2 years", aside: (p24.halo && p24.halo.length) ? p24.halo : null, widening: null };
  }
  return buildResult(chosen, base, iso, spec);
}
