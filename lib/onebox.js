...
async function assessHouseTier(famSpec, trimName, trimRe, iso, env, diag) {
  const raw = await fetchQualifying({ ...famSpec, houseTier: true, titleContains: trimName || undefined }, iso(HT_WINDOW_DAYS), env, diag);
  ...
  const isHouseTier = onlineVolumeN < HT_ONLINE_MIN && house.length >= 1 && pool.length >= 2;
