...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const base = `sales_archive?select=${cols}` +
    // RECORD path uses make=eq so the (make, sale_price DESC) index can serve the price sort; make
    // ILIKE cannot use that btree. Non-record keeps ILIKE for case-insensitive matching.
