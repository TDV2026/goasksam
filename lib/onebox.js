...
import { dedupBySaleIdentity, hammerUsd, priceDisplay, mileageInfo, isHouseSource, toUsd, sourceSlugOf } from "./_houseComps.js";
...
function performanceBadge(make, model, trim, context) {
  ...
  if (/mercedes|benz/i.test(make)) {
    ...
    if (m) { const b = (m[1].replace(/AMG\s*/i, "") + m[2]).toUpperCase(); const v = /black\s?series/i.test(vhay) ? "Black Series" : (/\bS\b/.test(hay.replace(new RegExp(b, "i"), "")) ? "S" : null); return { badge: b, re: new RegExp(`\\b${b}\\b`, "i"), variant: v }; }
...
function isQualifying(row, spec) {
  ...
  // (its cards show one); the trigger separately re-imposes the photo gate on the ONLINE count so
  // "thin online market" still means thin for VOLUME mode. Everything else (price/parts/
  // memorabilia/mods/trim/body) still applies.
  if (!row.image && !spec.houseTier) return false;
  ...
  // Memorabilia / replica / tribute / scale-model guard (Sep 2026): not a car, must never enter
  // the span/cluster - on a thin pool the junk sits at the floor (250 GTO wall art, Corvette art).
  if (isMemorabilia(row.raw_title || row.rtitle)) return false;
...
function buildSpec(vehicle, generation, searchText) {
  ...
  if (spec.year) {
    const bg = generationsForModel(spec.make, spec.model).find(g => spec.year >= g.yearStart && spec.year <= g.yearEnd);
    if (bg) { spec.yearMin = bg.yearStart; spec.yearMax = bg.yearEnd; spec.genCode = bg.code; }
  }
  return relabel(spec);
}
...
async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const cols = "price:sale_price,auction_end_date:sale_date,source:platform,raw_title:listing_title,year,vin_norm," +
...
const HALO_PATTERNS = [
  ...
  { make: /^bmw$/i, re: /\b(csl|\bcs\b|sport\s?evolution|\bevo\b|cecotto|\bgts\b)\b/i },
...
const r4Span = a => {
  ...
  let lo = 0;
  if (v.length >= 20) {
    const gaps = []; for (let i = 1; i < v.length; i++) gaps.push(v[i] - v[i - 1]);
    const med = [...gaps].sort((x, y) => x - y)[Math.floor(gaps.length / 2)] || 0;
    if (med > 0 && (v[1] - v[0]) > 3 * med) lo = 1;   // lone low outlier -> trim it
  }
  return [r500(v[lo]), r500(v[v.length - 1])];
};
...
function pickRepresentative(solid, exactSale, center, driver) {
  const comps = solid.filter(r => !sameAsSubject(r, exactSale));
  if (!comps.length) return null;
  const byPrice = [...comps].sort((a, b) => a._usd - b._usd);
  const subjMi = exactSale ? (Number(String(exactSale.mileage == null ? "" : exactSale.mileage).replace(/[^\d]/g, "")) || 0) : 0;
  const mid = center ? (center[0] + center[1]) / 2 : byPrice[Math.floor(byPrice.length / 2)]._usd;
  let closest;
  ...
  else closest = byPrice.slice().sort((a, b) => Math.abs(a._usd - mid) - Math.abs(b._usd - mid))[0];
  ...
  const above = comps.filter(r => r !== closest && r._usd > cUsd + step);
  const below = comps.filter(r => r !== closest && r._usd < cUsd - step);
...
// ---- HOUSE-TIER MODE assessment (Sep 2026) --------------------------------------------
// A rare car whose ONLINE approved market is too thin to stand on volume rules but which DOES
// trade at the auction houses. The trigger is DATA-DERIVED (not a marque list): over a 36-month
// model-scoped pool (memorabilia + UK already excluded upstream), fire when the online approved
// count < HT_ONLINE_MIN and at least one house sale is present. Houses are admitted here; hammer
// is the compute basis (premium backed out), all-in is shown on the receipt.
...
function htReceipt(r) {
  ...
  return {
    ...
    year: r.year || null,
    title: String(r.raw_title || r.rtitle || ""),
...
async function assessHouseTier(famSpec, trimName, trimRe, iso, env, diag) {
  ...
  // Trigger uses the VOLUME-capable online count (photo-gated, like volume mode): "thin online
  // market" means too few sales for volume mode to stand. Receipts below stay photo-relaxed.
  const onlineVolumeN = online.filter(r => r.image).length;
  // Fire on a single house receipt: for the rarest cars (a 250 GTO trades maybe once in 36 months)
  // that one sale IS the answer, rendered as a single-sale hero (design), never a refusal. RULE 1:
  // individual sales are receipts at any count. The online<15 gate keeps volume cars out.
  const isHouseTier = onlineVolumeN < HT_ONLINE_MIN && house.length >= 1;
  const meta = { isHouseTier, onlineN: onlineVolumeN, onlineReceiptsN: online.length, houseN: house.length, totalN: pool.length, windowMonths: 36 };
  if (!isHouseTier) return meta;
  ...
  // Intake markers: a marker separates the pool only when at least 2 receipts carry it AND at
  // least 2 do not (a genuine split, per the design). These drive the ASK-FIRST chips.
  // A marker only earns an intake question when it (a) splits the pool >=2 each side AND (b)
  // genuinely FORKS THE PRICE - the with-group and without-group medians differ materially. A
  // marker every car carries (all 275 GTBs are Scaglietti-coachbuilt) passes the count gate but
  // not the price gate, so it never wastes an ask; a real fork (Daytona Competizione vs road car)
  // does. Without a value fork we skip straight to the sale, per "no invented pattern".
  const medOf = arr => { if (!arr.length) return null; const s = arr.slice().sort((a, b) => a - b); const n = s.length; return n % 2 ? s[(n - 1) / 2] : (s[n / 2 - 1] + s[n / 2]) / 2; };
  const N = receipts.length;
  const counts = {};
  for (const rc of receipts) for (const m of rc.markers) counts[m.key] = (counts[m.key] || 0) + 1;
  const intakeMarkers = Object.entries(counts)
    .filter(([, c]) => c >= 2 && (N - c) >= 2)
    .map(([k, c]) => {
      const withMed = medOf(receipts.filter(r => r.markers.some(m => m.key === k)).map(r => r.hammer));
      const withoutMed = medOf(receipts.filter(r => !r.markers.some(m => m.key === k)).map(r => r.hammer));
      const sep = (withMed && withoutMed) ? Math.max(withMed, withoutMed) / Math.min(withMed, withoutMed) : 1;
      return { key: k, label: markerLabel(k), withN: c, withoutN: N - c, withMed, withoutMed, sep };
    })
    .filter(x => x.sep >= 1.4)
    .sort((a, z) => z.sep - a.sep);

  const sortedH = receipts.map(r => r.hammer).sort((a, z) => a - z);
  const medianHammer = sortedH.length ? (sortedH.length % 2 ? sortedH[(sortedH.length - 1) / 2] : Math.round((sortedH[sortedH.length / 2 - 1] + sortedH[sortedH.length / 2]) / 2)) : null;
  return { ...meta, receipts, pairs, pairsCount: pairs.length, pairPctEligible: pairs.length >= 3, intakeMarkers, medianHammer };
}
...
function buildResult(chosen, base, iso, spec, refine) {
  ...
  // The earned question (#3, Sep 2026): transmission OUTRANKS mileage when its split is both
  // gated (5+ manual AND 5+ auto) AND its median-price gap is larger than the mileage split's.
  // Otherwise mileage (the common case). Never asks a field the input already gave.
  let earned = null;
  ...
  const mileageOK = !(refined && refine.miMin != null) && buckets && buckets.length >= 2;
  const txOK = !txGiven && !(refined && refine.tx) && txGate;
  if (txOK && txGate.gap != null && (mileageGap == null || txGate.gap > mileageGap)) earned = { kind: "transmission", labels: txGate };
  else if (mileageOK) earned = { kind: "mileage", buckets };
  else if (txOK) earned = { kind: "transmission", labels: txGate };
  ...
  return {
    ...
    representative: pickRepresentative(solid, base.exactSale, cluster || span, driver),
...
export async function runOneBox(vehicle, generation, searchText, env, refine) {
  ...
  const haloRe = (spec.badge && /mercedes|benz/i.test(spec.make))
    ? /\bblack\s?series\b/i
  ...
  // ---- HOUSE-TIER MODE branch: rare car, thin online market, trades at the houses. Fires on
  // data (online < 15 / 36mo AND a house sale present), never a marque list. Uses the exact
  // resolved trim scope; houses are admitted as evidence. See assessHouseTier.
  const houseTier = await assessHouseTier(famSpec, trimName, trimRe, iso, env, diag);
  base.htMeta = houseTier; // diagnostics: carried on the non-firing result/refusal too
  if (houseTier && houseTier.isHouseTier) {
    return { tier: "house_tier", ...base, houseTier, refine };
  }
