...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const base = `sales_archive?select=${cols}` +
    ...
    // RECORD path: omit the (all-time, no-op) sale_date filter so the planner can serve the query
    // from the (make, sale_price DESC) index instead of scanning by date then sorting. Non-record
    // queries keep the date filter (it is selective and index-backed for a recent window).
    (spec.recordSort ? "" : `&sale_date=gte.${sinceIso.slice(0, 10)}`) +
