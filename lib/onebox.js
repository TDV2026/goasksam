...
// Same-spec establishment (WS1.1). "Same spec" may be claimed only when the shown comps
// are actually the same variant. When variants are MIXED and the normalized-value spread
// is wide, the pool is not one market: REFUSE and ask for the trim (a $376k likely-Scuderia
// next to a $91k base F430 must never render "same spec"). When mixed but tight, render but
// drop the equality claim (the explanation says "variant not stated").
const SAME_SPEC_SPREAD_MAX = 2.0;   // >2x normalized-value spread among mixed variants -> refuse
function analyzeSameSpec(cards) {
  const sigs = cards.map(c => c.variantSig || "");
  const distinct = new Set(sigs.filter(Boolean));
  // Mixed if there are 2+ distinct non-empty signatures, OR some cards name a variant and
  // others do not (an empty sig next to a "scuderia" sig is a base-vs-halo mix).
  const variantMixed = distinct.size >= 2 || (distinct.size === 1 && sigs.some(s => !s) && sigs.some(s => s));
  ...
  return { sameSpec: !variantMixed, variantMixed, spread, refuse: variantMixed && spread > SAME_SPEC_SPREAD_MAX };
}
...
// The VARIANT signature of a listing: the meaningful trim words in its title after the
// year, make, model, body style, colour and generic noise are removed (Carrera, Turbo,
// Scuderia, GTS...). Used to tell whether the shown comps are actually the SAME variant
// before claiming "same spec" (a base F430 and an F430 Scuderia are not same-spec).
const VARIANT_NOISE = new Set([
  "coupe", "coupé", "coup", "targa", "cabriolet", "cabrio", "convertible", "roadster", "spyder", "spider",
  "sedan", "saloon", "berlina", "wagon", "estate", "avant", "touring", "hardtop", "fastback", "notchback", "berlinetta",
  "the", "and", "with", "miles", "mile", "km", "edition", "series", "model", "car", "auto", "example",
  "black", "white", "silver", "gray", "grey", "red", "blue", "green", "yellow", "orange", "brown", "gold", "beige", "purple", "rosso", "nero", "bianco", "argento", "grigio"
]);
function variantSignature(row, spec) {
  const title = String(row.rtitle || row.raw_title || "").toLowerCase();
  const makeToks = new Set(normalize(spec.make).split(/\s+/).filter(Boolean));
  const modelToks = new Set(normalize(spec.model).split(/\s+/).filter(Boolean));
  // also strip a leading-letter model form ("f430" when the model is "430" or vice versa)
  for (const m of [...modelToks]) { modelToks.add("f" + m); modelToks.add(m.replace(/^f/, "")); }
  const toks = title.replace(/[^a-z0-9]+/g, " ").split(/\s+/).filter(Boolean);
  const sig = toks.filter(t =>
    !/^\d{4}$/.test(t) && !/^\d+k?$/.test(t) && t.length >= 2 &&
    !makeToks.has(t) && !modelToks.has(t) && !VARIANT_NOISE.has(t));
  return [...new Set(sig)].sort().join(" ");
}
...
function toCard(row, rank, spec, subject) {
  ...
  return {
    ...
    variantSig: variantSignature(row, spec),    // for the same-spec / variant-mixed check
    spec: spec.cardSpec,
...
export async function runOneBox(vehicle, generation, searchText, env) {
  ...
  if (n >= 3) {
    // The three shown are still the high / median-value / low of the FULL pool (so the
    // spread is honest), but each is now LABELED by relevance, not by that position.
    const midIdx = Math.ceil(n / 2) - 1; // lower-middle for even counts
    const { cards, refuse } = labelCards([pool[n - 1], pool[midIdx], pool[0]], spec, subject);
    // Variant-mixed at a wide spread: not one market -> ask for the trim instead of
    // rendering cards that would falsely read as same-spec.
    if (refuse) return { ...base, count: undefined, tier: "underspecified", samLine: SAM_LINE.underspecified, trustLine: null, cards: [] };
    return { ...base, tier: "three", trustLine: trustLine(n), samLine: SAM_LINE.three, cards };
  }
  if (n === 2) {
    const { cards, refuse } = labelCards([pool[1], pool[0]], spec, subject);
    if (refuse) return { ...base, count: undefined, tier: "underspecified", samLine: SAM_LINE.underspecified, trustLine: null, cards: [] };
    return { ...base, tier: "two", trustLine: trustLine(n), samLine: SAM_LINE.two, cards };
  }
  if (n === 1) {
    const one = toCard(pool[0], "Comparable sale", spec, subject); one.closest = true;
    one.explanation = explainComp(pool[0], subject, spec, true); // a single comp is its own spec, no mix
    return { ...base, tier: "one", trustLine: trustLine(n), samLine: SAM_LINE.one, cards: [one] };
  }
...
export async function runOneBoxModelChoice(vehicle, env) {
  ...
  let q = `vehicle_market_records?select=model&make=ilike.${encodeURIComponent(make)}&auction_status=ilike.sold&model=not.is.null&limit=5000`;
  if (year) q += `&year=gte.${year - 3}&year=lte.${year + 3}`;
  const rows = (await supabaseSelect(env, q)) || [];
  const counts = new Map();
  for (const r of rows) {
    const m = String(r.model || "").trim();
    if (m && !isBodyStyleEnumModelName(m)) counts.set(m, (counts.get(m) || 0) + 1);
  }
  // Merge same-family duplicate model names (WS1.3) BEFORE ranking, so "430"/"F430" and
  // "MX-5"/"MX5 Miata" collapse to one chip with the combined count.
  const merged = mergeModelChips([...counts.entries()]);
  const models = merged.filter(e => e[1] >= 2).sort((a, b) => b[1] - a[1]).map(e => e[0]);
