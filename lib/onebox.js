...
async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  // Space/hyphen spelling variants (Part 3): the archive stores alnum model codes both ways
  // ("300SL" and "300 SL", "365GTB" and "365 GTB"), so a single ILIKE misses half the pool
  // depending on how the seller typed it (300SL -> 39 rows, 300 SL -> 23). Match all forms via an
  // OR. Additive and safe: variants only ADD rows that literally contain that spelling, and a
  // pure-digit or single-form token (275, 911, GTB) yields ONE variant, so its query is unchanged.
  const orIlike = (col, t) => {
    const s = String(t || "").trim();
    const set = new Set([s]);
    set.add(s.replace(/[\s-]+/g, ""));
    set.add(s.replace(/([0-9])([A-Za-z])/g, "$1 $2").replace(/([A-Za-z])([0-9])/g, "$1 $2"));
    const vs = [...set].filter(Boolean);
    return vs.length <= 1
      ? `${col}=ilike.${encodeURIComponent("*" + s + "*")}`
      : `or=(${vs.map(v => `${col}.ilike.` + encodeURIComponent("*" + v + "*")).join(",")})`;
  };
  const modelQ = t => `${base}&${orIlike("model", t)}`;
  const titleQ = t => `${base}&${orIlike("listing_title", t)}`;
  const t0 = Date.now();
  let rawRows;
...
async function assessClassEra(spec, env, diag) {
  ...
  return { isClass: true, era: era.label, make: spec.make, model: spec.model || null, receipts, totalN: pool.length, medianHammer: htMedOf(hs), lowHammer: band[0], highHammer: band[1], trueLow: Math.min(...hs), trueHigh: Math.max(...hs) };
}
...
export async function runOneBox(vehicle, generation, searchText, env, refine) {
  ...
  if (thin && thin.isThin) {
    return { tier: "thin", ...base, thin, refine };
  }
  ...
  if (thin && thin.totalN === 0 && spec.year) {
    ...
    if (classEra && classEra.isClass && classEra.receipts.length) {
      return { tier: "class_era", ...base, classEra, refine };
    }
  }
