...
// card highlight, replacing the old middle-of-market highlight).
function labelCards(rows, spec, subject) {
  ...
  cards.forEach((c, i) => { c.rank = labels[i]; c.closest = labels[i] === "Closest match"; });
  return cards;
}
...
export async function runOneBox(vehicle, generation, searchText, env) {
  ...
  if (n >= 3) {
    ...
    const cards = labelCards([pool[n - 1], pool[midIdx], pool[0]], spec, subject);
    return { ...base, tier: "three", trustLine: trustLine(n), samLine: SAM_LINE.three, cards };
  }
  if (n === 2) {
    const cards = labelCards([pool[1], pool[0]], spec, subject);
...
export async function runOneBoxModelChoice(vehicle, env) {
  ...
  const models = [...counts.entries()].filter(e => e[1] >= 2).sort((a, b) => b[1] - a[1]).map(e => e[0]);
