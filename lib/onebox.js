...
async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  if (scope.byTitle) q += `&listing_title=ilike.${encodeURIComponent("*" + scope.byTitle + "*")}`;
...
export async function runOneBox(vehicle, generation, searchText, env) {
  ...
  // Round-3 quotes a 24-month market ("past 2 years"): a richer pool for the span/cluster,
  // and it makes the last-twelve-months recency sentence a meaningful SUBSET rather than the
  // whole pool. One fetch, hard ceiling at 24 months (never widen further).
  let windowLabel = "past 2 years";
  let pool = await fetchQualifying(spec, iso(730), env, diag);
  ...
  // Distinct platforms in the final pool (telemetry only). Real breadth, not the shown cards.
  const platformsCount = new Set(pool.map(r => platformName(r.source)).filter(Boolean)).size;

  // ---- Round-3 result model (mirrors /onebox-preview, the locked design of record) ----
  // Every statistic is the sale OUTCOME (what the buyer actually paid), USD-normalized; the
  // ranking `value` (implied hammer) is not used for display. Cards are most-recent first.
  const priced = pool.filter(r => { r._usd = outcomeUsd(r); return Number.isFinite(r._usd) && r._usd > 0; });
  priced.sort((a, b) => String(b.auction_end_date || "").localeCompare(String(a.auction_end_date || "")));
  const N = priced.length;
  const base = { resolvedCar, resolvedSpec: spec.resolvedSpec, count: N, platformsCount, windowLabel, dedup: diag, vehicle: { make: spec.make, model: spec.model, trim: spec.trim, year: spec.year } };
  const shapeCards = rows => rows.map(r => ({
    price: r._usd, mileageText: miText(r), platform: platNamed(r.source), platformSlug: platSlug(r.source),
    date: r.auction_end_date || null, month: monthYear(r.auction_end_date),
    title: String(r.raw_title || r.rtitle || "").replace(/[–—]/g, "-").replace(/\s+/g, " ").trim(),
    image: r.image || null, hollow: !!r._hollow
  }));

  // THIN: too few real sales to show any honest range -> refusal that shows what there is.
  const THIN_FLOOR = 4;
  if (N < THIN_FLOOR) {
    return { ...base, tier: "refusal", refusal: { kind: "thin", model: spec.subject, n: N, variants: [], yearSpanPhrase: null }, cards: shapeCards(priced) };
  }

  // Set-aside outliers: special/collector cars above the driver market (and salvage below it).
  // Fence the FULL pool's p75/p25; the SOLID pool is the coherent market we quote a range on.
  const ASIDE_HIGH = 1.5, ASIDE_LOW = 0.4;
  const p75all = percentile(priced.map(r => r._usd), 0.75), p25all = percentile(priced.map(r => r._usd), 0.25);
  for (const r of priced) r._hollow = r._usd > p75all * ASIDE_HIGH || r._usd < p25all * ASIDE_LOW;
  const solid = priced.filter(r => !r._hollow), hollow = priced.filter(r => r._hollow);
  const solidUsd = solid.map(r => r._usd);

  // TOO VARIED: even after setting aside outliers, the core still spans genuinely different
  // markets -> refusal with the templated variant reason + real cards. Two signals:
  //  (a) a huge robust price ratio (p90/p10), OR
  //  (b) a MULTI-GENERATION nameplate: a wide model-year span AND multiple distinct variant
  //      names AND a real price spread. This catches a year-unbounded query like a bare
  //      "Esprit" (S1 1977 to twin-turbo V8 2004) that a single range would misrepresent,
  //      while a generation-scoped model (a 997, an E30 M3) stays narrow and presents.
  const IMPLAUSIBLE = 6;
  const ratio = solid.length >= 6 ? (percentile(solidUsd, 0.9) / Math.max(1, percentile(solidUsd, 0.1))) : (solidUsd.length ? Math.max(...solidUsd) / Math.max(1, Math.min(...solidUsd)) : 1);
  const solidYears = solid.map(r => Number(r.year)).filter(Boolean);
  const yearSpan = solidYears.length ? Math.max(...solidYears) - Math.min(...solidYears) : 0;
  const solidVariants = variantTokens(solid);
  const multiMarket = yearSpan >= 15 && solidVariants.length >= 2 && ratio > 2.5;
  if (solid.length < THIN_FLOOR || ratio > IMPLAUSIBLE || multiMarket) {
    const years = priced.map(r => Number(r.year)).filter(Boolean);
    const yspan = years.length ? Math.max(...years) - Math.min(...years) : 0;
    const yearSpanPhrase = yspan >= 20 ? "more than twenty years" : yspan >= 10 ? "more than a decade" : yspan > 0 ? `${yspan} years` : null;
    return { ...base, tier: "refusal", refusal: { kind: "varied", model: spec.subject, n: N, variants: variantTokens(priced).slice(0, 6), yearSpanPhrase }, cards: shapeCards(priced) };
  }

  // PRESENT: a coherent single market. Span always; cluster at 8+; recency at 12+ and 3+ recent.
  const span = [r500(Math.min(...solidUsd)), r500(Math.max(...solidUsd))];
  const cluster = solid.length >= 8 ? [r500(percentile(solidUsd, 0.25)), r500(percentile(solidUsd, 0.75))] : null;

  let recency = null;
  if (N >= 12) {
    const winIso = iso(365).slice(0, 10);
    const in12 = priced.filter(r => String(r.auction_end_date || "").slice(0, 10) >= winIso);
    if (in12.length >= 3) {
      const recentSolid = solid.slice(0, Math.min(5, in12.length)).map(r => r._usd); // priced is most-recent first
      if (recentSolid.length >= 2) recency = { n12: in12.length, total: N, k: recentSolid.length, lo: Math.min(...recentSolid), hi: Math.max(...recentSolid) };
    }
  }

  const platforms = foldPlatforms(solid);
  const named = platforms.filter(p => p[0] !== "others");
  const base_ = {
    ...base, tier: "result",
    span, cluster, recency,
    basis: { total: N, windowLabel, setAside: hollow.length, asideTags: outlierTags(hollow) },
    platforms, topPlatform: named.length ? named[0][0] : null, singlePlatform: named.length <= 1,
    cards: shapeCards(solid), asideCards: shapeCards(hollow),
    // Sam's read is templated by the frontend from these flags; no per-car bespoke copy.
    samRead: { model: spec.subject }
  };
  return base_;
}
