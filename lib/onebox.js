...
async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const cols = "price:sale_price,auction_end_date:sale_date,source:platform,raw_title:listing_title,year,vin_norm," +
    ...
    "transmission:raw_record->>transmission,color:raw_record->>exterior_color,srcurl:raw_record->>url,srcurl2:raw_record->>source_url";
...
function r4DriverSearch(solid, cluster, vehicle) {
  ...
  for (const drv of driversForVehicle(vehicle)) {
    const yes = solid.filter(r => drv.re.test(titleOf(r))), no = solid.filter(r => !drv.re.test(titleOf(r)));
  ...
  return { wideBand: true, candidates, best: candidates[0] || null };
}
...
function buildResult(chosen, base, iso, spec, refine) {
  ...
  const refined = !!(refine && (refine.miMin != null || refine.tx || refine.driver));
  if (refined) {
    ...
    if (refine.driver) { const drv = driverByKey(base.vehicle, refine.driver); if (drv) rows = rows.filter(r => refine.driverVal === "no" ? !drv.re.test(titleOf(r)) : drv.re.test(titleOf(r))); }
  }
  ...
  } else if (ds.best && ds.best.kind === "driver" && !(refined && refine.driver)) {
    const drv = driverByKey(base.vehicle, ds.best.driverKey);
    earned = drv ? { kind: "driver", driverKey: drv.key, q: drv.q, yes: drv.yes, no: drv.no, family: drv.family } : pickMileageOrTx();
  } else if (ds.best && ds.best.kind === "transmission" && txOK) {
    earned = { kind: "transmission", labels: txGate };
  } else if (ds.best && ds.best.kind === "mileage" && mileageOK) {
    earned = { kind: "mileage", buckets };
  } else {
    earned = pickMileageOrTx();                       // wide band: best fact already answered -> next available
    // Wide band and truly nothing splits 5+/5+ (no mileage/gearbox/dictionary): one honest sentence.
    if (!earned && ds.wideBand && !ds.best) driverSentence = "These vary for reasons the sales record can't fully see, provenance and originality mostly. The titles below say which is which.";
  }
...
export async function runOneBox(vehicle, generation, searchText, env, refine) {
  ...
  const result = buildResult(chosen, base, iso, spec, refine);
  if (result.tier === "result") result.freshness = obFreshness(result.houseLed, result.newestSale, ingestDate, now);
  return result;
}
