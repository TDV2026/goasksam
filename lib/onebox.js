...
// Reason a title should be set aside from a RECORD/max answer: a make halo, a race car, or a
// restomod. Returns "halo"|"race car"|"restomod"|null. wantHalo (the query names the halo, e.g.
// "Sport Evolution") keeps halos in but still drops race cars/restomods.
export function recordExcludeReason(title, make, opts = {}) {
  const t = String(title || "");
  if (RACE_TITLE_RE.test(t)) return "race car";
  if (RESTOMOD_TITLE_RE.test(t)) return "restomod";
  if (!opts.wantHalo) { const h = haloMatcherFor(make); if (h && h.test(t)) return "halo"; }
  return null;
}
