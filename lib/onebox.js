...
export async function runOneBox(vehicle, generation, searchText, env) {
  ...
  const THIN_FLOOR = 4;
  if (N < THIN_FLOOR) {
    return { ...base, tier: "refusal", refusal: { kind: "thin", model: subject, n: N, variants: [], yearSpanPhrase: null }, cards: shapeCards(priced) };
  }
  ...
  if (solid.length < THIN_FLOOR || ratio > IMPLAUSIBLE) {
    ...
    return { ...base, tier: "refusal", refusal: { kind: "varied", model: subject, n: N, variants: variantTokens(priced).slice(0, 6), yearSpanPhrase }, cards: shapeCards(priced) };
  }
