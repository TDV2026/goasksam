...
export async function runOneBox(vehicle, generation, searchText, env, refine) {
  ...
  if (refine && refine.driver) await obEnrichDescriptions(chosen, env, diag);
  ...
  if (result.tier === "result") {
    ...
    if (!(refine && refine.driver) && fam.length && dd && dd.wideBand && !(dd.candidates || []).some(c => fam.some(f => f.key === c.k))) {
      ...
      if (dd) { dd.descFetchMs = diag.descFetchMs != null ? diag.descFetchMs : (ms != null ? ms : "fetch_failed"); dd.descEnriched = diag.descEnriched; dd.descPoolSize = diag.descPoolSize; dd.postDict = r2 && r2.driverDiag ? r2.driverDiag.dict : null; }
