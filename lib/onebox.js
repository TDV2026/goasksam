...
async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const cols = "price:sale_price,auction_end_date:sale_date,source:platform,raw_title:listing_title,year," +
    ...
    "transmission:raw_record->>transmission,srcurl:raw_record->>source_url";
...
function shapeCards(rows) {
  return rows.map(r => ({
    ...
    title: titleOf(r).replace(/[–—]/g, "-").replace(/\s+/g, " ").trim(), image: r.image || null, url: r.srcurl || null, hollow: !!r._hollow
  }));
}
...
function r4Direction(recent, prior) {
  const rs = r4Split(recent).solid, ps = r4Split(prior).solid;
...
function buildResult(chosen, base, iso, spec, refine) {
  ...
  const useRecent = r4Split(recent).solid.length >= 8;
  ...
  const { solid, aside } = r4Split(window);
  const THIN = 4;
  ...
  const direction = (!refined && useRecent) ? r4Direction(recent, prior) : null;
