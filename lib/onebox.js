...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  if (spec.recordSort && scope.byTitle) {
    ...
    // Record for a NON-badged model (944 Turbo, 2002tii, 190E, 240Z): scope by the MODEL trigram, and
    // do NOT ask the DB to sort by price (a leading-wildcard ILIKE + ORDER BY sale_price is planner-
    // unstable and timed out for a higher-volume model like 944). Instead PAGE the model's rows
    // unordered (trigram filter, index-served, cheap per page) to completion, then the record output
    // sorts by price + applies the set-aside in code. make ILIKE (not the marque price-walk, which
    // never reaches a cheaper model on a marque full of pricier cars). A named trim scopes the title.
    const rqBase = `sales_archive?select=${cols}&make=ilike.${encodeURIComponent("*" + spec.make + "*")}&model=ilike.${encodeURIComponent("*" + flexPattern(scope.byModel) + "*")}&sale_price=not.is.null`
      + (spec.titleContains ? `&listing_title=ilike.${encodeURIComponent("*" + spec.titleContains + "*")}` : "");
    const all = [];
    ...
    if (rawRows.length < 5) {
      for (let off = 0; off < 2000; off += 1000) {
        const pr = await supabaseSelect(env, `sales_archive?select=${cols}&make=ilike.${encodeURIComponent("*" + spec.make + "*")}&listing_title=ilike.${encodeURIComponent("*" + flexPattern(scope.byModel) + "*")}&sale_price=not.is.null&order=id.asc&limit=1000&offset=${off}`) || [];
