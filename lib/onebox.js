...
export async function runOneBox(vehicle, generation, searchText, env) {
  ...
  const n = pool.length;
  // Distinct platforms in the final pool - for the streaming working line ("N sales
  // across M platforms"). Real breadth of the pool, not the 3 shown cards.
  ...
  if (n >= 3) {
    ...
    // Answer-line span (STEP 2 wiring): the lowest/highest REAL surviving sale in the
    // FULL post-outlier pool (two actual transactions), plus the displayed closest match.
    // Endpoints come from the whole pool, so they will usually NOT equal the shown cards.
    const spanVals = pool.map(outcomeUsd).filter(Number.isFinite);
    const closestCard = cards.find(c => c.closest) || cards[0];
    const closestUsd = closestCard.display && closestCard.display.currency === "USD" ? Math.round(closestCard.display.amount) : (closestCard.usdApprox || null);
    const answer = { kind: "span", low: Math.min(...spanVals), high: Math.max(...spanVals), closest: closestUsd, closestMonth: monthName(closestCard.date) };
    return { ...base, tier: "three", trustLine: trustLine(n), samLine: SAM_LINE.three, cards, answer };
  }
  if (n === 2) {
    const { cards, refuse } = labelCards([pool[1], pool[0]], spec, subject);
    if (refuse) return { ...base, count: undefined, tier: "underspecified", samLine: SAM_LINE.underspecified, trustLine: null, cards: [] };
    // Two-sale honest degradation: the two actual sales (higher, then lower). No span.
    const answer = { kind: "two", high: outcomeUsd(pool[1]), low: outcomeUsd(pool[0]) };
    return { ...base, tier: "two", trustLine: trustLine(n), samLine: SAM_LINE.two, cards, answer };
  }
  if (n === 1) {
    ...
    // One-sale honest degradation: the single sale, no span.
    const answer = { kind: "one", one: outcomeUsd(pool[0]) };
    return { ...base, tier: "one", trustLine: trustLine(n), samLine: SAM_LINE.one, cards: [one], answer };
  }
...
export async function runOneBoxModelChoice(vehicle, env) {
  ...
  // Rank the "Which <make> model?" chips by REAL sales (sales_archive, deduped on source_id),
  // not vehicle_market_records (which measures fetch frequency and is sparse for a specific
  // year slice). A generous +/-6 window keeps the options era-appropriate without over-
  // narrowing: a tight year+/-3 slice of the search store crowded a high-volume nameplate
  // (an M3 for a 2020 BMW VIN) out of the top 6. Newest sales first so the list reflects the
  // current market. A missing chip is still recoverable via free text (the input persists).
  let q = `sales_archive?select=model&make=ilike.${encodeURIComponent(make)}&model=not.is.null&order=sale_date.desc.nullslast&limit=6000`;
  if (year) q += `&year=gte.${year - 6}&year=lte.${year + 6}`;
