...
async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const returnedRows = qualified.filter(r => Number.isFinite(r.value));
  if (diag && diag._trace) {
    const T = r => r.raw_title || r.rtitle || "";
    diag.stages = diag.stages || [];
    diag.stages.push({
      scope: scope.byTitle ? ("title:" + scope.byTitle) : ("model:" + scope.byModel),
      titleContains: spec.titleContains || null, yr: [spec.yearMin || null, spec.yearMax || null], body: spec.bodyStyle || null,
      raw: rawRows.length, deduped: rows.length, qualified: qualified.length, returned: returnedRows.length,
      failNoImage: rows.filter(r => !r.image).length,
      failPrice: rows.filter(r => !(Number(r.price) > 0)).length,
      failParts: rows.filter(r => isPartsListing(r.raw_title || r.rtitle, r.mileage)).length,
      failBadge: spec.badgeRe ? rows.filter(r => !spec.badgeRe.test(T(r))).length : 0,
      failMods: rows.filter(r => { const m = r.mods == null ? "" : String(r.mods); return SUBSTANTIAL_MOD.test(m) || MOD_MARKERS.some(w => String(T(r)).toLowerCase().includes(w)); }).length,
      failBody: spec.bodyStyle ? rows.filter(r => { const b = classifyBody(r); return b && bodyFamily(b) !== bodyFamily(spec.bodyStyle); }).length : 0,
      failValue: qualified.length - returnedRows.length
    });
  }
  return returnedRows;
}
...
function familyWideningSentence(trimLabel, model, isHalo) {
  ...
  return isHalo
    ...
    : `No recent ${trimLabel} sales in this window, so this reads other ${m}s.`;
}
...
function buildResult(chosen, base, iso, spec, refine) {
  ...
  if (base.dedup && base.dedup._trace) base.dedup.chosen = {
    step: chosen.step, windowLabel, useRecent,
    chosenRows: rows.length, solid: solid.length, aside: aside.length,
    recent12moSolid: r4Split(recent, keepAll).solid.length, poolSolidFull: r4Split(rows, keepAll).solid.length
  };
  const THIN = 4;
...
export async function runOneBox(vehicle, generation, searchText, env, refine) {
  ...
  const diag = (env && env._trace) ? { _trace: true } : {};
