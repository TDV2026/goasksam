...
async function assessHouseTier(famSpec, trimName, trimRe, iso, env, diag) {
  const raw = await fetchQualifying({ ...famSpec, titleContains: trimName || undefined }, iso(HT_WINDOW_DAYS), env, diag);
  const pool = raw
    .filter(r => (trimRe ? trimRe.test(String(r.raw_title || r.rtitle || "")) : true))
  ...
  const isHouseTier = online.length < HT_ONLINE_MIN && house.length >= 1 && pool.length >= 2;
  const meta = { isHouseTier, onlineN: online.length, houseN: house.length, totalN: pool.length, windowMonths: 36 };
  ...
  const medianHammer = medianPrice([...receipts].map(r => r.hammer).sort((a, z) => a - z));
