...
function buildSpec(vehicle, generation, searchText) {
  ...
  if (spec.year) {
    const matching = generationsForModel(spec.make, spec.model).filter(g => spec.year >= g.yearStart && spec.year <= g.yearEnd);
...
// RATIOS ARE PROVISIONAL pending Sam's pick from the before/after: set WIDE so this ship removes
// only egregious contaminants; tighten OB_OUTLIER_HI to also drop near-halos like the $600k card.
const OB_OUTLIER_HI = 10;   // PROVISIONAL: exclude a record priced > median * HI
const OB_OUTLIER_LO = 10;   // PROVISIONAL: exclude a record priced < median / LO
...
async function assessClassEra(spec, env, diag) {
  ...
  // Fence extreme price outliers out of the class band (data-error lows like a $513 mis-priced
  // 250F, halo highs) before the p10-p90. PROVISIONAL ratios (priceOutlierBounds); a class pool is
  // heterogeneous by design, so this only removes records implausible against the pool median.
  const _cb = priceOutlierBounds(pool.map(r => r._hammer));
  const fenced = _cb ? pool.filter(r => r._hammer <= _cb.hi && r._hammer >= _cb.lo) : pool;
  ...
  if (!spec.trim && !spec.yearRange && !spec.genCode) {
    ...
    if (matching.length >= 2) {
      ...
      const prompt = spec.year
        ? `A ${spec.year} ${spec.model} lands right on a generation change - the ${matching.map(g => g.code).join(" and ")} both claim it, and they price very differently. Which is yours?`
        : `The ${spec.model} spans generations that price very differently. Which is yours?`;
