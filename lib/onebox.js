...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const base = `sales_archive?select=${cols}` +
    ...
    // Record limit is modest: the JSONB-heavy select costs ~15ms/row, so 500 rows blew the 8s
    // statement timeout. 120 top-by-price rows is plenty of headroom above the ~20 specials that sit
    // at the top of any nameplate, so >=5 STANDARD sales always remain after the set-aside.
    (spec.recordSort ? `&order=sale_price.desc&limit=120` : `&order=sale_date.desc&limit=1000`);
  ...
  // STANDARD sale.
  // The record query keeps make=eq (base) so the (make, sale_price DESC) composite index walks the
  // marque by price with EARLY termination at the limit, and adds a cheap model ILIKE recheck (the
  // short model column, not the VIN-noisy title). Measured ~2s: the badge's specials dominate the top
  // of the marque's prices, so the walk finds the top-120 model matches quickly. (make ILIKE forces
  // the weak 2-char model trigram + a full sort -> ~8s; the \y regex was intermittently un-indexed.)
  // A plain trigram ILIKE also matches "M340i", so the WHOLE-WORD filter is applied IN CODE below.
  const recordModelQ = t => `${base}&model=ilike.${encodeURIComponent("*" + String(t) + "*")}`;
  ...
  if (spec.recordSort && scope.byTitle) {
    // Record + badged family: trigram ILIKE fetch (fast, reliable) + in-code whole-word filter, with a
    // bounded title fallback only if the model column is thin (mislabels), so the scan never times out.
    const got = await supabaseSelect(env, recordModelQ(scope.byTitle)); if (got === null && diag) diag.queryError = true;
    const ww = recordWholeWord(scope.byTitle);
    rawRows = (got || []).filter(r => ww.test(String(r.model || "")));
  ...
  else if (scope.byTitle) { const got = await supabaseSelect(env, titleQ(scope.byTitle)); if (got === null && diag) diag.queryError = true; rawRows = got || []; }
  else {
