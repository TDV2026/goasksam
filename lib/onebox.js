...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  const base = `sales_archive?select=${cols}` +
    ...
    (spec.recordSort ? `&order=sale_price.desc&limit=300` : `&order=sale_date.desc&limit=1000`);
  ...
  if (spec.recordSort && scope.byTitle) {
    // Record + badged family: pull the marque's top-300 by price (base, index-ordered, ~1s), filter the
  ...
  else if (spec.recordSort && scope.byModel) {
    // Record for a NON-badged model (944 Turbo, 2002tii, 190E, 240Z): scope by the MODEL trigram with
    // make ILIKE (NOT make=eq), so the model trigram drives the plan and the query does NOT walk the
    // marque's top prices - a marque price-walk never reaches a cheaper model on a marque full of
    // pricier cars (Porsche's top 300 are Carrera GTs/959s; BMW's are 507s/M1s), returning empty. A
    // named trim (titleContains) further scopes the title. Whole-word model filter in code.
    const rq = `sales_archive?select=${cols}&make=ilike.${encodeURIComponent("*" + spec.make + "*")}&model=ilike.${encodeURIComponent("*" + flexPattern(scope.byModel) + "*")}&sale_price=not.is.null`
      + (spec.titleContains ? `&listing_title=ilike.${encodeURIComponent("*" + spec.titleContains + "*")}` : "")
      + `&order=sale_price.desc&limit=150`;
    const got = await supabaseSelect(env, rq); if (got === null && diag) diag.queryError = true;
    const ww = recordWholeWord(scope.byModel);
    rawRows = (got || []).filter(r => ww.test(String(r.model || "")) || ww.test(String(r.raw_title || "")));
    // Title fallback if the model column is thin for this nameplate (mislabels): match the title.
    if (rawRows.length < 5) { const more = (await supabaseSelect(env, titleQ(scope.byModel))) || []; const seen = new Set(rawRows.map(r => r.id)); for (const r of more) if (!seen.has(r.id)) rawRows.push(r); }
  }
