...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  // STANDARD sale.
  // The record query uses make ILIKE (NOT make=eq) so the planner drives from the MODEL trigram index
  // and sorts the small M3-model set by price - make=eq would pick the (make,sale_price) composite and
  // walk past 507/M1/Z8 by price (the original timeout). Lean cols already set for recordSort.
  const recordModelQ = t => `sales_archive?select=${cols}&make=ilike.${encodeURIComponent("*" + spec.make + "*")}&sale_price=not.is.null` +
    (spec.yearMin ? `&year=gte.${spec.yearMin}` : "") + (spec.yearMax ? `&year=lte.${spec.yearMax}` : "") +
    `&model=ilike.${encodeURIComponent("*" + String(t) + "*")}&order=sale_price.desc&limit=120`;
