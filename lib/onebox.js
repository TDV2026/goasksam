...
export async function runOneBox(vehicle, generation, searchText, env) {
  ...
  if (pool.length >= 3) {
    ...
    const p = f => Number(core[Math.min(core.length - 1, Math.floor(core.length * f))].price);
    const ratio = core.length >= 6 ? p(0.9) / p(0.1) : (core.length ? Number(core[core.length - 1].price) / Number(core[0].price) : 1);
