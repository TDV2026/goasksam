...
function familyWideningSentence(trimLabel, model, isHalo) {
  const m = model || "car";
  return isHalo
    ? `No recent ${trimLabel} sales, so this reads other ${m}s. The ${trimLabel} has sat above these when it trades.`
...
function buildResult(chosen, base, iso, spec, refine) {
  ...
  if (base.dedup) base.dedup.win = {
    step: chosen.step, windowLabelUsed: windowLabel, useRecent,
    recent12moSolid: r4Split(recent, keepAll).solid.length,
    priorSolid: r4Split(prior, keepAll).solid.length,
    poolSolidFullWindow: r4Split(rows, keepAll).solid.length
  };
