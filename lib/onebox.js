...
export async function fetchQualifying(spec, sinceIso, env, diag) {
  ...
  // RECORD scope (Desk, Sep 2026 fix): match the badge as a WHOLE WORD on the MODEL column (Postgres
  // ~* with \y boundaries, served by the model trigram index), so "M3" catches "(E46) M3"/"M3 CSL"
  // but never "M340i", and the query does NOT walk past non-badge high-price cars (507, M1, Z8) the
  // way the title-ILIKE + price-sort scan did (that timed out at ~9s). Fast (~0.2-0.5s). The shared
  // set-aside then drops the DTM race car / CSL halo so the RECORD is the top STANDARD sale.
  const recordModelQ = t => `${base}&model=imatch.${encodeURIComponent("\\y" + String(t).replace(/[.*+?^${}()|[\]\\]/g, "") + "\\y")}`;
  const t0 = Date.now();
  let rawRows;
  ...
  if (spec.recordSort && scope.byTitle) {
    // Record + badged family: whole-word model match (fast), with a bounded title fallback only if the
    // model column is thin (mislabels), so the record scan never times out.
