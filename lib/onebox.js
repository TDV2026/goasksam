...
export async function runOneBox(vehicle, generation, searchText, env, debug) {
  ...
  if (debug) base.debug = { model: spec.model, trim: spec.trim, bodyStyle: spec.bodyStyle, yearMin: spec.yearMin, yearMax: spec.yearMax, byTitle: archiveScope(spec).byTitle || null, poolSize: n, sample: pool.slice(0, 12).map(r => ({ price: Math.round(+r.price), year: r.year, body: classifyBody(r) || "unstated", title: r.rtitle || r.raw_title })) };
