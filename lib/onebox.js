...
async function fetchQualifying(spec, sinceIso, env, diag) {
  // NOTE (Sep 9 2026): reads vehicle_market_records. sales_archive holds far more depth (E30 M3:
  // 52 vs 4 in vmr) but querying it live is too slow without indexes - a naive switch measured
  // 15-27s per One Box call (make/model ILIKE + a JSONB image filter over ~200k unindexed rows)
  // and is reverted. The family-scope fix below (archiveScope) still lifts the pool by scoping
  // the model correctly. A proper sales_archive-backed pool needs indexes (a Supabase DDL step).
