...
// low bracket that explain the spread, subject-excluded. Backend returns facts + delta TOKENS
// only (fewer_miles / more_miles / manual / earlier / higher / lower); the render turns tokens
// into prose (product rule 3). count drives the tier (3+ = three cards, 2 = two, 1 = one).
function pickRepresentative(solid, exactSale, center, driver) {
  ...
  const rest = [...comps.filter(r => r !== closest)].sort((a, b) => a._usd - b._usd);
  const low = rest[0] || null, high = rest.length > 1 ? rest[rest.length - 1] : null;
  const cMi = miOf(closest);
  const delta = (r, hi) => {
    if (!r) return null;
    const mi = miOf(r);
    if (cMi > 0 && mi > 0) { if (hi && mi < cMi * 0.85) return "fewer_miles"; if (!hi && mi > cMi * 1.15) return "more_miles"; }
    if (hi && R4_MANUAL(String(r.transmission || "")) && !R4_MANUAL(String(closest.transmission || ""))) return "manual";
    const rd = String(r.auction_end_date || "").slice(0, 10), cd = String(closest.auction_end_date || "").slice(0, 10);
    if (rd && cd && rd < cd && (new Date(cd) - new Date(rd)) / 864e5 > 180) return "earlier";
    return hi ? "higher" : "lower";
  };
