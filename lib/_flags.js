...
export async function findVinArchiveMatch(env, { vin } = {}) {
  ...
  try {
    ...
    let rows = [];
    {
      ...
      // Never swallow a failed lookup: a statement timeout (57014, an unindexed vin scan on
      // the large archive) or any non-OK status is logged LOUD so a regression surfaces
      // instead of silently returning "no match" for a car we actually hold.
      else console.error(`CRITICAL: findVinArchiveMatch vin=eq failed (${r.status}) for a ${want.length}-char id - possible missing index / statement timeout. ${(await r.text().catch(() => "")).slice(0, 160)}`);
    }
    ...
    if (!rows.length && want.length !== 17) {
      ...
      else console.error(`CRITICAL: findVinArchiveMatch ilike fallback failed (${r.status}) for a ${want.length}-char chassis - likely statement timeout on the unindexed scan. ${(await r.text().catch(() => "")).slice(0, 160)}`);
    }
