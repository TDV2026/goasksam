...
export async function buildSample(env, { size = 400, seed = 42 } = {}) {
  ...
  const uniq = [...new Map(pool.map(r => [r.chassis_vin_norm, r])).values()];
  const sample = seededShuffle(uniq, seed).slice(0, size).map(r => ({
    vin: r.chassis_vin_norm, make: r.make, model: r.model, year: Number(r.year), hammer: Number(r.hammer_usd) || null,
    marque: marqueGroup(r.make), tier: priceTier(r.hammer_usd), genMapped: generationsForModel(r.make, r.model).length > 0
  }));
  return sample;
}
...
function meta(subj, path) { return { vin: subj.vin, subject: `${subj.year} ${subj.make} ${subj.model}`, path, marque: subj.marque, tier: subj.tier, genMapped: subj.genMapped }; }
...
export function summarize(results) {
  const byInv = {}, byStratum = { marque: {}, tier: {}, genMapped: {}, path: {} };
  ...
  for (const r of results) {
    ...
    if (r.failures && r.failures.length) {
      ...
      for (const f of r.failures) {
        ...
        const add = (dim, key) => { const d = byStratum[dim]; (d[key] = d[key] || {})[f.inv] = (d[key][f.inv] || 0) + 1; };
        add("marque", r.marque); add("tier", r.tier); add("genMapped", String(r.genMapped)); add("path", r.path);
        failures.push({ subject: r.subject, vin: r.vin, path: r.path, marque: r.marque, tier: r.tier, genMapped: r.genMapped, inv: f.inv, detail: f.detail });
      }
    }
  }
  return { totalRuns: results.length, resultRuns: checked, runsWithFailures: failedRuns, byInvariant: byInv, byStratum, failures };
}
