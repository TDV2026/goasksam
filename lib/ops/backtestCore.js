...
export async function buildSample(env, { size = 400, seed = 42 } = {}) {
  ...
  const balanced = (rows, n) => {
    const groups = {}; for (const r of rows) (groups[r.marque] = groups[r.marque] || []).push(r);
    const order = Object.keys(groups); const out = []; let added = true;
    while (out.length < n && added) { added = false; for (const g of order) { if (groups[g].length) { out.push(groups[g].shift()); added = true; if (out.length >= n) break; } } }
    return out;
  };
  // Force ~30% generation-mapped subjects (911, M3/M4/M5, Corvette, Mustang, E-Type, etc.) so the
  // I1 generation invariant gets real weekly stress instead of mostly passing trivially on the
  // unmapped majority - wrong-generation (F82/G82, 997/991) was the worst bug class. Each half is
  // marque-balanced; a year is always present on the typed path, so mapped subjects reach a result
  // (not a generation_choice) and actually exercise I1.
  const mapped = uniq.filter(r => r.genMapped), unmapped = uniq.filter(r => !r.genMapped);
  const mappedN = Math.min(Math.round(size * 0.30), mapped.length);
  const out = [...balanced(mapped, mappedN), ...balanced(unmapped, size - mappedN)];
  return seededShuffle(out, seed ^ 0x9e3779b9);
}
...
export function checkInvariants(subj, vehicle, r) {
  // THIN MODE invariants (Sep 2026): T1 thin fires only when the volume-capable online count is
  // under the threshold; T2 the house steer is on IFF house share >= 2/3; T3 every receipt has a
  // hammer basis and every house receipt shows all-in (rule 1 / no invented price).
  if (r.tier === "thin" && r.thin) {
    const t = r.thin, tf = [];
    if (!(t.onlineN < 15)) tf.push({ inv: "T1", detail: `thin fired with onlineN ${t.onlineN} >= 15` });
    const denom = (t.houseN || 0) + (t.onlineReceiptsN || 0);
    const expectSteer = denom > 0 && (t.houseN / denom) >= (2 / 3);
    if (!!t.houseSteer !== expectSteer) tf.push({ inv: "T2", detail: `houseSteer ${t.houseSteer} but share ${t.houseN}/${denom}` });
    for (const rc of (t.receipts || [])) {
      if (!(rc.hammer > 0)) tf.push({ inv: "T3", detail: `thin receipt no hammer: ${rc.title}` });
      if (rc.isHouse && !(rc.allIn > 0)) tf.push({ inv: "T3", detail: `thin house receipt no all-in: ${rc.title}` });
    }
    return { checked: true, failures: tf };
  }
  // CLASS-ERA invariants: C1 every receipt has a hammer basis + house all-in; C2 the coarse band is
  // ordered (low <= mid <= high). It never claims to be the exact car (render-level, labelled).
  if (r.tier === "class_era" && r.classEra) {
    const ce = r.classEra, cf = [];
    for (const rc of (ce.receipts || [])) {
      if (!(rc.hammer > 0)) cf.push({ inv: "C1", detail: `class receipt no hammer: ${rc.title}` });
      if (rc.isHouse && !(rc.allIn > 0)) cf.push({ inv: "C1", detail: `class house receipt no all-in: ${rc.title}` });
    }
    if (!(ce.lowHammer <= ce.medianHammer && ce.medianHammer <= ce.highHammer)) cf.push({ inv: "C2", detail: `class band disordered: ${ce.lowHammer}/${ce.medianHammer}/${ce.highHammer}` });
    return { checked: true, failures: cf };
  }
