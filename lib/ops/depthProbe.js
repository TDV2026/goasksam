...
// The US-launch-relevant default set for the web trigger: the 8 launch platforms
// plus the two never-fetched candidates that the probe exists to validate.
export const LAUNCH_SOURCES = [
  "bringatrailer", "carsandbids", "hagerty", "pcarmarket", "hemmings",
  "sothebysmotorsport", "autohunter", "mbmarket"
];
...
export async function runDepthProbe({ apiKey, sources, windows, maxRequests = 40, pageLimit = 50, budgetLeft = Infinity, log = () => {} } = {}) {
  ...
  for (const source of SOURCES) {
    ...
    for (let p = 1; p <= 10000; p++) {
      ...
      if (reason) { meta[source].note = `stopped: ${reason}`; log(`[budget] ${reason} at ${metered} request(s)`); break outer; }
      metered++;
      let res;
      try {
        ...
        if (e.rateLimited) { log(`[ratelimit] ${label} p${p}: ${e.message}`); break outer; }
        break;
      }
      const rows = res.data || [];
