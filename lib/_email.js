...
function renderLeadEmail({ partnerName, reference, seller, car, choice, recommendedPath }) {
  const rows = [
    ["Car", car.raw],
...
export async function sendLeadNotification({ partnerEmail, partnerName, reference, seller, car, choice, recommendedPath }) {
  const subjectCar = String(car.raw || "a car").trim();
  const subject = `New GoAskSam lead: ${subjectCar}${partnerName ? ` (${partnerName})` : ""}`;
  const { text, html } = renderLeadEmail({ partnerName, reference, seller, car, choice, recommendedPath });
