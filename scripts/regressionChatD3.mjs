...
Decision facts (the engine's recommendation, do not contradict it): recommended platform Bring a Trailer; basis model_family; confidence medium; data confidence a solid recent sample in the last 180 days; price signal: sales closed around 12% higher on the recommended platform than on other platforms over the past 180 days. Reasons: Bring a Trailer has the strongest enthusiast audience for this car.`;
...
Decision facts (the engine's recommendation, do not contradict it): recommended platform ${pick}; basis model_family; confidence medium; data confidence a solid recent sample in the last 180 days; price signal: sales closed around 20% higher on the recommended platform than on other platforms over the past 180 days. Reasons: ${pick} has the deepest recent market for this car.
Evidence by platform (cite percentages only; sample size is qualitative, NEVER a count): ${pick}: a solid recent sample, about 20% vs other platforms (any_year scope); ${alt}: a modest recent sample.
...
Rendered destinations (what the seller is looking at): PICK ${pick}, ALT ${alt}. A "compare the options / compare the tradeoffs / which should I pick" request means comparing THESE TWO PLATFORMS. Compare them on four axes ONLY: price outcome, time to list, audience fit, and how much sales data backs each (qualitative only, NEVER a count of sales/comps). They are two DIFFERENT platforms: NEVER say both paths lead to the same platform, NEVER reframe this as PowerSeller-vs-doing-it-yourself, and NEVER contradict either card's stated finding. The pick stays the recommendation; the comparison explains it, it does not reopen it.`;
