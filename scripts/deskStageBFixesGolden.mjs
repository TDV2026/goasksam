...
const EXPECT = ["901 (1964 to 1973)", "G-body (1974 to 1989, incl. 930 Turbo)", "964 (1989 to 1994)", "993 (1995 to 1998)"];
const a = await interp("air-cooled 911s under $100k sold this year");
console.log("  chip labels:", JSON.stringify(a.chipLabels));
ck("air-cooled shows 4 removable members (sameModel)", a.members.length === 4 && a.reading.grouping && a.reading.grouping.sameModel, JSON.stringify(a.members));
ck("member chips use the APPROVED dictionary labels (not '930')", JSON.stringify(a.chipLabels) === JSON.stringify(EXPECT), JSON.stringify(a.chipLabels));
ck("G-body spans 1974 to 1989 (1989 included)", !!a.gbody && a.gbody.yearStart === 1974 && a.gbody.yearEnd === 1989, JSON.stringify(a.gbody));
ck("no bare '930' chip", !a.chipLabels.some(l => /^930\b/.test(String(l))) && !a.members.includes("930"), JSON.stringify(a.chipLabels));
...
console.log("  after 'drop the 964':", JSON.stringify(dropped.chipLabels));
...
ck("remaining chips keep approved labels after drop", dropped.chipLabels.every(l => EXPECT.includes(l)), JSON.stringify(dropped.chipLabels));
