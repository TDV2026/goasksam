...
// ---- Locality resolver (Part 1): city -> state, silent-exact / confirm-fuzzy ----
// The Ingo/Bay-Area fix: a Bay Area city resolves to "California" so a region-only
// specialist matches, and a typo confirms (never silently auto-corrects) per rule 6.
{
  const R=q=>resolveStateInput(q);
  check("resolver: 'san fransisco' (typo) -> CONFIRM San Francisco, California", (()=>{const r=R("san fransisco");return r.kind==="cityConfirm"&&r.value==="California"&&r.city==="San Francisco";})(), JSON.stringify(R("san fransisco")));
  check("resolver: 'oakland' (exact) -> silent California, no confirm", (()=>{const r=R("oakland");return r.kind==="state"&&r.value==="California";})(), JSON.stringify(R("oakland")));
  check("resolver: 'san jose' (exact) -> silent California", R("san jose").kind==="state"&&R("san jose").value==="California", JSON.stringify(R("san jose")));
  check("resolver: 'berkeley' / 'palo alto' -> California", R("berkeley").value==="California"&&R("palo alto").value==="California", "");
  check("resolver: 'denver' -> silent Colorado", R("denver").kind==="state"&&R("denver").value==="Colorado", JSON.stringify(R("denver")));
  check("resolver: 'portland' (ambiguous) -> CONFIRM Oregon even when correctly spelled", (()=>{const r=R("portland");return r.kind==="cityConfirm"&&r.value==="Oregon";})(), JSON.stringify(R("portland")));
  check("resolver: 'portland maine' -> Maine (trailing state wins over the city default)", R("portland maine").kind==="state"&&R("portland maine").value==="Maine", JSON.stringify(R("portland maine")));
  check("resolver: 'kansas city' -> Missouri default (not Kansas)", R("kansas city").value==="Missouri", JSON.stringify(R("kansas city")));
  check("resolver: state name/code/ZIP still win outright (California / CA / 94110)", R("california").value==="California"&&R("ca").value==="California"&&R("94110").value==="California", JSON.stringify([R("california"),R("ca"),R("94110")]));
