...
async function handleOps(req, res) {
  ...
  if (task === "pulse488") {
    ...
    for (let off = 0; off < 6000; off += 1000) {
      const chunk = await supabaseSelect(env, `vehicle_market_records?make=ilike.Ferrari&auction_end_date=gte.2025-06-01&auction_end_date=lt.2026-09-01&select=auction_end_date,raw_title,platform,price,raw_record&order=auction_end_date.asc&limit=1000&offset=${off}`) || [];
      rows = rows.concat(chunk); if (chunk.length < 1000) break;
    }
    const all488 = rows.filter(r => /488/.test(r.raw_title || "") && Number(r.price) > 0);
    const build = (lo, hi) => {
      const w4 = all488.filter(r => ONLINE4.has(r.platform) && r.auction_end_date >= lo && r.auction_end_date <= hi);
      const listing = w4.map(r => { const d = new Date(r.auction_end_date + "T12:00:00Z"); return { source: r.platform, date: String(r.auction_end_date).slice(0, 10), dow: DOW[d.getUTCDay()], price: Number(r.price), mileage: mileageOf(r.raw_record), variant: variant(r.raw_title), title: r.raw_title }; }).sort((a, b) => a.date < b.date ? -1 : 1);
      ...
      const otherSrc = {}; for (const r of all488) { if (r.auction_end_date >= lo && r.auction_end_date <= hi && !ONLINE4.has(r.platform)) otherSrc[r.platform] = (otherSrc[r.platform] || 0) + 1; }
