...
async function handleOps(req, res) {
  ...
  // task=nlst2: TEMP newsletter sell-through cut - by make, make+model, and auction-END weekday.
  // Archive-only (sales_archive sold + auction_attempts non-sold), ZERO OCD. Remove after capture.
  if (task === "nlst2") {
    if (!env) return res.status(500).json({ error: "Supabase env not set." });
    ...
    const MINCELL = Math.max(1, Number(req.query?.min || 15));
    const BASELINE = 74.4;
    const H = { apikey: env.supabaseKey, Authorization: `Bearer ${env.supabaseKey}` };
    ...
    if (sold.error) return res.status(200).json({ task: "nlst2", error: sold.error });
    ...
    if (att.error) return res.status(200).json({ task: "nlst2", error: att.error });
    const S = sold.filter(r => r.sale_date);
    const norm = s => String(s || "Unknown").trim();
    const nkey = s => norm(s).toLowerCase();
    const st = (soldN, tot) => tot ? Math.round(soldN / tot * 1000) / 10 : null;
    // ---- by make ----
    const mk = {}; // key -> {disp, sold, rnm, wd}
    for (const r of S) { const k = nkey(r.make); (mk[k] = mk[k] || { disp: norm(r.make), sold: 0, rnm: 0, wd: 0 }).sold++; }
    for (const a of att) { const k = nkey(a.make); const o = (mk[k] = mk[k] || { disp: norm(a.make), sold: 0, rnm: 0, wd: 0 }); if (a.auction_status === "reserve_not_met") o.rnm++; else if (a.auction_status === "withdrawn") o.wd++; }
    const makesAll = Object.values(mk).map(o => ({ make: o.disp, sold: o.sold, reserveNotMet: o.rnm, withdrawn: o.wd, total: o.sold + o.rnm + o.wd, sellThroughPct: st(o.sold, o.sold + o.rnm + o.wd) }));
    const byMakeTopAttempts = makesAll.slice().sort((a, b) => b.total - a.total).slice(0, 15);
    const makesQualified = makesAll.filter(m => m.total >= MINCELL);
    const lowestMakes = makesQualified.slice().sort((a, b) => a.sellThroughPct - b.sellThroughPct).slice(0, 8);
    // ---- by make+model ----
    const mm = {};
    for (const r of S) { const k = nkey(r.make) + "|" + nkey(r.model); (mm[k] = mm[k] || { disp: norm(r.make) + " " + norm(r.model), sold: 0, rnm: 0, wd: 0 }).sold++; }
    for (const a of att) { const k = nkey(a.make) + "|" + nkey(a.model); const o = (mm[k] = mm[k] || { disp: norm(a.make) + " " + norm(a.model), sold: 0, rnm: 0, wd: 0 }); if (a.auction_status === "reserve_not_met") o.rnm++; else if (a.auction_status === "withdrawn") o.wd++; }
    const modelsQualified = Object.values(mm).map(o => ({ model: o.disp, sold: o.sold, reserveNotMet: o.rnm, withdrawn: o.wd, total: o.sold + o.rnm + o.wd, sellThroughPct: st(o.sold, o.sold + o.rnm + o.wd) })).filter(m => m.total >= MINCELL);
    const lowestModels = modelsQualified.slice().sort((a, b) => a.sellThroughPct - b.sellThroughPct).slice(0, 10);
    // ---- single most notable outlier vs baseline (qualified make OR make+model, worst gap) ----
    const outlierPool = [...makesQualified.map(m => ({ level: "make", label: m.make, ...m })), ...modelsQualified.map(m => ({ level: "make+model", label: m.model, ...m }))];
    const worstOutlier = outlierPool.slice().sort((a, b) => a.sellThroughPct - b.sellThroughPct)[0] || null;
    // ---- by auction-END weekday ----
    const DOW = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    const dow = {}; for (const d of DOW) dow[d] = { sold: 0, notSold: 0 };
    for (const r of S) dow[DOW[new Date(r.sale_date + "T00:00:00Z").getUTCDay()]].sold++;
    for (const a of att) if (a.attempt_date) dow[DOW[new Date(a.attempt_date + "T00:00:00Z").getUTCDay()]].notSold++;
    const weekday = DOW.map(d => { const t = dow[d].sold + dow[d].notSold; return { day: d, sold: dow[d].sold, notSold: dow[d].notSold, total: t, sellThroughPct: st(dow[d].sold, t) }; });
    const wdQ = weekday.filter(w => w.total >= MINCELL);
    const bestWd = wdQ.slice().sort((a, b) => b.sellThroughPct - a.sellThroughPct)[0] || null;
    const worstWd = wdQ.slice().sort((a, b) => a.sellThroughPct - b.sellThroughPct)[0] || null;
    return res.status(200).json({
      task: "nlst2", platform: slug, dateRange: `${from} to ${to}`, minCell: MINCELL, baselineSellThroughPct: BASELINE,
      totals: { sold: S.length, attempts: att.length, totalAttempts: S.length + att.length },
      byMake_topByAttempts: byMakeTopAttempts,
      byMake_lowestSellThrough_qualified: lowestMakes,
      byMakeModel_lowestSellThrough_qualified: lowestModels,
      mostNotableOutlier: worstOutlier,
      byAuctionEndWeekday: weekday, bestWeekday: bestWd, worstWeekday: worstWd
    });
  }
