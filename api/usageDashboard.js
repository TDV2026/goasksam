...
async function handleOps(req, res) {
  ...
  // TEMP read-only investigation task (live-auction-house record quality). Archive
  // only, ZERO OCD cost, no writes. Remove after the report. Sub-modes keep the
  // response bounded; all read the service-role env server-side.
  if (task === "livehouse") {
    if (!env) return res.status(500).json({ error: "Supabase env not set." });
    ...
    async function pageAll(query, cap) {
      ...
      while (out.length < cap) {
        ...
        out.push(...rows); offset += step;
        if (rows.length < step) break;
      }
      return out;
    }
    // raw_record field getters (multi-name, case-insensitive-ish).
    const g = (rr, keys) => { for (const k of keys) { if (rr && rr[k] != null && rr[k] !== "") return rr[k]; } return null; };
    const getMileage = rr => g(rr, ["mileage", "odometer", "miles", "mileage_mi", "mileage_km", "km", "odometer_mi"]);
    const getVin = rr => g(rr, ["vin", "chassis", "chassis_number", "vin_number", "chassisNo"]);
    const getColor = rr => g(rr, ["exterior_color", "color", "ext_color", "paint", "exterior", "colour", "exterior_colour"]);
    const getCurrency = rr => g(rr, ["currency", "price_currency", "currency_code", "sale_currency"]);
    const getUrl = (row, rr) => row.source_url || g(rr, ["url", "listing_url", "link", "source_url", "permalink"]);
    const BASIS_KEYS = ["hammer_price", "hammer", "sold_price", "price_realized", "price_realised", "buyers_premium", "buyer_premium", "premium", "total_price", "price_incl_premium", "sale_type", "price_type"];

    if (sub === "sources") {
      const rows = await pageAll("select=source", 200000);
      const counts = {}; for (const r of rows) counts[r.source] = (counts[r.source] || 0) + 1;
      const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]);
      const houses = sorted.filter(([s]) => isHouse(s));
      return res.status(200).json({ task: "livehouse", sub, totalRecords: rows.length, distinctSources: sorted.length, allSources: sorted, detectedHouseSlugs: houses });
    }
    ...
    if (sub === "sample") {
      ...
      // top up to 10 if <10 with diverse houses
      if (picked.length < 10) for (const r of houseRows) { if (!picked.includes(r) && picked.length < 10) picked.push(r); }
      return res.status(200).json({ task: "livehouse", sub, houseRecordsScanned: houseRows.length, sample: picked });
    }
    ...
    if (sub === "census") {
      ...
      for (const row of rows) {
        ...
        if (row.year != null) sy++;
        const cur = getCurrency(rr); if (cur) { currencies[cur] = (currencies[cur] || 0) + 1; if (String(cur).toUpperCase() !== "USD") nonusd++; }
        let hb = false; for (const bk of BASIS_KEYS) if (rr[bk] != null && rr[bk] !== "") { basisKeysSeen[bk] = (basisKeysSeen[bk] || 0) + 1; hb = true; } if (hb) basisKeyHits++;
      }
      const pct = x => n ? +(100 * x / n).toFixed(1) : null;
    ...
    if (sub === "dups") {
      ...
      let dupGroups = 0, dupRecords = 0, distinctSridDups = 0, sameSridDups = 0; const examples = [];
      for (const [key, arr] of Object.entries(groups)) {
        if (arr.length < 2) continue;
        // only count as a real duplicate when ids differ
        const ids = new Set(arr.map(a => a.id));
        if (ids.size < 2) continue;
        dupGroups++; dupRecords += arr.length;
        const srids = new Set(arr.map(a => a.source_record_id));
        if (srids.size > 1) distinctSridDups++; else sameSridDups++;
        if (examples.length < 8) examples.push({ title: arr[0].raw_title, price: arr[0].price, date: arr[0].auction_end_date, count: arr.length, sources: [...new Set(arr.map(a => a.source))], sourceRecordIds: [...srids], distinctSrid: srids.size > 1 });
      }
      return res.status(200).json({
        task: "livehouse", sub, houseRecords: house.length, duplicateGroups: dupGroups, recordsInDupGroups: dupRecords,
        dupGroupsWithDistinctSourceRecordId: distinctSridDups, dupGroupsSharingSourceRecordId: sameSridDups,
        note: "distinctSourceRecordId groups slip past the (source,source_record_id) dedup key because OCD assigned two ids to one sale.", examples
      });
    }

    return res.status(400).json({ error: "livehouse: use sub=sources|sample|census|dups (census takes group=house|baseline)." });
  }
