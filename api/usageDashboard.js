...
async function handleOps(req, res) {
  ...
  // TEMP (VIN journey test): inspect STORED journey records for the VIN-origin markers
  // and prove no raw VIN appears anywhere. Remove after verification.
  if (task === "journeyvin") {
    if (!env) return res.status(500).json({ error: "Supabase env not set." });
    ...
    const journeys = jids.length ? await get(`journeys?journey_id=in.(${jids.join(",")})&select=journey_id,vehicle_year,vehicle_make,vehicle_model,vehicle_trim,vehicle_attrs,vehicle_location`) : [];
    // Scan EVERYTHING we selected for a 17-char VIN pattern.
    ...
    return res.status(200).json({
      task: "journeyvin",
      recentStartedEvents: Array.isArray(evts) ? evts.length : evts,
