...
async function handleOps(req, res) {
  ...
  // task=nlstats: TEMP newsletter stats for one platform+month, archive-sourced (sales_archive
  // + auction_attempts), ZERO OCD. Sold-side aggregates + a STRICTLY SEPARATE non-sold module.
  // Remove after the numbers are captured (no standing key-gated route).
  if (task === "nlstats") {
    if (!env) return res.status(500).json({ error: "Supabase env not set." });
    ...
    const H = { apikey: env.supabaseKey, Authorization: `Bearer ${env.supabaseKey}` };
    const fetchAll = async (table, dateCol, cols) => {
      const rows = []; let cursor = ""; const LIMIT = 1000;
      for (let p = 0; p < 60; p++) {
        const q = `${table}?source_slug=eq.${slug}&${dateCol}=gte.${from}&${dateCol}=lte.${to}&select=${cols},source_record_id&order=source_record_id.asc&limit=${LIMIT}` + (cursor ? `&source_record_id=gt.${encodeURIComponent(cursor)}` : "");
        ...
        rows.push(...b); cursor = b[b.length - 1].source_record_id;
        if (b.length < LIMIT) break;
      }
      return rows;
    };
    // sales_archive uses source_id as its row id, not source_record_id -> keyset on source_id.
    const fetchSold = async () => {
      const rows = []; let cursor = ""; const LIMIT = 1000;
      for (let p = 0; p < 60; p++) {
        const q = `sales_archive?source_slug=eq.${slug}&sale_date=gte.${from}&sale_date=lte.${to}&select=source_id,sale_price,sale_date,make,has_reserve,bids&order=source_id.asc&limit=${LIMIT}` + (cursor ? `&source_id=gt.${encodeURIComponent(cursor)}` : "");
        const r = await fetch(`${env.supabaseUrl}/rest/v1/${q}`, { headers: H });
        if (!r.ok) return { error: `sales_archive ${r.status}: ${(await r.text()).slice(0, 140)}` };
        const b = await r.json(); if (!Array.isArray(b) || !b.length) break;
        rows.push(...b); cursor = b[b.length - 1].source_id;
        if (b.length < LIMIT) break;
      }
      return rows;
    };
    const sold = await fetchSold();
    if (sold.error) return res.status(200).json({ task: "nlstats", error: sold.error });
    const S = sold.filter(r => Number(r.sale_price) > 0);
    const prices = S.map(r => Number(r.sale_price)).sort((a, b) => a - b);
    ...
    // (1) totals + implied commission (BaT buyer fee 5%, min $250, max $7,500 per lot).
    const feePerLot = p => Math.min(7500, Math.max(250, p * 0.05));
    const commissionCapped = round(S.reduce((a, r) => a + feePerLot(Number(r.sale_price)), 0));
    const commissionFlat5 = round(sum * 0.05);
    // (2) bands.
    const BANDS = [["under $10k", 0, 10000], ["$10k-25k", 10000, 25000], ["$25k-50k", 25000, 50000], ["$50k-100k", 50000, 100000], ["$100k-250k", 100000, 250000], ["$250k+", 250000, Infinity]];
    const bandOf = p => BANDS.find(([, lo, hi]) => p >= lo && p < hi)[0];
    const bandDist = {}; for (const [n] of BANDS) bandDist[n] = 0;
    for (const p of prices) bandDist[bandOf(p)]++;
    const bandPct = {}; for (const [n] of BANDS) bandPct[n] = { count: bandDist[n], pct: prices.length ? Math.round(bandDist[n] / prices.length * 1000) / 10 : 0 };
    // (3) top 5 makes by count + avg.
    const byMake = {}; for (const r of S) { const m = r.make || "Unknown"; (byMake[m] = byMake[m] || []).push(Number(r.sale_price)); }
    const topMakes = Object.entries(byMake).map(([make, ps]) => ({ make, count: ps.length, avg: round(ps.reduce((a, b) => a + b, 0) / ps.length) })).sort((a, b) => b.count - a.count).slice(0, 5);
    // (4) weekday breakdown (count + avg), best/worst by avg.
    const DOW = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    const byDow = {}; for (const d of DOW) byDow[d] = [];
    for (const r of S) { const dt = new Date(r.sale_date + "T00:00:00Z"); byDow[DOW[dt.getUTCDay()]].push(Number(r.sale_price)); }
    const weekday = DOW.map(d => ({ day: d, count: byDow[d].length, avg: byDow[d].length ? round(byDow[d].reduce((a, b) => a + b, 0) / byDow[d].length) : 0 }));
    const withVol = weekday.filter(w => w.count > 0);
    const bestDayAvg = withVol.slice().sort((a, b) => b.avg - a.avg)[0] || null;
    const worstDayAvg = withVol.slice().sort((a, b) => a.avg - b.avg)[0] || null;
    const bestDayVol = withVol.slice().sort((a, b) => b.count - a.count)[0] || null;
    // (5) per top-5 make: best weekday by avg price (count + avg).
    const bestDayByMake = topMakes.map(({ make }) => {
      const g = {}; for (const d of DOW) g[d] = [];
      for (const r of S) if ((r.make || "Unknown") === make) g[DOW[new Date(r.sale_date + "T00:00:00Z").getUTCDay()]].push(Number(r.sale_price));
      const days = DOW.map(d => ({ day: d, count: g[d].length, avg: g[d].length ? round(g[d].reduce((a, b) => a + b, 0) / g[d].length) : 0 })).filter(x => x.count > 0);
      const best = days.slice().sort((a, b) => b.avg - a.avg)[0] || null;
      return { make, bestDay: best };
    });
    // (6) reserve premium by band + >100k bidder/reserve split. has_reserve/bids fill reported.
    const RB = [["under $25k", 0, 25000], ["$25k-50k", 25000, 50000], ["$50k-100k", 50000, 100000], ["$100k+", 100000, Infinity]];
    const reserveFill = S.filter(r => r.has_reserve === true || r.has_reserve === false).length;
    const bidsFill = S.filter(r => Number.isFinite(Number(r.bids)) && r.bids != null).length;
    const reservePremium = RB.map(([name, lo, hi]) => {
      const inb = S.filter(r => { const p = Number(r.sale_price); return p >= lo && p < hi; });
      const res = inb.filter(r => r.has_reserve === true).map(r => Number(r.sale_price));
      const nres = inb.filter(r => r.has_reserve === false).map(r => Number(r.sale_price));
      const avg = a => a.length ? round(a.reduce((x, y) => x + y, 0) / a.length) : null;
      const ar = avg(res), an = avg(nres);
      return { band: name, reservedN: res.length, noReserveN: nres.length, avgReserved: ar, avgNoReserve: an, dollarPremium: (ar != null && an != null) ? ar - an : null, pctPremium: (ar != null && an != null && an > 0) ? Math.round((ar - an) / an * 1000) / 10 : null };
    });
    const over100 = S.filter(r => Number(r.sale_price) >= 100000);
    const over100bids = over100.filter(r => Number.isFinite(Number(r.bids)) && r.bids != null).map(r => Number(r.bids));
    const over100summary = { count: over100.length, avgBids: over100bids.length ? round(over100bids.reduce((a, b) => a + b, 0) / over100bids.length) : null, medianBids: median(over100bids.slice().sort((a, b) => a - b)), reservedPct: over100.length ? Math.round(over100.filter(r => r.has_reserve === true).length / over100.length * 1000) / 10 : null, noReservePct: over100.length ? Math.round(over100.filter(r => r.has_reserve === false).length / over100.length * 1000) / 10 : null };
    // (7/8) NON-SOLD module - STRICTLY SEPARATE. auction_attempts (reserve_not_met + withdrawn).
    const att = await fetchAll("auction_attempts", "attempt_date", "auction_status,high_bid");
    let nonSold = null;
    if (att.error) nonSold = { error: att.error };
    else {
      const rnm = att.filter(a => a.auction_status === "reserve_not_met").length;
      const wd = att.filter(a => a.auction_status === "withdrawn").length;
      const soldN = S.length; const totalAttempts = soldN + rnm + wd;
      const sellThrough = totalAttempts ? Math.round(soldN / totalAttempts * 1000) / 10 : null;
      // (8) by band: sold uses sale_price, non-sold uses high_bid as the band proxy. Gate thin cells.
      const MINCELL = 15;
      const byBand = BANDS.map(([name, lo, hi]) => {
        const sHere = prices.filter(p => p >= lo && p < hi).length;
        const nHere = att.filter(a => { const p = Number(a.high_bid); return p >= lo && p < hi; }).length;
        const tot = sHere + nHere;
        return { band: name, sold: sHere, notSold: nHere, total: tot, sellThroughPct: tot >= MINCELL ? Math.round(sHere / tot * 1000) / 10 : null, suppressed: tot < MINCELL };
      });
      nonSold = { totalAttempts, sold: soldN, reserveNotMet: rnm, withdrawn: wd, sellThroughPct: sellThrough, attemptsFetched: att.length, byBand, bandMinCell: MINCELL, note: "high_bid used as band proxy for non-sold (no sale price exists)" };
    }
    return res.status(200).json({
      task: "nlstats", platform: slug, dateRange: `${from} to ${to}`, source: "sales_archive (sold) + auction_attempts (non-sold)",
      soldSide: {
        lotsSold: S.length, totalVolume: sum, avgPrice: prices.length ? round(sum / prices.length) : null, medianPrice: median(prices),
        impliedCommission: { flat5pct: commissionFlat5, capped_5pct_min250_max7500: commissionCapped, feeNote: "BaT buyer fee is 5%, min $250, max $7,500 per lot; seller pays $0. flat5pct mirrors June's flat assumption; capped is the real per-lot structure." },
        priceBands: bandPct, topMakesByVolume: topMakes,
        weekday, bestDayByAvg: bestDayAvg, worstDayByAvg: worstDayAvg, bestDayByVolume: bestDayVol,
        bestDayByMake, reservePremiumByBand: reservePremium, over100k: over100summary,
        dataQuality: { rowsSold: S.length, has_reserve_populated: reserveFill, bids_populated: bidsFill }
      },
      nonSoldModule: nonSold
    });
  }
