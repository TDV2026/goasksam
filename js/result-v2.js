  ...
  try{
    ...
    return '<div class="pcard pcard-platform" onclick="chooseSellOption(\''+esc(option.key)+'\')">'
      + '<div class="pcard-left">'
        ...
        + (function(){ if(over&&over.whyLabel)return''; try{ var vm=(typeof composerVinMatchBullet==="function")?composerVinMatchBullet(v):null; if(!vm)return''; var bridge=(typeof vinBridgeLine==="function")?vinBridgeLine(sellState.sellDecision&&sellState.sellDecision.vinArchiveMatch, slug||option.name, name, ev):''; var body=vm.text+(bridge?(' '+bridge):''); /* photo display rolled back Sep 2026 (OCD dropped featured_image_url); text + link only. photoUrl stays dormant on the match. See ocd-photo-regression memory. */ return '<div class="pcard-vinmatch">'+esc(body)+(vm.receiptUrl?' <a href="'+esc(vm.receiptUrl)+'" target="_blank" rel="noopener noreferrer" class="vin-receipt-link">View that sale</a>':'')+'</div>'; }catch(e){return'';} })()
